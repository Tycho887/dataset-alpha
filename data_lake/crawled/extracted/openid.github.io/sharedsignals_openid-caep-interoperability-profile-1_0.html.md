---
{
  ...
  "url": "https://openid.github.io/sharedsignals/openid-caep-interoperability-profile-1_0.html",
  ...
  "date": "",
  "length": 19549,
  "crawled_at": "2026-04-23T20:52:32"
}
---

CAEP Interoperability Profile 1.0 - draft 02
caep-interop
April 2026
Tulshibagwale, et al.
Standards Track
...
) and the Continuous Access Evaluation
Profile (
[
CAEP
]
). It specifies required attributes for SSF endpoints, how to
use OAuth 2.0
[
RFC6749
]
for their authorization, and the core use cases
...
between cooperating peers. When combined with Continuous Access
Evaluation Profile (
[
CAEP
]
) to share events such as Session Revocation and
...
¶
Shared Signals Framework Specification 1.0
[
SSF
]
¶
...
events from OpenID Continuous Access
...
2.3.1.
Spec Version
The Transmitter Configuration Metadata MUST include a
spec_version
field, and
its value MUST be
1_0
...
Section 7.1.
¶
The endpoint identified by
configuration_endpoint
is used to perform the
Create Stream operation as defined in
[
SSF
]
Section 8.1.1.
...
(HTTP GET) operation as defined in
[
SSF
]
Sections 8.1.2.1.
...
Stream Management API operations, in addition to the requirements
...
2.3.8.1.
Delivery
The Transmitter MUST support Create Stream requests whose
delivery.method
is
...
(Poll)
¶
When a Receiver makes a Create Stream request as defined in
[
SSF
]
Section
...
.
¶
Upon successful creation of a Stream, the Transmitter MUST include a
delivery
field in the resulting Stream Configuration, and its
method
value MUST be one
...
Section 8.1.1.1 when invoked by a Receiver with valid authorization.
¶
...
Section 8.1.1.2 when invoked by a Receiver with valid
authorization.
¶
Get Stream Status
...
Section 8.1.2.1 when invoked by a Receiver with valid authorization.
¶
Stream Verification
...
Section 8.1.4.2 and MUST generate a Verification Event as defined in
[
SSF
]
Section 8.1.4.1 when requested by a Receiver with valid authorization.
¶
Deleting a Stream
...
Section 8.1.1.5 when invoked by a Receiver with valid authorization.
¶
2.4.
Receivers
...
2.4.2.
JWKS URI
The Receiver MUST obtain the Transmitter's signing key(s) using the
jwks_uri
from the Transmitter Configuration Metadata, as defined in
[
SSF
]
Section 7.1.
...
2.4.3.
Authorization Schemes
The Receiver MUST use OAuth 2.0
[
RFC6749
]
when making Stream Management API
...
2.4.5.1.
Delivery
When creating a Stream using the Create Stream operation defined in
[
SSF
]
Section 8.1.1.1, the Receiver MUST include a
delivery
object whose
method
value is one of the following, or omit the
delivery
object.
¶
urn:ietf:rfc:8935
...
(Poll)
¶
If the Create Stream request does not include the
delivery
property, it is
assumed to be delivery method of
urn:ietf:rfc:8936
(Poll), as
defined in
[
...
Section 8.1.1.1 using valid authorization.
¶
Read Stream Configuration
...
Section 8.1.1.2 using valid authorization.
¶
Get Stream Status
...
Section 8.1.2.1 using valid authorization.
¶
Stream Verification
...
Section 8.1.4.2 using valid authorization, and MUST be able to process
the resulting Verification Event as defined in
[
SSF
]
Section 8.1.4.1.
...
Section 8.1.1.5 using valid authorization.
¶
2.5.
Event Subjects
...
Authorization Server issues access tokens. In the context
...
exp
claim is not longer than 60 minutes after
nbf
claim. Please refer to
access token lifetimes in the security considerations of
[
FAPI
]
for additional
...
then the client MUST obtain the required
scopes by using it.
¶
...
Authorization Server that is used to issue tokens to
...
server MUST return errors as per section 3.1 of
[
RFC6750
]
[
...
to describe the metadata needed to interact with the
protected resource.
¶
2.8.
...
implementations MUST support the CAEP event type
credential-change
.
Within the
credential-change
event, implementations MUST support the following
field values:
¶
change_type
...
status, implementations MUST support the event type
device-compliance-change
.
...
These are covered in the "Security Considerations" sections of
[
SSF
]
and
...
Crowdstrike
Email:
atul.tulshibagwale@crowdstrike.com
Apoorva Deshpande
Okta
Email:
