---
{
  ...
  "url": "https://openid.github.io/sharedsignals/openid-caep-1_0.html",
  ...
  "date": "",
  "length": 30669,
  "crawled_at": "2026-04-23T20:58:02"
}
---
...
(SSF Profile) to ensure access security in a
...
language tag as the key and the locale-specific
...
Figure 1
:
Example: Administrative reason information with multiple languages
reason_user
¶
...
language tag as the key and the locale-specific end-user
...
Figure 2
:
Example: End user reason information with multiple languages
3.
Event Types
...
Figure 3
:
Example: Session Revoked - Required claims + Simple Subject
{
    "iss": "https://idp.example.com/123456789/",
    "jti": "24c63fb56e5a2d77a6b512616ca9fa24",
    "iat": 1615305159,
...
Figure 4
:
Example: Session Revoked - subject as `sub` claim (includes optional claims)
{
    "iss": "https://idp.example.com/123456789/",
    "jti": "24c63fb56e5a2d77a6b512616ca9fa24",
    "iat": 1615305159,
...
Figure 5
:
Example: Session Revoked - Complex Subject describing user + device + tenant (includes optional claims)
3.2.
Token Claims Change
Event Type URI:
¶
...
Figure 6
:
Example: OIDC ID Token Claims Change - Required claims only
{
    "iss": "https://idp.example.com/987654321/",
    "jti": "9afce1e4e642b165fcaacdd0e7aa4903",
    "iat": 1615305159,
...
Figure 7
:
Example: OIDC ID Token Claims Change - Optional claims
{
    "iss": "https://idp.example.com/987654321/",
...
Figure 8
:
Example: SAML Assertion Claims  Change - Required claims only
3.3.
Credential Change
Event Type URI:
¶
...
Figure 9
:
Example: Provisioning a new FIDO2  authenticator - Simple Subject + optional claims
3.4.
...
namespace:
¶
REQUIRED, JSON string: the namespace of the values in the
current_level
and
previous_level
claims. This string MAY be one of the following strings:
¶
RFC8176
: The assurance level values are from the
[
RFC8176
]
specification
...
If the Transmitter has specified the
previous_level
...
Figure 10
:
Example: Assurance Level  Increase - Simple Subject + optional claims
{
    "iss": "https://idp.example.com/3456789/",
    "jti": "07efd930f0977e4fcc1149a733ce7f78",
    "iat": 1615305159,
...
Figure 11
:
Example: Custom Assurance  Level - Simple Subject
3.5.
...
Figure 12
:
Example: Device  No Longer Compliant - Complex Subject + optional claims
3.6.
Session Established
Event Type URI:
¶
...
The
event_timestamp
in this event type specifies the time at which the session
was established.
¶
3.6.1.
Event Specific Claims
...
fp_ua
¶
Fingerprint of the user agent computed by the Transmitter. (
NOTE
, this is
...
the corresponding field in an OpenID Connect ID Token
[
OpenID.Core
]
¶
...
event_timestamp
field in the Session Presented event. Receivers may use this
...
event_timestamp
field in the Risk Level Change event. The Transmitter may
...
or any other entity as defined in Section 2 of
[
SSF
]
...
. Exchanging events described herein without
complying with the Shared Signals Framework
[
SSF
]
...
,
<
https://www.iso.org/iso/iso_catalogue/catalogue_tc/catalogue_detail.htm?csnumber=45138
>
.
[NIST-AUTH]
Grassi, P.
...
,
<
https://www.rfc-editor.org/info/rfc6711
>
.
[RFC8174]
Leiba, B.
