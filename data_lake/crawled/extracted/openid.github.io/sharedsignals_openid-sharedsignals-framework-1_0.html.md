---
{
  ...
  "url": "https://openid.github.io/sharedsignals/openid-sharedsignals-framework-1_0.html",
  "domain": "openid.github.io",
  "depth": 2,
...
¶
A profile for Security Event Tokens
[
RFC8417
]
¶
...
claim named
sub_id
MUST be used to describe the primary subject of the event.
¶
3.1.1.
Existing CAEP and RISC Events
Event types already defined in the CAEP (
[
CAEP
]
) and RISC (
...
specifications MAY use a
subject
field within the
events
claim of the SSF
event to describe the primary Subject Principal of the event. SSF Transmitters
MUST include the top-level
sub_id
claim even for these existing event types.
¶
3.1.2.
New Event Types
New event types MUST use the top-level
sub_id
claim and MUST NOT use the
subject
field in the
events
claim to describe the primary Subject Principal.
¶
3.1.3.
...
. Below is a non-normative example of a Simple Subject Member in an
SSF event.
¶
"sub_id": {
  "format": "email",
...
wildcards, please see the section on Subject Matching (
Section 8.1.3.1
).
¶
3.4.
Subject Identifiers in SSF Events
...
. Subject Identifiers of this type MUST
...
Figure 3
:
Example: 'jwt_id' Subject Identifier
3.5.2.
...
assertion identifier. Subject Identifiers of this
...
Figure 4
:
Example: 'saml_assertion_id' Subject Identifier
3.5.3.
IP Addresses Subject Identifier Format
...
specification by defining certain properties of SETs as described in
this section.
¶
4.1.1.
...
section (
Section 3
). The JWT "sub" claim MUST NOT be present in any SET
...
.
¶
SSF SETs MUST use explicit typing, as described in
Section 4.1.1
.
¶
...
The signature key can be obtained through "jwks_uri", see
Section 7
.
¶
4.1.5.
SSF Prescriptive SETs
...
Figure 7
:
Example: SET with array 'aud' claim
4.1.9.
The "txn" claim
...
The type of the event is specified by the key in the value of the
events
claim. The value of this field is the event object.
¶
4.2.2.
Event type specific fields
The event object inside the
events
claim MAY have one or more fields that are
...
¶
5.
Example SETs that conform to the Shared Signals Framework
...
Figure 8
:
Example: SET Containing an SSF Event with a Simple Subject Member
{
  "iss": "https://idp.example.com/",
  "jti": "756E69717565206964656E746966696572",
  "iat": 1520364019,
...
Figure 9
:
Example: SET Containing a RISC Event with a Phone Number Subject
{
  "iss": "https://idp.example.com/",
  "jti": "756E69717565206964656E746966696572",
  "iat": 1520364019,
...
Figure 10
:
Example: SET Containing a CAEP Event with Properties
{
  "iss": "https://idp.example.com/",
  "jti": "756E69717565206964656E746966696572",
  "iat": 1520364019,
...
Figure 11
:
Example: SET Containing an SSF Event with a Complex Subject Member
{
...
Figure 12
:
Example: SET Containing an SSF Event with a Simple Subject and a Property Member
{
...
Figure 13
:
Example: SET Containing an SSF Event with a Proprietary Subject Identifier Format
6.
Event Delivery
This section describes the supported methods of delivering SSF Events. It
provides SSF profiling specifications for the
[
RFC8935
]
and
...
version as described in the document
[
NAMINGCONVENTION
]
. If absent, the
...
Figure 14
:
Example: spec_version referring to the final 1_0 spec
issuer
¶
...
jwks_uri
¶
OPTIONAL. URL of the Transmitter's JSON Web Key Set
[
RFC7517
]
document.
  ...
  generate signed JWTs. If present, this URL MUST use HTTP over TLS
[
RFC9110
]
.
...
OPTIONAL. An array of JSON objects that specify the supported
  authorization scheme properties defined in
Section 7.1.1
. To enable
...
"ALL" indicates that any subjects that are appropriate for the stream are
  ...
  stream via the
remove_subject_endpoint
, causing events for those subjects
  to
not
be transmitted. The Receiver MAY re-add any subjects removed this
  way via the
add_subject_endpoint
.
¶
"NONE" indicates that no subjects are added by default. The Receiver MAY add
  subjects to the stream via the
add_subject_endpoint
, causing only events
...
The
authorization_schemes
key of Transmitter Configuration Metadata provides
...
(used for Poll-Based SET delivery
[
RFC8936
]
) hosted by the Transmitter.
¶
spec_urn
¶
...
Figure 15
:
Example: 'spec_urn' specifying the OAuth protocol for authorization
In this case, the Receiver may obtain an access token using the Client
Credentials Grant (Section 4.4 of
[
RFC6749
]
), or any other method suitable for the Receiver
and the Transmitter.
¶
7.2.
...
URL comes from a trusted source and uses the
https
scheme.
¶
...
and semantics of ".well-known" are defined in
[
RFC8615
]
.  "ssf-configuration"
...
Figure 16
:
Example: Transmitter Configuration Request (without path)
...
Figure 17
:
Example: Transmitter Configuration Request (with path)
...
Figure 18
:
Example: Transmitter Configuration Request for RISC Transmitters
7.2.3.
...
Figure 19
:
Example: Transmitter Configuration Response
7.2.4.
...
the Event Stream. It is represented as a JSON
[
RFC7159
]
object with the
...
¶
Transmitter-Supplied
, REQUIRED. A URL using the https scheme with no query
...
¶
Transmitter-Supplied
, REQUIRED. A string or an array of strings containing
  an audience claim as defined in JSON Web Token (JWT)
[
RFC7519
]
that
...
¶
Transmitter-Supplied
, OPTIONAL. An array of URIs identifying the set of
...
¶
Transmitter-Supplied
, REQUIRED. An array of URIs identifying the set of
...
¶
Transmitter-Supplied
, OPTIONAL. An integer indicating the minimum amount
...
¶
Receiver-Supplied
, OPTIONAL. A string that describes the properties of the
...
¶
Transmitter-Supplied
, OPTIONAL. The refreshable inactivity timeout of the
...
or delete the stream. The syntax is the same as that of
expires_in
from
Section A.14 of
[
...
¶
For streams created with the PUSH
[
RFC8935
]
delivery method:
...
¶
For streams created with the POLL
[
RFC8936
]
delivery method:
...
Stream Updated Event to the Receiver as described in
Section 8.1.2
.
¶
8.1.1.1.
...
Receiver MUST check the response and confirm that the
iss
value matches the
...
delivery
property, then the Transmitter
MUST assume that the
method
is "urn:ietf:rfc:8936" (poll). If the Transmitter
supports Poll-Based Delivery, the Transmitter MUST include a
delivery
property
in the response with this
method
property and an
endpoint_url
property. If
...
¶
Note that in the case of the poll method, the
endpoint_url
value is supplied
by the Transmitter.
¶
...
403
if the Event Receiver is not allowed to create a stream
409
if the Transmitter does not support multiple streams per Receiver
8.1.1.1.1.
Validating a Create Stream Response
aud
: the Receiver SHOULD validate the
aud
in the Create Stream Response.
...
Figure 23
:
Example: Read Stream Configuration Request
The following is a non-normative example response:
¶
HTTP/1.1 200 OK
Content-Type: application/json
...
Figure 24
:
Example: Read Stream Configuration Response
The following is a non-normative example request to read an Event Stream's
...
404
if there is no Event Stream with the given "stream_id" for this Event Receiver
8.1.1.3.
...
representation of the stream configuration properties to
change. On receiving a valid request, the Event Transmitter responds with a
"200 OK" response containing a JSON
[
RFC7159
]
...
iss
value matches the Issuer from which it received the
...
request MUST NOT be changed by the Transmitter. If
events_requested
property
...
properties MUST be ignored by the Transmitter. The
events_delivered
property,
...
Figure 29
:
Example: Update Stream Configuration Request
The following is a non-normative example response:
¶
HTTP/1.1 200 OK
Content-Type: application/json
...
Figure 30
:
Example: Update Stream Configuration Response
...
Description
202
if the update request has been accepted, but not processed. Receiver MAY try the same request later to get processing result.
400
if the request body cannot be parsed, a Transmitter-Supplied property is incorrect, or if the request is otherwise invalid
401
if authorization failed or it is missing
403
...
representation of the new configuration. On receiving a valid
...
The Receiver MUST check the response and confirm that the
iss
value matches
...
representation, then make a replacement request. If
events_requested
property is included in the request, it SHOULD NOT be an
empty array.
¶
Transmitter-Supplied properties besides the stream_id MAY be present,
but they MUST match the expected value. Missing Transmitter-Supplied
...
events_delivered
property,
if present, MUST match the Transmitter's expected value
before
any updates are
applied.
¶
...
Figure 31
:
Example: Replace Stream Configuration Request
The following is a non-normative example response:
¶
HTTP/1.1 200 OK
Content-Type: application/json
...
Figure 32
:
Example: Replace Stream Configuration Response
Pending conditions or errors are signaled with HTTP status codes as follows:
¶
Table 4
:
Replace Stream Configuration Errors
Code
Description
202
if the replace request has been accepted, but not processed. Receiver MAY try the same request later in order to get processing result.
400
if the request body cannot be parsed, a Transmitter-Supplied property is incorrect, or if the request is otherwise invalid
401
if authorization failed or it is missing
403
if the Event Receiver is not allowed to replace the stream configuration
404
if there is no Event Stream with the given "stream_id" for this Event Receiver
8.1.1.5.
Deleting a Stream
...
Figure 34
:
Example: Delete Stream Response
Errors are signaled with HTTP status codes as follows:
¶
Table 5
:
Delete Stream Errors
Code
Description
401
if authorization failed or it is missing
403
if the Event Receiver is not allowed to delete the stream
404
if there is no Event Stream with the given "stream_id" for this Event Receiver
8.1.2.
Stream Status
...
(
Section 8.1.2.2
). If a Transmitter decides to enable, pause or
disable updates from a stream independently of an update request from a
Receiver, it MUST send a Stream Updated Event (
Section 8.1.5
) to the
Receiver.
¶
...
Figure 35
:
Example: Check Stream Status Request
The following is a non-normative example response:
¶
HTTP/1.1 200 OK
Content-Type: application/json
...
Figure 36
:
Example: Check Stream Status Response
Errors are signaled with HTTP status codes as follows:
¶
Table 6
:
...
Examples:
¶
If a Receiver makes an unauthorized request, then the
Transmitter MUST respond with a 401 error status.
¶
If a Receiver makes an authorized request, but the Transmitter policy
does not permit the Receiver from obtaining the status, then the Transmitter
MAY respond with a 403 error status.
¶
If the Receiver requests the status for a stream that does not exist then the
...
request to the Status Endpoint. The POST body contains a JSON
[
RFC7159
]
object
...
representation of the updated stream
...
Figure 37
:
Example: Update Stream Status Request Without Optional  Fields
...
Figure 38
:
Example: Update Stream Status Request With Optional Reason
The following is a non-normative example response:
¶
HTTP/1.1 200 OK
Content-Type: application/json
...
Figure 39
:
Example: Update Stream Status Response
Errors are signaled with HTTP status codes as follows:
¶
Table 7
:
...
Examples:
¶
If a Receiver makes a request to update a stream status, and the Transmitter
...
Section 8.1.3.2
, the Transmitter SHOULD send any events
...
match if, for all fields in the Complex Subject (i.e.
user
,
group
,
...
404
if there is no Event Stream with the given "stream_id" for this Event Receiver, or if the subject is not recognized by the Event Transmitter. The Event Transmitter may choose to stay silent in this second case and respond with "200"
429
...
Figure 42
:
Example: Remove Subject Request
The following is a non-normative example response to a successful request:
¶
HTTP/1.1 204 No Content
Server: transmitter.example.com
...
Figure 43
:
Example: Remove Subject Response
Errors are signaled with HTTP status codes as follows:
¶
Table 9
:
...
404
if there is no Event Stream with the given "stream_id" for this Event Receiver, or if the subject is not recognized by the Event Transmitter. The Event Transmitter may choose to stay silent in this second case and respond with "204"
429
if the Event Receiver is sending too many requests in a given amount of time
8.1.4.
Verification
...
and / or
events_delivered
fields in the Stream Configuration (
Section 8.1.1
).
¶
8.1.4.1.
...
OPTIONAL. An opaque value provided by the Event Receiver when the event is
  triggered.
¶
As with any SSF event, the Verification Event has a top-level
sub_id
claim:
¶
sub_id
¶
REQUIRED. The value of the top-level
sub_id
claim in a Verification Event
MUST always be set to have a simple value of type
opaque
. The
id
of the
...
object containing the parameters of the verification request, if
...
if there is no Event Stream with the given "stream_id" for this Event Receiver
429
if the Event Receiver is sending too many requests in a given amount of time; see related "min_verification_interval" in
Section 8.1.1
...
Figure 44
:
Example: Trigger Verification Request
The following is a non-normative example response to a successful request:
¶
HTTP/1.1 204 No Content
Server: transmitter.example.com
...
Figure 45
:
Example: Trigger Verification Response
...
OPTIONAL. Provides a short description of why the Transmitter has updated the
  status.
¶
As with any SSF event, this event has a top-level
sub_id
claim:
¶
sub_id
...
sub_id
claim specifies the Stream ID for which the
status has been updated. The value of the
sub_id
field MUST be of format
opaque
, and its
id
...
The
ssf-configuration
well-known endpoint is registered in IANA's Well-Known
URIs registry, as defined by
[
RFC8615
]
.
...
Section 8.1.4.1
, to the Security Event Token Error Codes section of the
Security Event Token registry, as defined in Section 7.1 of
[
RFC8935
]
. The
...
Foundation, O.
,
"OpenID Naming and Content of Specifications"
,
n.d.
,
<
...
Email:
sean.odentity@disney.com
Jen Schreiber
Workday
Email:
jennifer.winer@workday.com
Tushar Raibhandare
Google
Email:
traib@google.com
