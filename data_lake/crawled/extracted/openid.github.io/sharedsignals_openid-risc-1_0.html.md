---
{
  ...
  "url": "https://openid.github.io/sharedsignals/openid-risc-1_0.html",
  ...
  "date": "",
  "length": 15838,
  "crawled_at": "2026-04-23T20:58:05"
}
---

OpenID RISC Profile Specification 1.0
openid-risc-profile-specification
August 2025
Scurtescu, et al.
Standards Track
[Page]
...
OpenID RISC Profile Specification 1.0
Abstract
This document defines the Risk Incident Sharing and Coordination (RISC) Event
        Types based on the
Shared Signals
        Framework (SSF)
[
...
that are required to
      implement Risk Incident Sharing and Coordination.
¶
1.1.
...
¶
Account Credential Change Required signals that the account identified by the subject was required to
        change a credential. For example the user was required to go through a password change.
¶
Attributes: none
¶
{
...
Figure 1
:
Example: Account Credential Change Required
2.2.
Account Purged
Event Type URI:
...
Account Disabled signals that the account identified by the subject has been disabled. The actual reason why the
        account was disabled might be specified with the nested
reason
...
This event SHOULD be issued only by the provider that is authoritative over the identifier.
          For example, if the person that owns
john.doe@example.com
goes
          through a name change and wants the new
...
¶
If an identifier used as a username or recovery option is changed, at a provider that is not
          authoritative over that identifier, then
...
Identifier Recycled signals that the identifier specified in the subject was recycled and now it
        belongs to a new user. The subject type MUST be either
email
or
phone
.
...
Figure 4
:
Example: Identifier Recycled
The
foo@example.com
email address was recycled.
¶
2.7.
...
¶
A Credential Compromise event signals that the identifier specified in the subject was found to
          be compromised.
¶
Attributes:
¶
...
.
¶
event_timestamp - OPTIONAL. JSON number: the time at which the event described by this SET was discovered by the
...
¶
reason_admin - OPTIONAL. The reason why the credential compromised event was generated, intended
              for administrators
¶
reason_user - OPTIONAL. The reason why the credential compromised event was generated, intended
              for end-users
¶
{
...
Figure 5
:
Example: Compromised credential found
2.8.
Opt Out
Users SHOULD be allowed to opt-in and out of RISC events being sent for their accounts. With regards
        to opt-out, an account can be in one of these three states:
¶
opt-in - the account is participating in RISC event exchange.
¶
opt-out-initiated - the user requested to be excluded from RISC event exchanges, but for
          practical security reasons for a period of time RISC events are still exchanged. The main reason
          for this state is to prevent a hijacker from immediately opting out of RISC.
¶
...
Both Transmitters and Receivers SHOULD manage Opt-Out state for users. Transmitters should send the
        events defined in this section when the Opt-Out state changes.
¶
2.8.1.
Opt In
Event Type URI:
...
Opt Out Initiated signals that the account identified by the subject initiated to opt out
          from RISC event exchanges. The account is in the
opt-out-initiated
state.
¶
Attributes: none
...
Opt Out Cancelled signals that the account identified by the subject cancelled the opt out from RISC
          event exchanges. The account is in the
opt-in
state.
¶
Attributes: none
...
Opt Out Effective signals that the account identified by the subject was effectively opted out from
          RISC event exchanges. The account is in the
opt-out
state.
¶
Attributes: none
...
¶
Recovery Information Changed signals that the account identified by the subject has changed some of
        its recovery information. For example a recovery email address was added or removed.
¶
Attributes: none
¶
2.11.
...
¶
Sessions Revoked signals that all the sessions for the account identified by the subject have been
        revoked.
¶
Attributes: none
¶
