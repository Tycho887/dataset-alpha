---
{
  "title": "Initiating User Registration via OpenID Connect 1.0",
  ...
  "date": "",
  "length": 9775,
  "crawled_at": "2026-04-23T20:49:51"
}
---
...
¶
This specification defines a new value for the
prompt
parameter.
¶
2.
...
indicates to the OpenID Provider that
        ...
        login flow. Care must be taken if combining this value with other
prompt
values.
        Mutually exclusive conditions can arise so it is RECOMMENDED that
create
...
metadata field) the OP SHOULD respond with
        an HTTP 400 (Bad Request) status code and an
error
value of
invalid_request
.
        It is RECOMMENDED that the OP return an
error_description
value identifying the invalid parameter value.
¶
In
Figure 1
is an example of an authorization request
          using the
code
response type
...
If integrity protecting the authorization request is required, please refer to
RFC 9101
[
RFC9101
]
