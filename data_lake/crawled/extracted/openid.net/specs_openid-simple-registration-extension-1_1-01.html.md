---
{
  ...
  "url": "https://openid.net/specs/openid-simple-registration-extension-1_1-01.html",
  "domain": "openid.net",
  "depth": 1,
  ...
  "date": "",
  "length": 6280,
  "crawled_at": "2026-04-23T20:50:39"
}
---
...
The request parameters detailed here SHOULD be sent with
      ...
      requests.  This extension works with both versions
1.1
(
Recordon, D. and B. Fitzpatrick, “OpenID Authentication 1.1,” .
)
[OpenIDAuthentication1.1] and
2.0
(
Recordon, D., Hoyt, J., Hardt, D., and B. Fitzpatrick, “OpenID Authentication 2.0 - Draft 10,” 2006.
)
[OpenIDAuthentication2.0] of the OpenID
      Authentication protocol.
All of the following request fields are OPTIONAL, though
...
Comma-separated list of
	  ...
	  are those that are specified in the
Response Format
(
Response Format
)
...
The response's "openid.signed" field list MUST include the
...
If the Consumer's signature verification fails, then
      no registration data from the Identity Provider SHOULD be used.
The Consumer MUST be prepared to handle a response which
...
A single field MUST NOT be repeated in the response, and all
      ...
      this specification.
An Identity Provider MAY return any subset of the following
      fields in response to the query. In particular:
openid.ns.sreg:
"http://openid.net/extensions/sreg/1.1"
openid.sreg.nickname:
Any UTF-8 string that
	  the End User wants to use as a nickname.
openid.sreg.email:
The email address of the
	  End User as specified in section 3.4.1 of
[RFC2822]
(
...
openid.sreg.country:
The
	  End User's country of residence as specified by
ISO3166
.
