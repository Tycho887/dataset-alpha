---
{
  "title": "Final: OpenID Connect\n    Dynamic Client Registration 1.0 incorporating errata set 1",
  "url": "https://openid.net/specs/openid-connect-registration-1_0-errata1.html",
  "domain": "openid.net",
  "depth": 2,
  ...
  "date": "",
  "length": 57405,
  "crawled_at": "2026-04-23T20:51:20"
}
---

Final: OpenID Connect
    Dynamic Client Registration 1.0 incorporating errata set 1
TOC
Final
N. Sakimura
...
authorization_code
: The
	      Authorization Code Grant Type described in OAuth 2.0 Section 4.1.
implicit
: The Implicit Grant Type
	      described in OAuth 2.0 Section 4.2.
refresh_token
: The Refresh Token Grant Type
	      described in OAuth 2.0 Section 6.
The following table lists the correspondence between
response_type
values that the Client will use and
grant_type
...
.
	    ...
	    MUST only register URLs using the
https
scheme as
redirect_uris
;
...
Discovery parameter
	    contains a list of the supported
subject_type
...
REQUIRED for signing the ID Token
	    issued to this Client.
	    The value
none
MUST NOT be used as the ID Token
alg
value
...
REQUIRED for encrypting the ID Token
	    issued to this Client.
...
Claim in the ID Token is REQUIRED.
	    It is REQUIRED when the value is
true
.
	    (If this is
false
, the
auth_time
Claim can still be dynamically requested
	    as an individual Claim for the ID Token
	    using the
claims
request parameter described in
	    Section 5.5.1 of
OpenID Connect Core 1.0
(
Sakimura, N., Bradley, J., Jones, M., de Medeiros, B., and C. Mortimore, “OpenID Connect Core 1.0,” November 2014.
...
(
Sakimura, N., Bradley, J., Jones, M., de Medeiros, B., and N. Agarwal, “OpenID Connect Session Management 1.0,” November 2014.
)
[OpenID.Session].
TOC
2.1. 
...
To support open Dynamic Registration, the Client
...
To register a new Client at the Authorization Server, the Client
	  sends an HTTP
POST
message to the Client Registration Endpoint with
...
If a Client Configuration Endpoint and a Registration Access Token
	  ...
	  the Authorization Server MUST provide the Client with the fully
        qualified URL in the
registration_client_uri
element of the Client Registration Response, per
Section 3.2
(
Client Registration Response
)
...
Depending on deployment characteristics, the Client Configuration
        ...
        ID
s6BhdRkqt3
could be given a Client
        Configuration Endpoint URL of
...
Upon a successful read operation, the Authorization Server
	  ...
	  Note that some values, including the
client_secret
...
5. 
"sector_identifier_uri" Validation
The sector identifier list provides a way for a group of Web sites under
      single administrative control
      to have consistent pairwise
sub
values,
...
https
scheme that references
	a JSON file containing an array of
redirect_uri
values.
	The values registered in
redirect_uris
...
Processing some OpenID Connect messages requires comparing
	...
	compared to specific member names such as
client_id
...
(
Richer, J., Jones, M., Bradley, J., Machulak, M., and P. Hunt, “OAuth 2.0 Dynamic Client Registration Protocol,” August 2014.
)
[I‑D.ietf‑oauth‑dyn‑reg], draft -20.
...
(
Richer, J., Jones, M., Bradley, J., and M. Machulak, “OAuth 2.0 Dynamic Client Registration Management Protocol,” August 2014.
)
[I‑D.ietf‑oauth‑dyn‑reg‑management], draft -05,
...
In a situation where the Authorization Server is supporting open Client 
	  ...
	  be displayed to the End-User (e.g.
logo_uri
and
policy_uri
). A rogue Client could 
	  specify a registration request with a reference to a drive-by download in the
policy_uri
. The Authorization Server SHOULD check to see if the
logo_uri
and
policy_uri
have the 
	  same host as the hosts defined in the array of
redirect_uris
.
TOC
9.2. 
...
Richer, J., Jones, M., Bradley, J., Machulak, M., and P. Hunt, “
OAuth 2.0 Dynamic Client Registration Protocol
,” draft-ietf-oauth-dyn-reg-20 (work in progress), August 2014 (
TXT
).
[I-D.ietf-oauth-dyn-reg-management]
Richer, J., Jones, M., Bradley, J., and M. Machulak, “
OAuth 2.0 Dynamic Client Registration Management Protocol
,” draft-ietf-oauth-dyn-reg-management-05 (work in progress), August 2014 (
TXT
).
[OpenID.Session]
Sakimura, N., Bradley, J., Jones, M., de Medeiros, B., and N. Agarwal, “
OpenID Connect Session Management 1.0
,” November 2014.
TOC
Appendix A. 
