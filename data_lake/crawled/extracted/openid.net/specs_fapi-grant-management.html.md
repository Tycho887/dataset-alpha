---
{
  ...
  "url": "https://openid.net/specs/fapi-grant-management.html",
  "domain": "openid.net",
  "depth": 1,
  ...
  "date": "",
  "length": 30626,
  "crawled_at": "2026-04-23T20:49:04"
}
---
...
¶
Other use cases that are covered OAuth Incremental Authorization (
...
: this mode requires the client to specify a grant id using the
grant_id
paramter. If the parameter is present and the AS supports the grant management action
replace
...
In case the
grant_id
is unknown or invalid, the authorization server SHALL respond with an error code
invalid_grant_id
.
¶
...
metadata) but not specified, SHALL respond with the error code
invalid_request
.
¶
5.4.
...
grant_id
if it supports any of the grant management actions
query
,
revoke
,
...
6.2.
Endpoint
The grant management API is provided by a new endpoint provided by the authorization server. The client MAY utilize the server metadata parameter
...
¶
scopes
: JSON array where every entry contains a
scope
field  and may contain one or more
resource
fields. This structure allows the AS to represent the relationship between scope values and the resource indicators (as defined in
[
RFC8707
]
) they where requested and approved with. The concrete mapping is at the discretaion of the AS. The AS could,
...
¶
claims
: JSON array containing the names of all OpenID Connect claims (see
[
OpenID
]
) as requested and consented in one or more authorization requests associated with the respective grant.
¶
authorization_details
: JSON Object as defined in
[
I-D.ietf-oauth-rar
]
...
A client (as logical entity) MAY use multiple client ids to deliver its service across different platforms, e.g. apps for iOS and Android and a Web App. It is RECOMMENDED that the AS supports sharing of grants among client ids belonging to the same client. Sector identifier URIs as defined in
[
OpenID.Registration
]
...
grant_id
is issued by the authorization server for each established grant between a client and a user. This should prevent correlation between different clients.
¶
...
¶
grant_id
potentially could be shared by different client_id belonging to the same entity.
¶
10.
...
¶
During the execution of a transaction utilising grant mode
replace
...
,
<
https://tools.ietf.org/html/draft-ietf-oauth-rar-05
>
.
[RFC7231]
Fielding, R., Ed.
