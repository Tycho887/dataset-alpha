---
{
  ...
  "date": "",
  "length": 23830,
  "crawled_at": "2026-04-23T20:49:54"
}
---
...
OPTIONAL.
	    ...
	    (This parameter is intended to be analogous to the
login_hint
...
[OpenID.Core]
	    ...
	    whereas,
logout_hint
is used in RP-Initiated Logout Requests.)
client_id
OPTIONAL.
...
is used but
id_token_hint
is not. Another use is for
	    symmetrically encrypted ID Tokens used as
id_token_hint
values
...
Registration parameter or via another mechanism.
	    An
id_token_hint
is also RECOMMENDED when this parameter is included.
state
OPTIONAL.
...
OpenID Providers MUST support the use of the HTTP
GET
and
POST
methods defined in
...
parameter
	...
	An
id_token_hint
carring an ID Token for the RP
...
post_logout_redirect_uri
value supplied does not exactly match
...
parameter
	      after a logout has been performed.
	      These URLs SHOULD use the
https
scheme
	      and MAY contain port, path, and query parameter components;
	      however, they MAY use the
http
scheme,
	      provided that the Client Type is
confidential
...
6. 
Security Considerations
Logout requests without a valid
id_token_hint
value
