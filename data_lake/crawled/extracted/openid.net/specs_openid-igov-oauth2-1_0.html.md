---
{
  "title": "International Government Assurance Profile (iGov) for OAuth 2.0 - draft 09",
  ...
  "date": "",
  "length": 79034,
  "crawled_at": "2026-04-23T20:48:29"
}
---

International Government Assurance Profile (iGov) for OAuth 2.0 - draft 09
iGov OAuth 2.0
December 2025
Burgin & Clancy
Standards Track
...
1.
Introduction
This document profiles the OAuth 2.0 web authorization framework
                for
                ...
                purposes:
¶
Define a mandatory baseline set of security controls suitable
                        for
...
¶
Identify optional, advanced security controls for sensitive use
...
¶
The specification also defines features for interaction between
                    these components:
¶
...
¶
When an iGov-compliant component is interacting with other
...
1.4.
Global Requirements
iGov implementations SHOULD be crypto-agile, enabling rapid substitution of algorithms and keys so services remain secure as threats and standards evolve. Implementers of this profile SHOULD monitor the progress of specifications of post-quantum
                 cryptography.
¶
All network connections MUST be made using TLS 1.3 or above.
                 Each originator of a TLS connection MUST verify the destination's certificate.
                 However, the following four TLS 1.2 cipher suites MAY be used when interoperability is required:
¶
...
¶
When post quantum safety is required for TLS, implementers SHOULD adopt a cipher suite included in
NIST Post-Quantum Cryptography Standards
[
NISTPQC
]
,
                 such as X25519MLKEM768 or SecP256r1MLKEM768, as specified in
Post-quantum hybrid ECDHE-MLKEM Key Agreement for TLSv1.3
[
ECDHE.MLKEM
]
.
                 Of these, X25519MLKEM768 is broadly supported at the time of writing.
¶
Authorization servers, clients, and resource servers, when creating or processing JWTs
¶
1. MUST adhere to [RFC8725]
¶
2. MUST use PS256, ES256, or Ed25519
¶
3. MUST NOT use or accept the
none
algorithm.
¶
...
¶
NOTE: Permitted cipher suites are those listed in
BCP195
[
BCP195
]
...
¶
Endpoints for use by web browsers MUST use mechanisms to ensure that connections cannot be downgraded
...
to mitigate these attacks. Protected resources SHOULD
...
Clients that have credentials with the authorization server.
¶
public clients:
Clients without credentials. Public client use cases are out of scope for this profile.
¶
2.2.
Client Type Use Cases
This specification has been designed around the following client use cases:
¶
web application:
...
¶
native application:
A native application is a client installed and
...
. In this use case, native applications are public clients.
¶
browser-based application:
A browser-based application is a client
                          ...
                          However, best current practices are detailed in
...
.
¶
This profile establishes requirements that can only be met by confidential clients.
...
2.3.
Client Registration
All clients MUST register with the authorization server. For
...
¶
Client registration MAY be completed by either out-of-band configuration or using the
...
to sender constrain tokens, the client MUST include the dpop_bound_access_tokens parameter
                in its registration metadata.
¶
Clients using mTLS for client authentication or to sender-constrain tokens MUST register their TLS certificate's subject DN with the
                authorization server.
                Clients using self-signed certificate option are not guaranteed uniqueness of their certificate fingerprint.
¶
2.3.1.
Redirect URI
Clients using the authorization code grant type
                        MUST register
                        their full redirect URIs.
¶
Clients MUST protect the values passed back to its redirect
                        URI by ensuring that the redirect URI is one of the following:
¶
Hosted on a website with Transport Layer Security (TLS)
...
¶
Clients MUST NOT forward values passed back to their redirect
                        URIs to other arbitrary or user-provided URIs (a practice known as an "open redirector").
¶
Refer to
...
While a bearer token can be used by anyone in possession of the token, a sender-constrained token is
...
, sender-constrained tokens could
...
¶
All clients MUST use proof of possession to sender-constrain access tokens using either
...
to communicate authentication context and implement interoperable step up authentication.
¶
This profile acknowledges government use cases will likely operate within an ecosystem of authentication methods of highly variable security
                ...
                policies using access tokens augmented with the strength and recency of the authentication event that led to the issuance of each specific access token.
¶
This profile leverages the supporting server metadata, request, token claims and values, and error messages from
OAuth 2.0
...
.
¶
Digital identity policies and semantic mappings to string values are necessary for implementation but are out of scope for this profile.
¶
...
is RECOMMENDED to provide the identity
...
All clients MUST use the PKCE S256 code challenge method as described in
...
¶
Clients that have multiple client types MUST have a separate client ID for each client type.
¶
Clients MUST include their full redirect URI in the
                        authorization request.
¶
The client MAY specify a strength of authentication and maximum age to the authorization server that should be met
                        when issuing an access token for the requesting client by including parameters in the authorization request:
¶
acr_values
...
, the authentication context conveys information about how
...
¶
max_age
a non-negative integer value that indicates the allowable elapsed time in seconds since the last active authentication event
                            associated with the access token.
¶
Furthermore, if the authorization request is a follow-up to a prior request that did not meet the resource server's initial or subsequent
...
¶
The following is a sample response from a client to
                        the
...
Status: 302 Found
¶
This causes the browser to send the following (non-normative) request to the
...
. The JWT assertion MUST be signed
...
, the request MUST be made over a mutually authenticated TLS channel.
¶
2.6.3.
Client Keys
...
¶
For reference, the corresponding public/private key pair for
                        this
...
¶
Note that the second example contains both the public and
                        private
...
.
¶
An example of an OAuth-protected call to the OpenID Connect
...
.
¶
The authorization server MUST protect all communications to and
                from
...
authorization_code
grant type and MAY support the
client_credentials
grant type. The implicit grant type MUST NOT be used.
¶
Authorization servers MUST limit each registered client (identified by a client ID)
                    to a single client type only, since a single piece of software will be functioning at runtime as only one client type.
¶
3.1.2.
Client authentication
Authorization servers MUST enforce client authentication for confidential clients. Public clients cannot authenticate
                    to the authorization server.
¶
The authorization server MUST validate all redirect
                        URIs for authorization code grant types.
¶
The authorization server MUST confirm thumbprints of client keys.
¶
Authorization servers MUST only grant access to higher level scope resources to
...
¶
When a client requests both a higher level scope and a lower level scope in the original authorization request, Authorization
                    ...
                    access has expired without requiring an additional user authentication/authorization.
¶
3.1.3.
...
for authorization code grant types. Authorization
...
¶
grant_types
grant type used by the client; must be
...
¶
client_uri
URL of a web page containing further
                                information
                                about the client
¶
...
OPTIONAL. Indicates the client will include the acr_values and max_age
...
¶
jwks_uri or jwks
client's public key in a JWK Set [RFC7517], or
...
3.1.4.
Client Approval
When prompting the end user with an interactive approval page,
                        the authorization server MUST indicate to the user:
¶
Whether the client was dynamically registered, or else
                                statically registered by a trusted administrator.
¶
Whether the client is associated with a software statement,
                                and in which case provide information about the trusted issuer
                                of the software statement.
¶
What kind of access the client is requesting, including
                                scope, protected resources (if applicable beyond scopes),
                                and access duration.
¶
For example, if the user believes they are authorizing a very limited type of access,
                     providing the access level at the consent screen may help the user make informed decisions about
                     sharing sensitive information.
¶
3.1.5.
Sender-constrained Tokens
The authorization server MUST support and verify sender-constrained tokens.
¶
The Authorization Server MUST NOT issue the client an access token if the client included the
tls_client_certificate_bound_access_tokens
...
¶
issuer
REQUIRED. The fully qualified issuer URL of the
                                server
¶
...
Indicates that client authentication to the authorization server will occur using mutual TLS
                                with the client utilizing a self-signed certificate.
¶
If the authorization server is also an OpenID Connect Provider,
...
.
¶
The following example shows a JSON document found at the
...
¶
{
  ...
  "userinfo_signing_alg_values_supported": [
    "HS256", "HS384", "HS512", "PS256", "RS384", "RS512"
  ],
...
¶
It is RECOMMENDED that authorization servers provide cache information through HTTP headers and make the cache valid for at
                    least one week. This allows clients and protected resources to cache the discovery information.
¶
The authorization server MUST provide its public key in JWK Set format. The
                        key
                        MUST contain the following fields:
¶
kid
The key ID of the key pair used to sign this
                                token
¶
kty
The key type
...
3.1.7.
Revocation
Token revocation allows a client to signal to an authorization
                        server that a given token will no longer be used.
¶
Authorization servers MUST provide a revocation endpoint URL as
                    specified in
OAuth 2.0 Token Revocation
[
RFC7009
...
for end users to view a list of clients that have been granted access to resources on the
                    user's behalf and for end users to revoke this access.
¶
Authorization servers MUST revoke a token if the client
...
¶
Authorizatin servers MUST NOT allow a client to use a token
                        after it is revoked.
¶
3.1.8.
PKCE
Authorization servers MUST require use of PKCE
                        (
Proof Key for Code Exchange by OAuth Public Clients
[
RFC7636
                        ...
                        and reject requests to the authorization endpoint from clients that do not contain a code challenge.
¶
Authorization servers MUST support the S256 code challenge method. Authorization servers MUST NOT allow a
                        client to use the plain code challenge method.
¶
The PKCE code_verifier value MUST contain at least 128 bits of entropy.
¶
Authorization servers MUST confirm thumbprints of client keys.
¶
3.1.9.
Redirect URIs
The authorization server MUST validate all redirect URIs for authorization code grant types.
¶
To prevent open redirection and other injection attacks, the authorization server MUST match the entire redirect
                    URI using an exact string match against registered values and MUST reject requests with an invalid or missing redirect URI.
¶
3.1.10.
Refresh Tokens
...
Authorization servers respond to an Authorization Request from a client with an Authorization Response.
                The following fields MUST be included in the response:
¶
state
REQUIRED. The value of the state parameter passed in in the
                            authentication request. This value MUST match exactly.
¶
code
REQUIRED. The authorization code, a random string issued by
...
3.3.
JSON Web Tokens (JWT)
Authorization servers MUST issue
...
¶
The server MUST issue tokens as JWTs with, at minimum, the
                    following claims:
¶
iss
The issuer URL of the server that issued the
                            token
¶
client_id
The client id of the client to whom this token
                            was
                            issued
¶
exp
The expiration time (integer number of seconds
                            since from 1970-01-01T00:00:00Z UTC), after which the token MUST
                            be considered invalid
¶
jti
A unique JWT Token ID value with at least 128
                            bits
...
¶
sub
The identifier of the end-user that authorized
                            this
...
¶
aud
The audience of the token, an array containing
                            the
...
¶
cnf
The confirmation claim, as defined in
...
is used in a JWT to contain
                                            members used to identify the proof-of-possession key.
¶
When
OAuth 2.0 Mutual-TLS
                                            Client Authentication and Certificate-Bound Access Tokens
...
is used, the confirmation claim value MUST be computed using the
JWK SHA-256
                                            Thumbprint
[
RFC7638
]
of the DPoP public key and include conformation method value jkt.
¶
In order to support backwards compatibility, AS MAY include the azp claim while resource servers
                        modernize.
¶
The following sample claim set illustrates the use of the
...
The key ID of the key pair used to sign this
                            token
¶
This example access token has been signed with the server's
                    private key using PS256:
¶
NOTE: '\' line wrapping per RFC 8792
...
3.4.
Token Lifetimes
Access tokens SHOULD have a valid lifetime no greater than one hour, and
...
3.5.
Scopes
Scopes define individual pieces of authority that can be
                   requested
...
¶
Authorization servers SHOULD define and document default scope
...
Token introspection allows a protected resource to query the
                        authorization server for metadata about a token. Authorization servers MUST NOT allow clients to use token introspection endpoints.
¶
The protected
...
.
¶
Authorization servers respond to an introspection request with a JSON
...
Section 2.6.2
. Authorization servers MUST NOT allow clients to
...
¶
Protected resources MUST check the
aud
(audience) claim in tokens to ensure that it
                    includes the protected resource's identifier.
¶
...
to sender-constrain tokens, the protected resource
...
¶
If the authentication event associated with the access token does not satisfy the requirements of the resource server for the given request,
                the protected resource MUST return a 401 Unauthorized status code along with a WWW-Authenticate header as defined in
OAuth
                2.0 Step Up Authentication Challenge Protocol
[
RFC9470
]
. This header MUST include the insufficient_user_authentication error code to indicate that the
                presented access token is inadequate.
¶
Additionally, the response MUST include the acr_values and/or max_age auth-params to communicate the necessary authentication context
                ...
                the maximum permissible time in seconds since the last user authentication.
¶
The mechanisms by which the protected resource determines whether the authentication requirements are met are outside the scope of this profile.
¶
Furthermore, protected resources MAY include both acr_values and max_age if both are relevant.
¶
Protected resources MAY include the scope parameter if additional scopes are required to access the resource, as per Section 3.1 of
...
Protected Resources grant access to clients if they present a valid sender-constrained
access_token
...
¶
If a protected resources requires a higher end-user authentication trust
...
For example, a resource server associates
...
for the "public" and "sensitive" scopes
                    with an expiry time of 15 minutes and a
refresh_token
...
¶
The client makes a
                    refresh token request (as described in
OAuth 2.0
[
RFC6749
]
...
¶
In this manner, protected resources and authorization servers work together to meet
                    risk tolerance levels for sensitive resources and end-user authentication.
¶
4.3.
Connections with Authorization Servers
Protected resources MUST provide a JWKS URI endpoint to distribute public keys to support signing, encryption, and
                authentication to authorization server revocation and introspection endpoints.
...
¶
Protected resources MUST limit which authorization servers it
                    will
...
For a comprehensive protection against network attackers, all endpoints should additionally use
DNSSEC
[
RFC9364
]
...
5.2.
Best Practices
Authorization server, client, and protected resource implementations SHOULD consider including requirements from
Best Current Practice for OAuth 2.0 Security
[
BCP240
]
...
Authorization Servers SHOULD take device posture into account, when possible. Some examples of device posture include:
¶
the user's lock screen setting
¶
the client's level of privilege over the device operating system (e.g. if the client has 'root access', the
                    device operating system may be compromised to gain additional privileges not intended by the vendor)
¶
...
with normative
                ...
                to mitigate the threats in
[
RFC6973
]
¶
In OpenID Connect implementations, cilents and servers SHOULD implement the privacy threat mitigations in Section 17 of
OpenID Connect Core 1.0
[
OpenID.Core
]
...
,
<
http://www.rfc-editor.org/info/bcp240
>
.
[browser-based-apps]
Parecki, A.
...
The OpenID Community would like to thank the following people for
                ...
                Stas Mironov, and Andy Regenscheid.
¶
Special thank you to the original iGov Profile editors: Paul Grassi, Justin Richer,
                and Michael Varley.
¶
...
¶
The OpenID Foundation (OIDF) grants to any Contributor, developer, implementer, or other interested party a non-exclusive, royalty free, worldwide copyright license to reproduce,
            ...
            documents, provided that attribution be made to the OIDF as the source of the material, but that such attribution does not indicate an endorsement by the OIDF.
¶
The technology described in this specification was made available from contributions from various sources, including members of the OpenID Foundation and others. Although the
            ...
            to practice this specification.
¶
Appendix C.
Document History
...
-09
¶
Revised Global Requirements for cryptography to emphasize crypto-agility and align with FAPI
¶
Revised Post-Quantum guidance with NIST references and ECDHE-MLKEM draft
¶
Eliminated RS256 requirements throughout and partially updated examples with PS256
¶
Fixed typos and eliminated redundant or orphaned requirements
¶
...
¶
Added guidance text around using scopes and refresh_tokens to protect sensitive resources.
¶
