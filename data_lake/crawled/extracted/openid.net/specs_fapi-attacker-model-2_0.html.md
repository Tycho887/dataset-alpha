---
{
  ...
  "url": "https://openid.net/specs/fapi-attacker-model-2_0.html",
  "domain": "openid.net",
  "depth": 2,
  ...
  "date": "",
  "length": 22022,
  "crawled_at": "2026-04-23T20:49:15"
}
---

FAPI 2.0 Attacker Model
fapi-attacker-model-2
February 2025
Fett
Standards Track
[Page]
...
for FAPI 1.0, which this work draws from. Formal proofs
...
Section 9
, has helped to refine and improve
...
5.2.
Authorization
The FAPI 2.0 Security Profile aims to ensure that
no attacker can
...
attacker types listed here. This is supported by a formal security analysis, see
Section 9
.
¶
...
¶
TLS:
It is assumed that TLS connections are not broken, i.e.,
...
¶
JWKS:
Where applicable, key distribution mechanisms work as
...
¶
Note:
An attacker that can read the authorization response is not
...
¶
Note:
The attackers for the authorization request are more
...
¶
Note:
For the authorization and resource endpoints, it is assumed that the
...
¶
7.6.
Attacker at the token endpoint: A4 - read and tamper with token requests and responses
...
¶
7.7.
Attacker at the resource server: A5 - read resource requests
...
¶
Note:
An attacker that can read the responses from the resource server is not
...
that provides a formal model of the FAPI 2.0 Security Profile
...
A4
A7
A5 - with reduced capabilities, see note in
Section 7.7
A8
removed - see note in
Section 7.7
