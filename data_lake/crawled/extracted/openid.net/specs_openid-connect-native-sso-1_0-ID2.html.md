---
{
  "title": "OpenID Connect Native SSO for Mobile Apps 1.0 - draft 07",
  "url": "https://openid.net/specs/openid-connect-native-sso-1_0-ID2.html",
  "domain": "openid.net",
  "depth": 1,
...
specification allowing mobile apps to share identity (SSO)
...
,
 the term "native app" defined by
RFC 8252
[
RFC8252
]
...
¶
After step 8, Native App #1 stores the device_secret and id_token
    in the shared device storage accessible only to Native App #2.
¶
Native App #2 uses the stored data from the shared device storage
    to obtain tokens for the user thus enabling the app to access
    the user's resources (i.e. SSO)
¶
+----------+     +----------+      +-----------+      +------------+
|  Native  |     |  Native  |      |  System   |      |            |
|  App     |     |  App     |      |  Browser  |      |    AS      |
...
will be
   ...
   authorization request.
¶
The new scope value is defined as
device_sso
.
...
The device secret contains relevant data to the device and the
   ...
   is completely opaque to the client and as such the AS MUST
          ...
          is not maintaining state on the backend.
¶
In the context of this extension the device secret may be shared
   ...
   requests a device secret via the
device_sso
scope and a
device_secret
exists, then
...
3.3.
Token Request
The native app interacts with the token endpoint in order to retrieve the
device_secret
and other tokens. This
       specification applies to the
authorization_code
and
refresh_token
grant types. Support for other
       grant types is out of scope for this specification.
¶
This specification defines the following additional
   parameter to the /token endpoint for the
authorization_code
and
refresh_token
grant types:
...
applies. If
device_sso
is included in the set of requested scopes
   the Authorization Service MUST return the
device_secret
.
¶
In the case of the
...
grant, the Authorization
   Server must determine from the optional
scope
parameter or
   the scopes originally associated with the refresh token if the
device_sso
scope is part of the request. If so, the
   Authorization Server MUST return the
device_secret
.
¶
3.4.
...
device_secret
is provided as part of the token request,
   and the
device_secret
is invalid, then the AS must
   process the request as if no
device_secret
was specified.
   Additionally, in the response from the token endpoint, the
device_secret
and the
id_token
MUST both be issued or neither issued.
¶
The following sections describe additional claims for the
id_token
and when processing specific grant types.
¶
3.4.1.
ID Token Claims
This specification defines additional requirements for the
id_token
issued
...
id_token
representing the logged in
       user and the
device_secret
representing the user's device and session.
¶
Add a
ds_hash
...
REQUIRED. The ds_hash value provides a binding between
   ...
   and device_secret.
¶
Add a session id to the id_token that represents the user's
...
¶
Note that the implementation of this spec and the specification of the
ds_hash
and
sid
...
device_sso
scope, then the Authorization Server MUST issue a
device_secret
and an
id_token
...
3.4.3.
Refresh Token Grant
The Authorization Server MUST issue a
device_secret
if
...
¶
If the Authorization Server issues a new
device_secret
as
   part of the token response, then the AS MUST also issue an updated
id_token
with the required claims.
¶
4.
Token Exchange Profile for Native SSO
...
¶
4.1.
OAuth2 Token Exchange Profile
The Authorization Server requires the
client_id
of
   the requesting OAuth client. The
client_id
value can be
   obtained from the parameter defined by
OAuth 2.0
[
RFC6749
]
or via
   client authentication methods that identify the
client_id
.
...
OPTIONAL. The client_id value of the requesting OAuth client. This parameter
   is REQUIRED if the client is a
public
client.
¶
Additionally, this profile defines the use of the following token exchange parameters:
¶
grant_type
...
When a mobile app wants to request native SSO (i.e. obtain refresh and access
   tokens for an already signed in user) it makes a standard OAuth2 /token
   endpoint request following the profile for Token Exchange defined above.
¶
...
The
client_id
in this request is sent via the
client_id
parameter. The
client_id
may also be provided via standard
   client authentication methods.
¶
4.3.
Native SSO Processing Rules
When the authorization server receives a request at the token endpoint
   conforming to this profile it MUST perform the following checks before
   issuing any tokens:
¶
Validate the device_secret to ensure the token is still valid. The
   format of this secret is defined by the Authorization server and
   is out of scope for this specification.
¶
Verify the id_token for integrity protection by validating the
...
parameter.
¶
Verify that the session id in the id_token (sid claim) is still
...
¶
Validate that the client requesting native SSO is authorized to
   ...
   authentications.
¶
The AS SHOULD verify that the scopes requested by the client in
...
Based on the AS definition of the device_secret, the AS may perform
   ...
   expires, all refresh_tokens associated with it MUST be invalidated.
¶
4.4.
...
In the case that the AS requires the user to interact with the AS directly, for example to
   ...
   OpenID Connect defined error of
interaction_required
. This
   instructs the client to start an authorization request with the AS via the standard web
   based flows, as described in
Section 3.1
.
¶
In the case of any errors, the AS MUST return a valid OAuth2 Error response
   as described in Section 2.2.2 of the Token Exchange spec.
¶
The following is a non-normative example
¶
HTTP/1.1 200 OK
...
Sufficient care must be made to protect the
device_secret
.
...
6.2.
Cross-Device SSO
If it is possible to move correctly bound
device_secret
and
id_token
to a separate device, that device can obtain
 ...
 to prevent exfiltration of the
device_secret
and
id_token
from the device.
...
¶
6.4.
User Identity across SSO events
...
Parameter usage location: Authorization Response,
   Access Token Request, Access Token Response
¶
