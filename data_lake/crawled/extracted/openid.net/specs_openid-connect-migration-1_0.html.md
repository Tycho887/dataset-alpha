---
{
  "title": "Final: OpenID 2.0 to\n    OpenID Connect Migration 1.0",
  "url": "https://openid.net/specs/openid-connect-migration-1_0.html",
  "domain": "openid.net",
  "depth": 1,
  ...
  "date": "",
  "length": 25269,
  "crawled_at": "2026-04-23T20:49:44"
}
---
...
Terminology
2.
Requesting the OpenID 2.0 Identifier and OpenID Connect Identifier Together
3.
Verification of the Relying Party by the OpenID Provider
4.
Returning the OpenID 2.0 Identifier
4.1.
Error Responses
5.
Verification of the ID Token
6.
Verification that the OpenID Connect OP is Authoritative
7.
Associating the Existing OpenID 2.0 Account with the OpenID Connect Identifier
8.
Implementation Considerations
8.1.
After End-Of-Life of the OpenID 2.0 OP
9.
Privacy Considerations
9.1.
Correlation
9.2.
Identification by Other Parties
9.3.
Secondary Use
9.4.
Disclosure
...
Sequence Diagrams
Appendix B.
Differences from Google’s Migration Guide as of June 3, 2014
Appendix C.
Acknowledgements
Appendix D.
Notices
...
OpenID Authentication 2.0 is a popular authentication federation
      ...
      those user identifiers to continue serving these users.
...
[OpenID.2.0] are used by this
        specification. When the same term is defined in both specifications,
        the term defined in OpenID Connect Core takes precedence.
This specification also defines the following terms:
OpenID 2.0 Identifier
Verified Claimed
            Identifier as specified by OpenID Authentication 2.0.
Connect OP
OpenID Connect OP
...
To obtain the OpenID 2.0 Identifier, the RP sends a modified OpenID
      Connect Authentication Request by adding
openid2
as an additional scope value.
NOTE: If the OP does not support the
openid2
scope, 
...
REQUIRED. The
openid.realm
value as defined in Section 9.1 of
OpenID 2.0
(
OpenID Foundation, “OpenID Authentication 2.0,” December 2007.
)
...
Yadis
(
Miller, J., “Yadis Specification 1.0,” March 2005.
)
[Yadis].
      In the Yadis case, it is using an Accept header with
      its value set to
application/xml+xrds.
...
The End-User performs authentication and 
...
To verify the issuer in the ID Token is 
      	authoritative for
openid2_id
, get the issuer from 
...
iss
claim of ID Token 
		exactly matches the value of
iss
claim of this response.
TOC
3. 
Verification of the Relying Party by the OpenID Provider
...
If the verification of the Relying Party was successful and an
      ...
      with the following claim name:
openid2_id
OpenID 2.0 Identifier.
          It MUST be represented as a JSON string.
If the associated OpenID 2.0 Identifier for the user is not found, 
      then the OP MUST NOT include this claim in the response.
The following is a non-normative example of ID Token claims with
...
5. 
Verification of the ID Token
The RP MUST verify the ID Token as specified in 3.1.3.7 of
OpenID Connect Core 1.0
(
Sakimura, N., Bradley, J., Jones, M., de Medeiros, B., and C. Mortimore, “OpenID Connect Core 1.0,” November 2014.
)
...
A malicious OP may try to impersonate the user by returning the
      ...
      verification rules hold:
If the RP a priori knows that the authority hosted only one
          ...
          the OpenID 2.0 OP’s OP Endpoint URL.
If they are not (or when a higher confidence is sought), RP MUST
          make a GET call to the obtained verified claimed ID with an Accept
          header set to
application/json
. 
          The server SHOULD return a JSON with
iss
...
If all three fail, the verification has failed and the RP MUST NOT accept the
      OpenID 2.0 Identifier.
NOTE: XRI users may need to configure the forwarding service 
    and define (+openid_iss) themselves.
The following is a non-normative example of obtaining  
...
7. 
Associating the Existing OpenID 2.0 Account with the OpenID Connect Identifier
Once the association between the OpenID Connect Identifier and 
      OpenID 2.0 Identifier has been
      verified, the RP SHOULD associate the existing OpenID 2.0 account with
      the OpenID Connect account.
TOC
8. 
Implementation Considerations
There are OpenID 2.0 libraries that use
openid.identity
instead of
openid.claimed_id
to link to the user account at
      the RP. This is a bug as
openid.identity
may be recycled.
...
RFC 6973
(
Cooper, A., Tschofenig, H., Aboba, B., Peterson, J., Morris, J., Hansen, M., and R. Smith, “Privacy Considerations for Internet Protocols,” July 2013.
)
[RFC6973].
TOC
9.1. 
...
When multiple accounts are available for the user, the OP MUST
        make sure that the user picks the intended identity.
TOC
9.2. 
 Identification by Other Parties
Since the channel is encrypted, this risk is low. If the channel
...
. If in doubt, skipping step 1 and just doing step
      2 is safer.
TOC
11. 
References
...
[RFC6973]
Cooper, A., Tschofenig, H., Aboba, B., Peterson, J., Morris, J., Hansen, M., and R. Smith, “
...
Appendix B. 
Differences from Google’s Migration Guide as of June 3, 2014
In this appendix, the differences between this spec and
      ...
      this specification. Google's migration guide is available at
...
openid2_realm
.
Google uses the existence of the
openid.realm
parameter to switch the
      behavior at the Connect OP. The new scope value
openid2
...
may cause confusion among
      ...
      providing
openid2_id
even after the OpenID
      2.0 OP has been taken down, this claim may persists much longer than the
      OpenID 2.0 OP. Thus, the chance of confusion should be minimized.
Google does not take care of
XRI
(
Reed, D. and D. McAlpin, “Extensible Resource Identifier (XRI) Syntax V2.0,” November 2005.
)
