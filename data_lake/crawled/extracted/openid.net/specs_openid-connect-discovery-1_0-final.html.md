---
{
  "title": "Final: OpenID Connect Discovery 1.0",
  "url": "https://openid.net/specs/openid-connect-discovery-1_0-final.html",
  "domain": "openid.net",
  "depth": 2,
  ...
  "date": "",
  "length": 50913,
  "crawled_at": "2026-04-23T20:51:17"
}
---

Final: OpenID Connect Discovery 1.0
TOC
Final
N. Sakimura
NRI
...
2.2.2.
User Input using URL Syntax
2.2.3.
User Input using Hostname and Port Syntax
2.2.4.
...
WebFinger
(
Jones, P., Salgueiro, G., Jones, M., and J. Smarr, “WebFinger,” September 2013.
)
[RFC7033] to locate
...
NOTE: this document defines various kinds of Identifiers, designed for use in different contexts.
	      Examples include URLs using the
https
scheme and e-mail addresses.
...
OpenID Connect uses the following discoverable
rel
value in
WebFinger
(
...
To start discovery of OpenID endpoints, the End-User supplies an
      ...
      Then it makes an HTTP
GET
request to the Host's
WebFinger
(
Jones, P., Salgueiro, G., Jones, M., and J. Smarr, “WebFinger,” September 2013.
)
...
and
rel
parameters to obtain the location of the
      requested service.
...
NOTE: A URI relative reference includes a string that looks like 
          an e-mail address in the form of
userinfo@host
. 
...
The Identifier normalization rules MAY be extended by
	  ...
	  such as telephone numbers or
XRIs
(
Reed, D. and D. McAlpin, “Extensible Resource Identifier (XRI) Syntax V2.0,” November 2005.
)
[XRI_Syntax_2.0] to also be used.
TOC
2.1.1. 
User Input Identifier Types
A user input Identifier can be categorized into the following 
            types, which require different normalization processes:
User input Identifiers starting with the
XRI
(
Reed, D. and D. McAlpin, “Extensible Resource Identifier (XRI) Syntax V2.0,” November 2005.
)
[XRI_Syntax_2.0] global context
              symbols ('=','@', and '!') are RESERVED.  Processing of
              these identifiers is out of scope for this
              specification.
All other user input Identifiers MUST be treated as
...
. 
	    ...
	    However, it is also 
            a valid userpart "@" host section of an
acct
URI
[I‑D.ietf‑appsawg‑acct‑uri]
(
Saint-Andre, P., “The 'acct' URI Scheme,” July 2013.
)
,
	    and this specification 
            treats it such as to exclude various
            extra strings allowed in
addr-spec
of
RFC 5322
(
...
is prepended to the Identifier.
...
2.2.2. 
User Input using URL Syntax
To find the Issuer for the given URL,
https://example.com/joe
, the WebFinger
...
example.org
might have a local account named
joe@example.com
.
	    ...
	    identifier when constructing the
acct:
URI
...
REQUIRED.
	    URL using the
https
scheme with no query or fragment component that the OP asserts as its
	    Issuer Identifier.
	    If Issuer discovery is supported (see
Section 2
(
OpenID Provider Issuer Discovery
)
...
[RFC6749] scope values that this server
	    supports. The server MUST support the
openid
scope value.
...
values that this
	    OP supports.
	    Dynamic OpenID Providers MUST support the
code
,
id_token
, and the
...
OPTIONAL.
	    JSON array containing a list of the JWE
[JWE]
(
Jones, M., Rescorla, E., and J. Hildebrand, “JSON Web Encryption (JWE),” November 2013.
)
...
registration parameter.
	    Pre-registration is REQUIRED when the value is
true
.
	    If omitted, the default value is
false
...
RFC 5785
(
Nottingham, M. and E. Hammer-Lahav, “Defining Well-Known Uniform Resource Identifiers (URIs),” April 2010.
)
[RFC5785] and apply to the Issuer value
	when it contains no path component.
openid-configuration
MUST point to a JSON document compliant with this specification and
	MUST be returned using the
application/json
content type.
TOC
4.1. 
...
value returned MUST be identical to
	  ...
	  This MUST also be identical to the
iss
Claim value
	  in ID Tokens issued from this Issuer.
TOC
...
An attacker may also attempt to impersonate an OpenID Provider by publishing
	  a Discovery document that contains an
issuer
Claim
...
Claim
	  ...
	  and that this also exactly matches the
iss
Claim
...
Saint-Andre, P., “
The 'acct' URI Scheme
,” draft-ietf-appsawg-acct-uri-06 (work in progress), July 2013 (
TXT
).
[JWA]
Jones, M., “
