---
{
  ...
  "date": "",
  "length": 27292,
  "crawled_at": "2026-04-23T20:49:59"
}
---
...
(
Jones, M. and J. Bradley, “OpenID Connect Back-Channel Logout 1.0,” September 2022.
)
[OpenID.BackChannel]
...
In OpenID Connect, the session at the RP typically starts when the RP
	...
	Refer to the OpenID Connect Core 1.0
[OpenID.Core]
(
Sakimura, N., Bradley, J., Jones, M., de Medeiros, B., and C. Mortimore, “OpenID Connect Core 1.0,” November 2014.
)
...
[RFC7159]
(
Bray, T., Ed., “The JavaScript Object Notation (JSON) Data Interchange Format,” March 2014.
)
string that represents the End-User's login state
	    at the OP.
...
The Session State value is initially calculated on the server.
	...
	The generation of suitable Session State values is specified in
Section 3.2
(
OP iframe
)
,
	...
	following the algorithm specified in Section 4 of
RFC 6454
(
Barth, A., “The Web Origin Concept,” December 2011.
)
[RFC6454].
TOC
3. 
...
unless the syntax of the message sent was determined by the OP to be malformed,
	  in which case the received data will be
error
.
	  Upon receipt of
changed
, the RP MUST perform re-authentication with
prompt=none
to obtain the current
	  session state at the OP.
	  Upon receipt of
error
, the RP MUST NOT perform re-authentication
	  with
prompt=none
, so as to not cause potential
...
document.domain
to comply with same-origin restrictions.
...
.
	  The RP MUST assign an
id
attribute to the iframe so that it can
...
.
	  ...
	  at the Client in response to spurious
changed
events.
...
In addition, the User Agent state used to verify the session state
	  ...
	  Calls to check_session() will return
changed
against earlier versions of session state after such events.
	  ...
	  traffic caused by the Client's response to
changed
notifications.
...
This can prevent the ability for notifications from the OP at the RP
	  ...
	  the data needed to recalculate
session_state
might not be available.
	  Cookie based implementations might then return
changed
...
[OpenID.BackChannel] is not known to be affected by these developments.
TOC
6. 
Security Considerations
The OP iframe MUST enforce that the caller is from an expected origin.
...
,” RFC 6454, DOI 10.17487/RFC6454, December 2011.
[RFC6749]
Hardt, D., Ed., “
...
Microsoft
Email:
Naveen.Agarwal@microsoft.com
URI:
https://www.linkedin.com/in/nvnagr
Nat Sakimura
NAT.Consulting
Email:
nat@nat.consulting
