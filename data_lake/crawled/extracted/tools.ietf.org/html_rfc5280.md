---
{
  ...
  "date": "",
  "length": 347758,
  "crawled_at": "2026-04-23T20:51:34"
}
---
...
95
7.1
. Internationalized Names in Distinguished Names ............
96
7.2
. Internationalized Domain Names in GeneralName .............
97
Cooper, et al.              Standards Track                     [Page 3]
RFC 5280
PKIX Certificate and CRL Profile            May 2008
7.3
. Internationalized Domain Names in Distinguished Names .....
98
7.4
...
Section 2
describes Internet PKI requirements and the assumptions
   that affect the scope of this document.
Section 3
presents an
...
Section 4
profiles the X.509 version 3 certificate, and
Section 5
...
Section 6
includes certification path validation procedures.  These
...
Procedures for identification and encoding of public key materials
   and digital signatures are defined in [
RFC3279
], [
RFC4055
], and
...
RFC4491
   ...
   implementations that use the algorithms identified in [
RFC3279
],
   [
RFC4055
...
RFC4491
] MUST identify and encode the public key
...
Appendix
A
contains all ASN.1 structures defined or referenced within this
   specification.  As above, the material is presented in the 1988
   ASN.1.
Appendix B
contains notes on less familiar features of the
   ASN.1 notation used within this specification.
Appendix C
contains
...
Section 7
, with rules for encoding and comparing
...
, which specified the
      ...
      *
Section 4.2.1.5
recommends marking the policy mappings extension
        as critical.
RFC 3280
required that the policy mappings
        extension be marked as non-critical.

      *
Section 4.2.1.11
requires marking the policy constraints
...
and
5.3
clarify the rules for handling unrecognized
        CRL extensions and CRL entry extensions, respectively.

      *
Section 5.3.2 in RFC 3280
, which specified the
...
The users of certificates will operate in a wide range of
   ...
   system [
X.500
] or a Lightweight Directory Access Protocol (LDAP)
   directory system [
RFC4510
].  The profile does not prohibit the use of
...
Following is a simplified view of the architectural model assumed by
   ...
   [
RFC2560
], certificate revocation lists (CRLs), or some other
...
RFC1422
   ...
   deploy
RFC 1422
made it clear that the v1 and v2 certificate formats
   ...
   1996, standardization of the basic v3 format was completed [
X.509
].
...
PKIX Certificate and CRL Profile            May 2008
RFC 1422
uses the X.509 v1 certificate format.  The limitations of
   ...
   With X.509 v3, most of the requirements addressed by
RFC 1422
can be
...
RFC4055
   ...
   [
RFC4491
] list supported signature algorithms, but other signature
...
Section 4.1.2.6
   ...
   attribute types are provided in the ASN.1 modules in
Appendix A
.
...
convert such names into DNS names.  The syntax and associated OID for
   this attribute type are provided in the ASN.1 modules in
Appendix A
.
   Rules for encoding internationalized domain names for use with the
   domainComponent attribute type are specified in
Section 7.3
.
...
.  If the names in the issuer and subject field in a
   certificate match according to the rules specified in
Section 7.1
,
...
4.1.2.4
   ...
   in
Section 4.2.1.3
, is present and the value of cRLSign is TRUE),
...
5.1.2.3
   ...
   (
Section 4.1.2.4
).  Implementations of this specification MUST be
   ...
   provided in the ASN.1 modules in
Appendix A
.  Implementations of this
   specification MAY use the comparison rules in
Section 7.1
to process
...
(c)  TeletexString, BMPString, and UniversalString are included
   ...
   attribute [
RFC2985
].  The attribute value for emailAddress is of type
...
4.1.2.8
.  Unique Identifiers
These fields MUST only appear if the version is 2 or 3 (
Section
4.1.2.1
...
The extensions defined for X.509 v3 certificates provide methods for
   ...
   (
Section 4.2.1.1
).  An extension includes the boolean critical, with
   ...
   Conforming CAs MUST support key identifiers (Sections
4.2.1.1
and
...
this specification; certificate issuers are cautioned that marking
   ...
   the following extensions: key usage (
Section 4.2.1.3
), certificate
   policies (
Section 4.2.1.4
...
4.2.1.14
   ...
   the authority and subject key identifier (Sections
4.2.1.1
and
   4.2.1.2) and policy mappings (
Section 4.2.1.5
) extensions.
4.2.1
.  Standard Extensions
This section identifies standard certificate extensions defined in
   [
X.509
] for use in the Internet PKI.  Each extension is associated
   with an OID defined in [
X.509
].  These OIDs are members of the id-ce
...
that generates unique values.  Two common methods for generating key
   identifiers from the public key are described in
Section 4.2.1.2
.
...
Section 4.2.1.9
   ...
   field of the authority key identifier extension (
Section 4.2.1.1
) of
...
duplicated, only one copy need be displayed.  To prevent such
   ...
   option.
      ...
      normalized according to Unicode normalization form C (NFC) [
NFC
].
...
address, a DNS name, an IP address, and a Uniform Resource Identifier
   ...
   as described in
Section 4.1.2.4
.  Note that where such names are
   ...
   rfc822Name is a "Mailbox" as defined in
...
.
   ...
   internationalized domain names are specified in
Section 7.5
.
...
addition, while the string " " is a legal domain name, subjectAltName
   ...
   encoding internationalized domain names are specified in
Section 7.2
.
...
.
   ...
   field in
Section 4.1.2.4
.  The DN MUST be unique for each subject
   entity certified by the one CA as defined by the issuer field.  A CA
   MAY issue more than one certificate with the same DN to the same
   subject entity.
   ...
   Kerberos [
RFC4120
] format names can be encoded into the otherName,
...
As with
Section 4.2.1.6
, this extension is used to associate Internet
...
Section 6
.  (That is, issuer alternative names are not used in name
...
Within this profile, the minimum and maximum fields are not used with
   ...
   emailAddress (
Section 4.1.2.6
).  When constraints are imposed on the
...
Section 7.1
.  CAs issuing certificates with a restriction of the form
   ...
   The syntax of iPAddress MUST be as described in
Section 4.2.1.6
with
   the following additions specifically for name constraints.  For IPv4
   addresses, the iPAddress field of GeneralName MUST contain eight (8)
   octets, encoded in the style of
RFC 4632
...
The CRL distribution points extension identifies how CRL information
   ...
   Further discussion of CRL management is contained in
Section 5
.
...
RFC2585
   ...
   header field of the response.  When the LDAP URI scheme [
RFC4516
] is
...
The freshest CRL extension identifies how delta CRL information is
   ...
   CAs.  Further discussion of CRL management is contained in
Section 5
.

   The same syntax is used for this extension and the
   cRLDistributionPoints extension, and is described in
Section
4.2.1.13
...
Each entry in the sequence AuthorityInfoAccessSyntax describes the
   ...
   specified in [
RFC4523
].  The protocol that application uses to access
...
RFC4516
   ...
   the DER encoded certificates or cross-certificate pairs [
RFC4523
],
...
RFC2585
   ...
   specify the media type application/pkcs7-mime [
RFC2797
] in the
   ...
   encoded certificate SHOULD have a suffix of ".cer" [
RFC2585
] and the
...
RFC2797
   ...
   accessLocation that is an HTTP [
RFC2616
] or LDAP [
RFC4516
] URI.
...
specifications for the supported services.  This extension may be
   ...
   [
RFC4523
].  The protocol the application uses to access the directory
...
RFC4516
   ...
   [
RFC3161
].  Where the timestamping services are available via HTTP or
...
If the scope of the CRL includes one or more certificates issued by
   ...
   CRL will be issued in the nextUpdate field (
Section 5.1.2.5
), include
   the CRL number extension (
Section 5.2.3
), and include the authority
...
CertificateList  ::=  SEQUENCE  {
   ...
   -- are all defined in the ASN.1 in
Section 4.1
...
The signatureAlgorithm field contains the algorithm identifier for
   ...
   The field is of type AlgorithmIdentifier, which is defined in
Section
4.1.1.2.  [RFC3279]
, [
RFC4055
...
This field indicates the date by which the next CRL will be issued.
   ...
   [
X.509
].  The behavior of clients processing CRLs that omit
...
5.1.2.7
.  Extensions
This field may only appear if the version is 2 (
Section 5.1.2.1
).  If
...
The authority key identifier extension provides a means of
   ...
   The syntax for this CRL extension is defined in
Section 4.2.1.1
.
5.2.2
.  Issuer Alternative Name
...
.
   ...
   The OID and syntax for this CRL extension are defined in
Section
4.2.1.7
.
5.2.3
...
The CRL number is a non-critical CRL extension that conveys a
   ...
   That is, if the this update field (
Section 5.1.2.4
) in the two CRLs
...
Section 4.2.1.13
.  However, only the distribution point field is
...
This section defines the use of the Authority Information Access
   extension in a CRL.  The syntax and semantics defined in
Section
4.2.2.1
for the certificate extension are also used for the CRL
   extension.
...
RFC2797
   ...
   [
RFC4523
].  When different public keys are used to validate
   signatures on certificates and CRLs, the desired certificate is
   stored in the userCertificate attribute as specified in [
RFC4523
].
...
X9.55
   ...
   issuers SHOULD include reason codes (
Section 5.3.1
) and invalidity
   dates (
Section 5.3.2
...
Certification path validation procedures for the Internet PKI are
   based on the algorithm supplied in [
X.509
].  Certification path
   ...
   In
Section 6.1
, the text describes basic path validation.  Valid
...
validation procedure is the same regardless of the choice of trust
   ...
   trust anchors.
Section 6.2
describes methods for using the path validation algorithm
   in specific implementations.
Section 6.3
describes the steps necessary to determine if a
...
This text describes an algorithm for X.509 path processing.  A
   ...
   While the certificate and CRL profiles specified in Sections
4
and
5
of this document specify values for certificate and CRL fields and
   ...
   conformance to the profiles in Sections
4
and
5
, this profile
...
The trust anchor is an input to the algorithm.  There is no
   ...
   different paths, as discussed further in
Section 6.2
.
   ...
   (for all i in [1..n]) are listed below.
      ...
      (a)  Verify the basic certificate information.  The certificate
         ...
         (3)  At the current time, the certificate is not revoked.  This
              ...
              (
Section 6.3
), by status information, or by out-of-band
              mechanisms.
...
(e)  If the certificate policies extension is not present, set the
   ...
   listed in
Section 6.1.4
.  If i is equal to n, perform the wrap-up
   steps listed in
Section 6.1.5
.
...
example, a trusted CA may only be trusted for a particular
   ...
   An implementation MAY augment the algorithm presented in
Section 6.1
to further limit the set of valid certification paths that begin with
   ...
   presented in
Section 6.1
defines the minimum conditions for a path to
   be considered valid.
...
Section 6.1
does not assume that trust anchor information is provided
...
.
         ...
         (2)  If the current time is before the value of the next update
              ...
              locally cached complete CRL as specified in
Section 5.2.4
.
...
(3)  Verify that the delta CRL authority key identifier
      ...
      (i)  If use-deltas is set, then search for the certificate on the
           ...
           issuer and serial number as described in
Section 5.3.3
, then
...
(1)  If the reason code CRL entry extension is present, set the
      ...
      (j)  If (cert_status is UNREVOKED), then search for the
           ...
           in
Section 5.3.3
, then set the cert_status variable to the
...
Representation of internationalized names in distinguished names is
   covered in Sections
4.1.2.4
, Issuer Name, and 4.1.2.6, Subject Name.
...
RFC4518
   ...
   preparation algorithm described in [
RFC4518
...
Internationalized Domain Names (IDNs) may be included in certificates
   ...
   ASCII Compatible Encoding (ACE) format as specified in
Section 4 of
   RFC 3490
before storage in the dNSName field.  Specifically,
   conforming implementations MUST perform the conversion operation
   specified in
Section 4 of RFC 3490
, with the following
   clarifications:
   ...
   noted in
Section 4.2.1.10
, any DNS name that may be constructed by
   ...
   conversion operation specified in
Section 4 of RFC 3490
, with the
...
Domain Names may also be represented as distinguished names using
   ...
   specified in
Section 4.1 of RFC 3490
.  The label SHALL be considered
...
.
   ...
   Specifically, conforming implementations should perform the
   "ToUnicode" conversion operation specified, as described in
Section
7.2
, on each ACE label before displaying the name.
7.4
.  Internationalized Resource Identifiers
Internationalized Resource Identifiers (IRIs) are the
   ...
   sequences of characters from the ASCII character set.  [
RFC3987
]
...
To accommodate IRIs in the current structure, conforming
   implementations MUST map IRIs to URIs as specified in
Section 3.1 of
   [RFC3987]
...
RFC3987
   ...
   implementations MUST prepare URIs for comparison as follows:
      ...
      *  Step 1: Where IRIs allow the usage of IDNs, those names MUST be
         converted to ASCII Compatible Encoding as specified in
Section
7.2
above.
...
.
      ...
      *  Step 5: If recognized, the implementation MUST perform scheme-
         based normalization as specified in
Section 5.3.3 of [RFC3987]
.
...
.
   ...
   conversion of the host-part of the Mailbox as described in
Section
7.2
.
8
...
delta CRLs are issued more frequently than the full CRLs, then
   ...
   of the certification path validation mechanism described in
Section 6
that omit revocation checking provide less assurance than those that
   ...
   described in
Section 6
relies upon the integrity of the trusted CA
...
RFC2821
   ...
   [
RFC2985
].  As a result, there is a risk that two different email
...
corrupted certificates or CRLs contain links to malicious code.
   ...
   key update" are specified in [
RFC4210
], where the CA protects its new
...
ISO8859
   ...
   specified in
Section 7.1
assume that TeletexStrings are encoded as
...
, January 2005.
   ...
   [
RFC4516
]  Smith, M., Ed., and T. Howes, "Lightweight Directory
...
ISO10646
   ...
   [
NFC
]      Davis, M. and M. Duerst, "Unicode Standard Annex #15:
...
, January 1999.
   ...
   [
RFC2560
]  Myers, M., Ankney, R., Malpani, A., Galperin, S., and C.
...
, September 2005.
   ...
   [
RFC4325
]  Santesson, S. and R. Housley, "Internet X.509 Public Key
...
[
RFC4630
]  Housley, R. and S. Santesson, "Update to DirectoryString
...
, August 2006.
   ...
   [
X.500
]    ITU-T Recommendation X.500 (2005) | ISO/IEC 9594-1:2005,
              Information technology - Open Systems Interconnection -
              The Directory: Overview of concepts, models and services.

   [
X.501
]    ITU-T Recommendation X.501 (2005) | ISO/IEC 9594-2:2005,
              Information technology - Open Systems Interconnection -
...
X.520
   ...
   [
X.660
]    ITU-T Recommendation X.660 (2004) | ISO/IEC 9834-1:2005,
              Information technology - Open Systems Interconnection -
...
UTF8String ::= [UNIVERSAL 12] IMPLICIT OCTET STRING
      -- The content of this type conforms to
RFC 3629
.
...
RFC 5280
PKIX Certificate and CRL Profile            May 2008
-- Naming attributes of type DomainComponent (from
RFC 4519
)
...
, serial numbers can be expected to
   ...
   As noted in
Section 5.2.3
, CRL numbers can be expected to contain
...
The character string type TeletexString is a superset of
   ...
   characters allowed by [
ISO10646
].  ISO 10646 is the Universal
   multiple-octet coded Character Set (UCS).
...
RFC2277
   ...
   For many of the attribute types defined in [
X.520
], the
   AttributeValue uses the DirectoryString type.  Of the attributes
   specified in
Appendix A
, the name, surname, givenName, initials,
...
Appendix A
, in order to avoid the use of parameterized type
   ...
   in
Appendix A
describes the syntax for each of these attributes as
...
Implementers should note that the DER encoding of the SET OF values
   ...
   support OIDs where the length of the dotted decimal (see
Section 1.4
   of [RFC4512]
) string representation can be up to 100 bytes
   ...
   in [
RFC3986
].  The content-specific rules for encoding values in the
   nameConstraints extension are specified in
Section 4.2.1.10
, and
...
RFC2459
], the predecessor to this profile
...
This appendix contains four examples: three certificates and a CRL.
   ...
   certification path.
Appendix C.1
contains an annotated hex dump of a "self-signed"
...
Appendix C.2
contains an annotated hex dump of an end entity
