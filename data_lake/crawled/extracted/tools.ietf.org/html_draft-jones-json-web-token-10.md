---
{
  ...
  "url": "https://tools.ietf.org/html/draft-jones-json-web-token-10",
  ...
  "date": "",
  "length": 41609,
  "crawled_at": "2026-04-23T20:48:13"
}
---
...
14
9.3
.  Registration of application/jwt MIME Media Type  . . . . .
14
9.4
.  Registration of "JWT" Type Value . . . . . . . . . . . . .
15
10
...
RFC4627
   ...
   is performed using JSON Web Signature (JWS) [
JWS
].  Encryption is
   performed using JSON Web Encryption (JWE) [
JWE
].
...
[RFC4648], Section 5
, with the (non URL-
      safe) '=' padding characters omitted, as permitted by
Section 3.2
.
      (See
Appendix C
...
RFC3986
   ...
   IntDate  A JSON numeric value representing the number of seconds from
      1970-01-01T0:0:0Z UTC until the specified UTC date/time.  See
RFC
3339
[
RFC3339
] for details regarding date/times in general and UTC
      in particular.
3
.  JSON Web Token (JWT) Overview
JWTs represent a set of claims as a JSON object that is base64url
   ...
   Claims Set represents this JSON object.  As per
RFC 4627
[RFC4627]
   Section 2.2
, the JSON object consists of zero or more name/value
   ...
   Signature (JWS) [
JWS
] and/or encrypted in the manner described in
...
Internet-Draft            JSON Web Token (JWT)                  May 2012
defined in the IANA JSON Web Token Claims registry
Section 9.1
or be
   ...
   o  Universally Unique IDentifier (UUID) as defined in
RFC 4122
[
RFC4122
].
...
JWS
   ...
   are defined by [
JWE
].  This specification further specifies the use
...
[
RFC5226
] sense.  The registry records
   the reserved claim name and a reference to the RFC that defines it.
   This specification registers the claim names defined in
Section 4.1
.
9.2
...
This specification registers the following "typ" header parameter
   ...
   established by the JSON Web Algorithms (JWA) [
JWA
] specification:
...
(work in progress),
   ...
   [
JWA
]      Jones, M., "JSON Web Algorithms (JWA)", May 2012.
...
RFC3275
   ...
   JSON [
RFC4627
] object encoding syntax.  It also supports securing
...
-07
   ...
   o  Reference and use content from [
JWS
] and [
JWE
], rather than
