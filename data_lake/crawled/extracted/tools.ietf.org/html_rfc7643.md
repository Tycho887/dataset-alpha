...
RFC 7643
SCIM Core Schema              September 2015
Copyright Notice
...
While there are existing standards for describing and exchanging user
   ...
   this schema via an HTTP-based protocol [
RFC7644
].  The SCIM
...
RFC4512
   ...
   propagating personal information (see
Section 9.3
).
...
RFC2119
   ...
   The word "DEFAULT" as used in
Section 7
indicates that a "keyword"
...
Service Provider
   ...
   Endpoint
      ...
      to the service provider's Base URI (see
Section 1.3 of [RFC7644]
),
...
A SCIM server provides a set of resources, the allowable contents of
   ...
   [
XML-Schema
].  Instead, SCIM's support of schema is attribute based,
   ...
   context of a SCIM protocol request (see [
RFC7644
]).  For example, a
...
A resource is a collection of attributes identified by one or more
   ...
   [
RFC7159
] format and MUST specify schema via the "schemas" attribute
   per
Section 3
.
...
2.3
.  Attribute Data Types
Attribute data types are derived from JSON [
RFC7159
].  The JSON
   format defines a limited set of data types; hence, where appropriate,
   alternate JSON representations derived from XML Schema [
XML-Schema
]
...
Table 1 maps the following SCIM data types to their corresponding
   ...
   | Type      | "type"      |                                         |
   +-----------+-------------+-----------------------------------------+
   | String    | "string"    | String per
Section 7 of [RFC7159]
|
   |           |             |                                         |
   | Boolean   | "boolean"   | Value per
Section 3 of [RFC7159]
|
   |           |             |                                         |
   | Decimal   | "decimal"   | Number per
Section 6 of [RFC7159]
|
   |           |             |                                         |
   | Integer   | "integer"   | Number per
Section 6 of [RFC7159]
|
   |           |             |                                         |
   | DateTime  | "dateTime"  | String per
Section 7 of [RFC7159]
|
   |           |             |                                         |
...
RFC4648
   ...
   |           |             | 5 of [
RFC4648
] that is passed as a JSON |
   |           |             | string per
Section 7 of [RFC7159]
|
   |           |             |                                         |
   | Reference | "reference" | String per
Section 7 of [RFC7159]
|
   |           |             |                                         |
   | Complex   | "complex"   | Object per
...
Section 7
   of [RFC7159]
.  An attribute with SCIM schema type "string" MAY
...
Section 3 of [RFC7159]
.  A boolean has no case sensitivity or
   uniqueness.
...
A real number with at least one digit to the left and right of the
   period.  The JSON format is defined in
Section 6 of [RFC7159]
.  A
...
.  Unless
   ...
   string per
Section 7 of [RFC7159]
.  A binary is case exact and has no
   uniqueness.
2.3.7
.  Reference
...
.  However, the base URI for relative URI
...
endpoint); e.g., the base URI for a request to
   ...
   per
Section 7 of [RFC7159]
.  A reference is case exact.  A reference
   has a "referenceTypes" attribute that indicates what types of
   resources may be linked, as per
Section 7
of this document.
...
Section 4 of [RFC7159]
.  The order of the component attributes is not
...
Multi-valued attributes contain a list of elements using the JSON
   array format defined in
Section 5 of [RFC7159]
.  Elements can be
...
Each SCIM resource is a JSON object that has the following
   ...
   Resource Type
      ...
      ("meta.resourceType"; see
Section 3.1
) that defines the resource's
...
Section 6
).  This same value is also present in the resource's
      "schemas" attribute.

   Extended Attributes
      Extended schema attributes are specified by the resource's
      resource type "schemaExtensions" attribute (see
Section 6
).
...
Each SCIM resource (Users, Groups, etc.) includes the following
   ...
   characteristics (see
Section 2.2
) listed here SHALL take precedence
   ...
   id
      ...
      "always".  See
Section 9
for additional considerations regarding
      privacy.
   ...
   externalId
      ...
      controlled by the client setting the value.  See
Section 9
for
...
RFC7232
         ...
         support for versioning (see
Section 3.14 of [RFC7644]
).  If a
...
), then the origin server MUST mark the
...
Section 6
and defines a schema representation (see
Section 8.7
).
3.3
.  Attribute Extensions to Resources
SCIM allows resource types to have extensions in addition to their
   ...
   [
RFC4512
].  However, unlike LDAP, there is no inheritance model; all
   ...
   to retrieve the resource's "ResourceType" schema (see
Section 6
).
...
This section defines the default resource schemas present in a SCIM
   ...
   to support other resource types (see
Section 3.2
).
4.1
.  "User" Resource Schema
...
Section 1.1.3 of [RFC3986]
) and that points to a location
      representing the user's online profile (e.g., a web page).  URIs
      are canonicalized per
Section 6.2 of [RFC3986]
.
...
.  The intent of this value is to
      enable cloud applications to perform matching of language tags
      [
RFC4647
] to the user's language preferences, regardless of what
...
interaction that does not involve a user (such as in a delegated
      OAuth 2.0 [
RFC6749
] style interaction) where normal HTTP
...
RFC6557
      ...
      also known as the "Olson" time zone database format [
Olson-TZ
]
...
Section 9.2
for acceptable hashing and encryption handling when
...
RFC5321
      ...
      value according to [
RFC5321
], e.g., "bjensen@example.com" instead
...
RFC3966
   ...
   photos
      A URI that is a uniform resource locator (as defined in
Section 1.1.3 of [RFC3986]
) that points to a resource location
...
taken by this user; instead, profile photos of the user that are
   ...
   addresses
      ...
      country  The country name component.  When specified, the value
         MUST be in ISO 3166-1 "alpha-2" code format [
ISO3166
]; e.g.,
...
sub-attribute MUST be the "id", and the "$ref" sub-attribute must
      ...
      "Group" Resource (
Section 4.2
).  This attribute has a mutability
      of "readOnly".
...
), which MUST be base64
      encoded per
Section 4 of [RFC4648]
.  A single value MUST NOT
...
IANA has added an entry to the "IETF URN Sub-namespace for Registered
   ...
   for the Registered Parameter Identifier as per [
RFC3553
]:
...
Namespace ID:
   ...
   urn:ietf:params:scim:{type}:{name}{:other}
      ...
      name
         ...
         requirements (see [
RFC2141
]) and defines a major namespace of a
...
Relevant Ancillary Documentation:
   ...
   Process of Identifier Assignment:
      ...
      "scim@ietf.org", as documented in
Section 10.3
.
...
This section defines the process for registering new SCIM schemas
   ...
   Schema URIs" registry (see
Section 10.1
).  A schema URI is used as a
   value in the "schemas" attribute (
Section 3
) for the purpose of
...
The IANA has populated the "System for Cross-domain Identity
   ...
   | core:2.0:ServiceProviderConfig    | Configuration     |
Section 5
|
   |                                   | Schema            |           |
   |                                   |                   |           |
   | urn:ietf:params:scim:schemas:     | Resource Type     | See       |
   | core:2.0:ResourceType             | Configuration     |
Section 6
|
   |                                   |                   |           |
   | urn:ietf:params:scim:schemas:     | Schema            | See       |
   | core:2.0:Schema                   | Definitions       |
Section 7
|
   |                                   | Schema            |           |
   +-----------------------------------+-------------------+-----------+
...
, DOI 10.17487/RFC3553,
              June 2003, <
http://www.rfc-editor.org/info/rfc3553
>.
...
RFC 3966
, DOI 10.17487/RFC3966, December 2004,
              <
