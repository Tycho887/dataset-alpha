...
provides options for
  ...
  and remote processing in Sections
3.1
and
3.2
...
Section 3
, protected resource access in Step
...
) define endpoints that also utilize client authentication,
...
, which defines mechanisms for dynamically registering
  ...
  Also the metadata defined by
[
RFC7591
]
, and registered extensions to
...
, this requires that the client send
...
.  The presence of the
client_id
parameter enables the authorization server to easily
...
MUST
enforce the binding between client and certificate, as
  described in either Section
2.1
or
2.2
below.  If no certificate is
  presented, or that which is presented doesn't match that which is expected
  for the given
client_id
, the authorization server returns a normal
...
, the IP address comparison of the value in
...
) or a reference to a trusted source
        for its X.509 certificates (using
jwks_uri
from
[
RFC7591
...
or
jwks
metadata parameters from
[
RFC7591
]
are used to convey the client's
          certificates via JSON Web Key (JWK) in a JWK Set
[
RFC7517
]
. The
...
requires that the key in the first certificate of
          the
x5c
parameter match the public key represented by those
...
or TLS 1.3 post-handshake authentication
[
RFC8446
]
, this document
...
,
  using an HTTP 401 status code and the
invalid_token
error code.
¶
...
To represent the hash of a certificate in a JWT, this specification
    defines the new JWT Confirmation Method
[
RFC7800
]
member
x5t#S256
for the X.509
    Certificate SHA-256 Thumbprint. The value of the
x5t#S256
member
    is a base64url-encoded
[
RFC4648
]
SHA-256
...
certificate thumbprint
    confirmation method. The new JWT content introduced by this specification is the
cnf
confirmation method claim at the bottom of the example that has
    the
x5t#S256
confirmation method member containing the value that is the hash
...
Figure 2
:
Example JWT Claims Set with an X.509 Certificate Thumbprint Confirmation Method
3.2.
...
x5t#S256
certificate thumbprint
          confirmation method. The new introspection response content introduced by this specification is the
cnf
...
Figure 3
:
Example Introspection Response for a Certificate-Bound Access Token
3.3.
...
parameter to signal the server's capability to issue certificate-bound access tokens:
¶
tls_client_certificate_bound_access_tokens
OPTIONAL
. Boolean value indicating server support for
...
Mutual-TLS OAuth client authentication and certificate-bound access tokens
        ...
        authentication credentials associated with the
client_id
).
...
, etc., conventionally defined for the
...
or actual distinct hosts) to differentiate its TLS behavior as appropriate.
¶
{
...
Figure 4
:
Example Authorization Server Metadata with Mutual-TLS Endpoint Aliases
6.
Implementation Considerations
6.1.
Authorization Server
The authorization server needs to set up its TLS configuration appropriately
    for the OAuth client authentication methods it supports.
¶
...
¶
In order to support the Self-Signed Certificate method alone, the authorization server
...
, the authorization server
    ...
    certificate that the client has proven possession of the corresponding private key.
¶
...
MAY
choose to terminate TLS connections at a load balancer,
...
requires that an authorization server (AS)
          ...
          issued to clients utilizing the
tls_client_auth
or
self_signed_tls_client_auth
...
If the PKI method of client authentication is used, an attacker could try to impersonate a client using
   ...
   different CA that the authorization server trusts.
          To cope with that threat, the authorization server
SHOULD
only accept, as trust anchors,
...
SHOULD
use an established and well-tested X.509 library
          (such as one used by an established TLS library) for validation of X.509 certificate chains
          and
SHOULD NOT
attempt to write their own X.509 certificate validation procedures.
¶
8.
...
defined the
cnf
(confirmation) claim that enables
...
cnf
structure.
          When included as a top-level member of an OAuth token introspection response,
cnf
...
.
          While this specification only explicitly uses the
x5t#S256
confirmation method member (see
Section 3.2
), it needs to define and register
          the higher-level
cnf
...
For reference, an
x5t#S256
value and the X.509 certificate
        from which it was calculated are provided in the following examples,
        Figures
5
and
6
, respectively. A JWK representation of the
        certificate's public key along with the
x5c
member is also
        provided in
Figure 7
