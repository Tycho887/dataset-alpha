---
{
  ...
  "date": "",
  "length": 205395,
  "crawled_at": "2026-04-23T20:57:06"
}
---
...
BCP 47
.  For a list of changes in this document,
   see
Section 8
.
...
A language tag is composed from a sequence of one or more "subtags",
   ...
   other subtags in a tag by a hyphen ("-", [
Unicode
] U+002D).
...
.
   ...
   Although [
RFC5234
] refers to octets, the language tags described in
...
ISO646
   ...
   document that uses the UTF-16LE [
RFC2781
] encoding of [
Unicode
].
...
The namespace of language tags and their subtags is administered by
   ...
   in
Section 5
of this document.  The Language Subtag Registry
   ...
   o  "Code" refers to values defined in external standards (and that
      ...
      [
ISO15924
] script code that was used to define the 'Hant' script
...
.
   ...
   o  The single-character subtag 'i' is used by some grandfathered tags
      (see
Section 2.2.8
) such as "i-klingon" and "i-bnn".  (Other
...
ISO639-5
   ...
   3.  The subtags in the range 'qaa' through 'qtz' are reserved for
       ...
       private use subtags following 'x-').  Please refer to
Section 4.6
...
Extended language subtags are used to identify certain specially
   ...
   1.  Extended language subtags consist solely of three-letter subtags.
       ...
       defined according to the assignments found in [
ISO639-3
].
...
3.  The script subtags 'Qaaa' through 'Qabx' are reserved for private
       ...
       registered script values.  Please refer to
Section 4.6
for more
...
4.  Three-character region subtags consist solely of digit (number)
       ...
       [
UN_M.49
] or assignments subsequently made by the governing
       ...
       C.  When ISO 3166-1 reassigns a code formerly used for one
           ...
           described in
Section 3.4
and MUST be used to form language
...
RFC 5646
Language Tags                September 2009
5.  The alphanumeric codes in
Appendix X
of the UN document MUST NOT
       be entered into the registry and MUST NOT be used to form
...
Variant subtags are used to indicate additional, well-recognized
   ...
   2.  Variant subtags, as a collection, are not associated with any
       ...
       defined in
Section 3.5
.  Note that any particular variant subtag
   ...
   4.  Variant subtags MUST be registered with IANA according to the
       rules in
Section 3.5
of this document before being used to form
...
Extensions provide a mechanism for extending language tags for use in
   ...
   that is not part of language identification.  See
Section 3.7
.  The
...
2.  Extension subtags are separated from the other subtags defined in
       ...
       registration authority via the mechanism described in
Section 3.7
...
Private use subtags are used to indicate distinctions in language
   ...
   6.  Private use subtags are NOT RECOMMENDED where alternatives exist
       or for general interchange.  See
Section 4.6
...
Many of these registered tags were made redundant by the advent of
   either
RFC 4646
or this document.  A redundant tag is a grandfathered
...
o  Either the tag is in the list of grandfathered tags or all of its
   ...
   A tag is considered valid for a given extension (
Section 3.7
) (as of
   ...
   reference [
RFC3066
].  A wider array of tags was considered well-
   formed under that document.  Any tags that were valid for use under
RFC 3066
are both well-formed and valid under this document's syntax;
...
Language Tag Syntax
   ...
   document is functionally equivalent to the one in [
RFC4646
], it has
...
.
   ...
   taken from
RFC 3066
have been maintained as part of the obsolete
RFC
3066
registry.  The subtags added to the registry by either [
RFC4645
]
   or [
RFC5645
] do not have separate registration forms (so no forms are
   archived for these additions).
3.1.1
.  File Format
The registry is a [
Unicode
] text file and consists of a series of
   records in a format based on "record-jar" (described in
   [
record-jar
]).  Each record, in turn, consists of a series of fields
...
A collection of fields forms a "record".  Records are separated by
   ...
   character sequence.  For more information, see [
UAX14
].
   ...
   the printable characters from the US-ASCII [
ISO646
] repertoire unless
   ...
   o  'Preferred-Value'
      ...
      *  Preferred-Value's field-body contains a canonical mapping from
         ...
         +  For fields of type 'extlang', 'grandfathered', or
            ...
            language range" [
RFC4647
] that is preferred for forming the
...
The field 'Subtag' contains the subtag defined in the record.  The
   ...
   or 'redundant' and contains a tag registered under [
RFC3066
].
   ...
   in
Section 2.1.1
.  All subtags use lowercase letters in the field-
...
Descriptions in the registry that correspond to ISO 639, ISO 15924,
   ...
   the bounds of the stability rules (
Section 3.4
), via subsequent
...
The field 'Deprecated' contains the date the record was deprecated
   ...
   maintenance process described in
Section 3.3
or via the registration
   process described in
Section 3.5
.  Usually, the addition of a
   ...
   creation of the registry.  The exact rules for this appear in
Section
2 of [RFC4645]
.  Note that these records have a 'Deprecated' field
...
Section 4.5
).  In cases where the subtag or tag also has a
...
RFC4646
] was adopted.  For example, the tag
   ...
   changes using the process in
Section 3.5
, although anyone MAY request
...
The field 'Prefix' contains a valid language tag that is RECOMMENDED
   ...
   fields using the "Extended Filtering" algorithm (see [
RFC4647
]), and
...
The field 'Suppress-Script' contains a script subtag (whose record
   ...
   consumers based on
RFC 3066
.  For example, virtually all Icelandic
...
.
   ...
   'no' (Norwegian).  For more information, see
Section 4.1
.
3.1.11
.  Scope Field
The field 'Scope' contains classification information about a primary
   ...
   o  'macrolanguage' - Indicates a macrolanguage as defined by ISO
      639-3 (see
Section 3.1.10
).  A macrolanguage is a cluster of
...
The Language Subtag Reviewer moderates the ietf-languages@iana.org
   ...
   other registry maintenance duties described in
Section 3.3
.  Only the
...
BCP 47
and its requirements; be willing to
   ...
   IETF decisions (see [
RFC2026
]).  The IESG can reverse or overturn the
...
Section 3.4
) or submit an appropriate registration form for an
...
5.   Values in the field 'Prefix' in records of type 'variant' MAY
   ...
   11.  The field 'Scope' MAY be added or removed from a primary or
        ...
        are individual languages as described in
Section 3.1.11
.
...
Section 3.3
and subject to stability provisions as described in
Section 3.4
.
   ...
   described in
Section 3.4
.  Changes to all other fields in the IANA
   ...
   requirements in
Section 3.1
before the record will be approved.
   ...
   Examples of completed registration forms can be found in
Appendix B
.
   A complete list of approved registration forms is online through
http://www.iana
.org; readers should note that the Language Tag
...
Section 3.1.5
.  The Reviewer MUST also ensure that an appropriate
   File-Date record is included in the request, to assist IANA when
   updating the registry (see
Section 5.1
).
...
RFC 5646
Language Tags                September 2009
a request due to discussion, as described in
Section 3.5
, unless the
...
.
   ...
   Registrations initiated due to the provisions of
Section 3.3
or
Section 3.4
...
thus help ensure that it is correct and consistent.  The Language
   ...
   IESG [
RFC2028
] under the same rules as other IETF decisions
   [
RFC2026
].  This includes a decision to extend the review period or
...
Section 3.1
is allowed.  Such changes are subject to the stability
      provisions in
Section 3.4
...
Extension subtags are those introduced by single-character subtags
   ...
   policy defined by [
RFC5226
].  This policy requires the development of
...
specify that case is not significant and that subtags MUST NOT
   ...
   described by the ABNF in
Section 3.1.1
.  Upon publication of an
   ...
   be appealed to the IESG [
RFC2028
] under the same rules as other IETF
   decisions (see [
RFC2026
...
After the adoption of this document, the IANA Language Subtag
   ...
   of subtags valid in a language tag.  [
RFC5645
   ...
   [
RFC4646
] when this document is adopted MUST be completed under the
...
The Language Subtag Registry is the source of data elements used to
   ...
   o  It is not designed to be the sole data source in the creation of a
      ...
      the registry are generally available, notably [
CLDR
].  Nor does
...
These provisions mean that no valid language tag can become invalid,
   ...
   'Suppress-Script' fields in the registry (defined in
Section 3.1
):
   ...
   2.  The script subtag SHOULD NOT be used to form language tags unless
       ...
       Script subtags were first formally defined in [
RFC4646
].  Their
...
Section 14.4 of
       [RFC2616]
) and the user requested the language range "en-US",
...
always preferred to less specific alternatives such as 'mul' and
   ...
   5.  [
ISO639-2
] has defined several codes included in the subtag
       ...
       *  The 'mul' (Multiple) primary language subtag identifies
          ...
          Language' header [
RFC3282
] allows a list of languages to be
...
*  The 'zxx' (Non-Linguistic, Not Applicable) primary language
       ...
       *  The 'mis' (Uncoded) primary language subtag identifies content
          ...
          incompatible with the stability goals of
BCP 47
.  It is always
...
including "sl-rozaj".  However, it follows both 'rozaj' and
   ...
   7.  The grandfathered tag "i-default" (Default Language) was
       originally registered according to [
RFC1766
] to meet the needs of
       [
RFC2277
].  It is not used to indicate a specific language, but
...
Some primary language records in the registry have a 'Macrolanguage'
   field (
Section 3.1.10
) that contains a mapping from each "encompassed
   ...
   ('ar'), are handled differently.  See
Section 4.1.2
.
...
To provide compatibility, Chinese languages encompassed by the 'zh'
   ...
   during the
RFC 3066
era.  Those grandfathered tags not already
   ...
   tags registered for them during the
RFC 3066
era.  For example, the
...
German dictionary to find out what the marked section meant.  If
   ...
   o  For systems and APIs, language tags form the basis for most
      ...
      CLDR (Common Locale Data Repository) (see UTS #35 [
UTS35
])
      project.
...
.
   ...
   have a fixed length limitation.  For example, [
RFC2231
] has no
   ...
   charset's name) coupled with the 76-character limit in [
RFC2047
].
...
extensions or private use subtags might wish to reserve or
   ...
   script        =  5 ; if not suppressed: see
Section 4.1
...
Section 3.1
   ...
   2.  Redundant or grandfathered tags are replaced by their 'Preferred-
       ...
       *  The field-body of the 'Preferred-Value' for grandfathered and
          redundant tags is an "extended language range" [
RFC4647
] and
...
If more than one variant appears within a tag, processors MAY reorder
   ...
   recommendations for variant ordering in
Section 4.1
.
...
This section deals with the processes and requirements necessary for
   ...
   this document and in accordance with the requirements of [
RFC5226
].
...
o  Announcing each updated version of the registry on the
   ...
   date" format of [
RFC3339
].  The date SHALL be the date on which that
...
Language tags used in content negotiation, like any other information
   ...
   countermeasures, is left to each application protocol (see
BCP 72
[
RFC3552
] for best current practice guidance on security threats and
   defenses).
   ...
   attacks.  See
Section 4.4
for details on language tag truncation,
...
To prevent denial-of-service attacks, applications SHOULD NOT depend
   ...
   specification of valid subtags for an extension (see
Section 3.7
)
...
was to incorporate two
   ...
   o  Changed the ABNF related to grandfathered tags.  The irregular
      ...
      both types of grandfathered tags to
Section 2.2.8
.

   o  Added the paragraph on "collections" to
Section 4.1
.

   o  Changed the capitalization rules for 'Tag' fields in
Section 3.1
.
...
up into subsections.
   ...
   o  Modified
Section 3.5
to allow 'Suppress-Script' fields to be
...
disallowing 'Prefix' field conflicts
   ...
   o  Modified text in
Section 3.5
to require the subtag reviewer to
   ...
   o  Modified text in
Section 4.1
to include the (heretofore anecdotal)
...
.
   ...
   o  Replaced the example of deprecating "zh-guoyu" with "zh-
      hakka"->"hak" in
Section 4.5
, noting that it was this document
      that caused the change.

   o  Replaced the section in
Section 4.1
dealing with "mul"/"und" to
      include the subtags 'zxx' and 'mis', as well as the tag
      "i-default".  A normative reference to
RFC 2277
was added.

   o  Added text to
Section 3.5
clarifying that any modifications of a
...
RFC5234
   ...
   o  Clarified and revised text in Sections
3.3
,
3.5
, and
...
o  Added text to
Section 5
requiring IANA to send an announcement to
   ...
   o  Modified the rules in
Section 2.2.4
so that "exceptionally
   ...
   o  Modified the IANA considerations section (
Section 5
) to remove
...
,
   ...
   [
SpecialCasing
]  The Unicode Consoritum, "Unicode Character Database,
                    Special Casing Properties", March 2008, <
http://
...
>.
   ...
   [
UN_M.49
]        Statistics Division, United Nations, "Standard
...
[
Unicode
]        Unicode Consortium, "The Unicode Consortium. The
...
, March 1995.
   ...
   [
RFC2028
]        Hovey, R. and S. Bradner, "The Organizations
                    Involved in the IETF Standards Process",
BCP 11
,
RFC 2028
, October 1996.
...
, September 2006.
   ...
   [
RFC5645
]        Ewell, D., Ed., "Update to the Language Subtag
                    Registry", September 2009.

   [
UTS35
]          Davis, M., "Unicode Technical Standard #35: Locale
...
The following people contributed to this document:
   ...
   Reviewer for almost the entire
RFC 1766
/RFC 3066 period, as well as
   the Language Subtag Reviewer since the adoption of
RFC 4646
.
   ...
   registrations, and his careful editorship of both
RFC 4645
and
   [
RFC5645
