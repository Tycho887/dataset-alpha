...
RFC 7644
SCIM Protocol Specification        September 2015
Copyright Notice
...
The SCIM protocol is an application-level HTTP-based protocol for
   ...
   defined in the SCIM Core Schema document [
RFC7643
].
1.1
.  Intended Audience
...
RFC7235
   ...
   TLS Client Authentication
      ...
      (also known as mutual authentication).  See
Section 7.3 of
      [RFC5246]
.
   ...
   HOBA Authentication
      ...
      for an HTTP authentication method (see [
RFC7486
]).  The design can
   ...
   Bearer Tokens
      Bearer tokens [
RFC6750
] MAY be used when combined with TLS and a
      token framework such as OAuth 2.0 [
RFC6749
].  Tokens that are
      ...
      such as anonymous registration (see
Section 3.3
).  For security
      considerations regarding the use of bearer tokens in SCIM, see
Section 7.4
.  While bearer tokens most often represent an
   ...
   PoP Tokens
      ...
      presenter being a holder of the key.  See [
OAuth-PoP-Arch
] for an
...
Cookies
      ...
      provider (see [
RFC6265
]).  An example of this is scenarios where
      ...
      purposes of SCIM, the security considerations in
Section 7.4
apply.
...
Section 2.2
   ...
   authorizations, as documented in
...
In some SCIM deployments, it MAY be acceptable to permit
   ...
   accept a SCIM Create request (see
Section 3.3
) from an anonymous
   client.  See
Section 7.6
...
3.1
.  Background
SCIM is a protocol that is based on HTTP [
RFC7230
].  Along with HTTP
   headers and URIs, SCIM uses JSON [
RFC7159
] payloads to convey SCIM
   ...
   "application/scim+json" (see
Section 8.1
).

   A SCIM "resource" is a JSON object [
RFC7159
] that may be created,
...
Section 8.7 of [RFC7643]
   ...
   extension model are defined in [
RFC7643
], SCIM clients should expect
...
RFC7159
   ...
   (e.g., [
XML-Schema
]) are not recommended.  A SCIM service provider
...
The SCIM protocol specifies well-known endpoints and HTTP methods for
   ...
   ([
RFC7643
]); i.e., "User" and "Group" resources correspond to
...
Section 4.3 of [RFC7231]
on a URL derived from the Base URL.
...
To create new resources, clients send HTTP POST requests to the
   ...
   associated resource type endpoint discovery (see
Section 4
).
...
RFC7643
   ...
   o  Attributes whose mutability is "readWrite" (see
Section 2.2 of
      [RFC7643]
) and that are omitted from the request body MAY be
...
Resources MAY be retrieved via opaque, unique URLs or via queries
   (see
Section 3.4.2
).  The attributes returned are defined in the
   server's attribute schema (see
Section 8.7 of [RFC7643]
) and may be
   modified by request parameters (see
Section 3.9
).  By default,
...
A query against a server root indicates that all resources within the
   ...
   endpoint), filters MUST be processed as outlined in
Section 3.4.2.2
.
...
Filtering is an OPTIONAL parameter for SCIM service providers.
   ...
   (see
Section 4
).  Clients MAY request a subset of resources by
...
RFC 7644
SCIM Protocol Specification        September 2015
SCIM filters MUST conform to the following ABNF [
RFC5234
] rules as
   specified below:
...
RFC 7159
   ...
   [
RFC7159
], "DIGIT" and "ALPHA" are defined per
Appendix B.1 of
   [RFC5234]
, and "URI" is defined per
...
Filters MUST be evaluated using the following order of operations, in
   ...
   specified using standard attribute notation (
Section 3.10
).  For
   ...
   response as per
Section 3.12
.  For example, if a client specified an
...
Section 4
).  Sorting allows clients to specify the order in which
...
The following attributes control which attributes SHALL be returned
   ...
   attributes  A multi-valued list of strings indicating the names of
      ...
      MUST be in standard attribute notation (
Section 3.10
) form.  See
Section 3.9
...
for additional retrieval query parameters.  OPTIONAL.
   ...
   filter  The filter string used to request a subset of resources.  The
      filter string MUST be a valid filter (
Section 3.4.2.2
) expression.
      OPTIONAL.
   ...
   sortBy  A string indicating the attribute whose value SHALL be used
      ...
      in standard attribute notation (
Section 3.10
) form.  See
Section 3.4.2.3
.  OPTIONAL.
...
Resources can be modified in whole or in part using HTTP PUT or HTTP
   ...
   in
Section 4.3 of [RFC7231]
.  Resources such as Groups may be very
   large; hence, implementers SHOULD support HTTP PATCH [
RFC5789
] to
...
The general form of the SCIM PATCH request is based on JSON Patch
   [
RFC6902
].  One difference between SCIM PATCH and JSON Patch is that
   SCIM servers do not support array indexing and do not support
   [
RFC6902
] operation types relating to array element manipulation,
   such as "move".
...
Section 3.4.2.2
.  The "valuePath" rule allows specific values of a
   ...
   attribute's mutability and schema as defined in Sections
2.2
and
2.3
of [
RFC7643
].  For example, a client MUST NOT modify an attribute
...
.
   ...
   The attribute notation rules described in
Section 3.10
apply for
...
replacing) an attribute with its fully qualified name, including
   ...
   code and a JSON detail error response as defined in
Section 3.12
.
   ...
   subject to the "attributes" query parameter (see
Section 3.9
), or MAY
...
The SCIM bulk operation is an optional server feature that enables
   ...
   querying the service provider configuration (see
Section 4
).  The
...
RFC7643
   ...
   to the common attributes defined in [
RFC7643
].
   ...
   Operations
      ...
      version  The current resource version.  Version MAY be used if the
         service provider supports entity-tags (ETags) (
Section 2.3 of
         [RFC7232]
) and "method" is "PUT", "PATCH", or "DELETE".
...
path  The resource's relative path to the SCIM service provider's
      ...
      status  The HTTP response status code for the requested operation.
         ...
         the detail error response as per
Section 3.12
.
   ...
   operation (see
Section 3.7.2
).  The service provider MUST return the
...
Servers MUST accept requests and be able to return JSON-structured
   responses using UTF-8 encoding [
RFC3629
].  UTF-8 SHALL be the default
...
and MAY specify the desired response
   data format via an HTTP "Accept" header (
...
For any SCIM operation where a resource representation is returned
   ...
   characteristic set to "always" (see
Section 2.2 of [RFC7643]
).  The
...
to indicate operation success or failure.  In
...
| invalidPath   | The "path" attribute was       | PATCH (Section   |
   ...
   |               | or attribute type (see Section |
Section 3.3
,     |
   |               | 2.2 of [
RFC7643
]), or resource | Query - Section  |
   |               | schema (see
Section 4
of       | 3.4.3), PUT      |
   |               | [
...
The SCIM protocol supports resource versioning via standard HTTP
   ETags (
Section 2.3 of [RFC7232]
).  Service providers MAY support weak
...
.  An example representation of SCIM service
      provider configuration may be found in
Section 8.5 of [RFC7643]
.
...
/Schemas
      ...
      The contents of each schema returned are described in
Section 7 of
      [RFC7643]
...
.
   ...
   /ResourceTypes
      ...
      in
Section 6 of [RFC7643]
, and an example representation can be
      found in
Section 8.6 of [RFC7643]
.
   ...
   single User or Group is retrieved, as per
Section 3.4.1
.  When
   ...
   Figure 9 below.  Query parameters described in
Section 3.4.2
, such as
...
To increase the likelihood that the input and comparison of usernames
   ...
   (PRECIS) preparation and comparison rules described in Sections
3
and
   4, respectively, of [
RFC7613
], which is based on the PRECIS framework
   specification [
RFC7564
].  See
Section 3.4 of [RFC7613]
for discussion
...
The service provider MAY use one of the authentication mechanisms
   discussed in
Section 2
to determine the identity of the client and
...
When using authorization tokens such as those issued by OAuth 2.0
   [
RFC6749
], implementers MUST take into account threats and
   countermeasures as documented in
Section 8 of [RFC7521]
.
...
Since the possession of a bearer token or cookie MAY authorize the
   ...
   random guessing attack; for example, see
Section 5.2 of [RFC6750]
and
...
.
   ...
   authenticating to an authorization server.  See
Section 6 of
   [RFC6749]
.
   ...
   limits session cookie lifetime as per
Section 5.2.1 of [RFC6265]
.
...
RFC7521
   ...
   [
HTTP-BASIC-AUTH
]; therefore, implementers are encouraged to use
...
7.5.1
.  Personal Information
The SCIM Core Schema specification [
RFC7643
...
.  Disclosure of Sensitive Information in URIs
As mentioned in
Section 9.4 of [RFC7231]
, SCIM clients requesting
...
When comparing Unicode strings such as those in query filters or
   ...
   appropriately prepared before comparison.  See
Section 5
.
8
.  IANA Considerations
...
As per the "SCIM Schema URIs for Data Resources" registry established
   by [
RFC7643
], the following defines and registers the SCIM protocol
...
RFC 5789
, DOI 10.17487/RFC5789, March 2010,
              <
...
>.
   ...
   [
RFC7643
]  Hunt, P., Ed., Grizzle, K., Wahlstroem, E., and
...
>.
   ...
   [
RFC7486
]  Farrell, S., Hoffman, P., and M. Thomas, "HTTP Origin-
              Bound Authentication (HOBA)",
RFC 7486
,
