---
{
  ...
  "date": "",
  "length": 39287,
  "crawled_at": "2026-04-23T20:51:50"
}
---
...
RFC 8471
The Token Binding Protocol Version 1.0     October 2018
Copyright Notice
...
Servers often generate various security tokens (e.g., HTTP cookies,
   OAuth tokens [
RFC6749
]) for applications to present when accessing
...
TOKENBIND-TLS13
   ...
   [
RFC5705
] from the TLS connection with the private key.  The
...
RFC8472
   ...
   Token Binding negotiation TLS extension; see [
RFC8472
] ("Transport
   ...
   negotiated (e.g., via [
RFC8472
]) and then validates the signatures
   ...
   [
RFC8472
] describes the negotiation of the Token Binding protocol and
   key parameters.  [
RFC8473
] ("Token Binding over HTTP") explains how
...
The Token Binding message is sent by the client to prove possession
   ...
   Token Binding protocol (e.g., via [
RFC8472
   ...
   two communicating parties.  For example, [
RFC8473
] specifies an
...
ANSI.X9-62.2005
   ...
   protocols, such as Token Binding over HTTP [
RFC8473
], MAY prohibit
...
.  Note that application-layer messages
   ...
   defined in [
RFC5705
], by supplying the following input values:
...
A Token Binding message may optionally contain a series of
   ...
   Initially, no extension types are defined (see
Section 6.3
...
The client MUST include at least one TokenBinding structure in the
   ...
   negotiated with the server (e.g., via [
RFC8472
] or a different
   mechanism).
...
Section 7
("Security Considerations").  Therefore, the server
   ...
   secret TLS extension [
RFC7627
] and the renegotiation indication TLS
   extension [
RFC5746
].
   ...
   match those negotiated with this client (e.g., via [
RFC8472
] or a
...
This registry operates under the "Specification Required" policy as
   defined in [
RFC8126
].  The designated expert will require the
...
The Token Binding protocol MUST be negotiated using a mechanism that
   prevents downgrade attacks.  For example, [
RFC8472
] specifies a TLS
...
The Token Binding protocol relies on the TLS exporters [
RFC5705
] to
...
RFC5746
              ...
              Extension",
RFC 5746
, DOI 10.17487/RFC5746, February 2010,
              <
