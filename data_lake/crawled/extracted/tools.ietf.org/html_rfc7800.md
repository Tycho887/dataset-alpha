---
{
  ...
  "url": "https://tools.ietf.org/html/rfc7800",
  ...
  "date": "",
  "length": 33137,
  "crawled_at": "2026-04-23T20:48:20"
}
---
...
RFC 7800
Proof-of-Possession Key for JWTs          April 2016
Table of Contents
1
.  Introduction  . . . . . . . . . . . . . . . . . . . . . . . .
2
...
1
.  Introduction
This specification describes how a JSON Web Token [
JWT
] can declare
   that the presenter of the JWT possesses a particular proof-of-
...
By including a "cnf" (confirmation) claim in a JWT, the issuer of the
   ...
   "sub" (subject) claim [
JWT
], the presenter is normally the subject
...
identified by the JWT.  (In some applications, the subject identifier
   ...
   [
JWT
].)  If the JWT contains no "sub" claim, the presenter is
...
OASIS.saml-core-2.0-os
   ...
   registry for these members in
Section 6.2
and registers the members
...
JWK
   ...
   explained in [
JWT
].  If the JWT is not encrypted, the symmetric key
...
When the key held by the presenter is a symmetric key, the "jwe"
   member is an encrypted JSON Web Key [
JWK
] encrypted to a key known to
   the recipient using the JWE Compact Serialization containing the
   symmetric key.  The rules for encrypting a JWK are found in
Section 7
of the JSON Web Key [
JWK
] specification.
...
The following example illustrates a symmetric key that could
   ...
   The UTF-8 [
RFC3629
] encoding of this JWK is used as the JWE Plaintext
...
The proof-of-possession key can be passed by reference instead of
   ...
   value is a URI [
RFC3986
] that refers to a resource for a set of JSON-
   encoded public keys represented as a JWK Set [
JWK
], one of which is
   ...
   [
RFC5246
] and the identity of the server MUST be validated, as per
...
4
.  Security Considerations
All of the security considerations that are discussed in [
JWT
] also
   ...
   audience restriction, as described in Section 4.1.3 of [
JWT
], as it
