---
{
  ...
  "url": "https://tools.ietf.org/html/draft-ietf-oauth-pop-architecture-08",
  ...
  "date": "",
  "length": 51105,
  "crawled_at": "2026-04-23T20:51:33"
}
---
...
3
3.1
.  Preventing Access Token Re-Use by the Resource Server . .
4
3.2
.  TLS and DTLS Channel Binding Support  . . . . . . . . . .
4
3.3
.  Access to a Non-TLS Protected Resource  . . . . . . . . .
4
3.4
.  Offering Application Layer End-to-End Security  . . . . .
5
4
...
Authors' Addresses  . . . . . . . . . . . . . . . . . . . . . . .
22
Hunt, et al.             Expires January 9, 2017                [Page 2]
Internet-Draft         OAuth 2.0 PoP Architecture              July 2016
1
.  Introduction
The OAuth 2.0 protocol family ([
RFC6749
], [
RFC6750
], and [
...
Section 4
,
   proposes different ways to mitigate those threats in
Section 6
,
   outlines an architecture for a solution that builds on top of the
   existing OAuth 2.0 framework in
Section 7
...
In this use case we consider the scenario where an OAuth 2.0 request
   to a protected resource is secured using TLS or DTLS (see [
RFC4347
]),
...
This use case is for a web client that needs to access a resource
   ...
   described in [
RFC6750
], there may be the desire to avoid using TLS
...
RFC6750
] are not followed) using a number of ways,
...
An attacker may generate a bogus token or modify the token content
      ...
      client or MAY be assumed to be constrained (see [
RFC7252
]), may
...
[
RFC4962
] gives useful guidelines for designers of
   authentication and key management protocols.  While
RFC 4962
was
...
Internet-Draft         OAuth 2.0 PoP Architecture              July 2016
applies OAuth 2.0 terminology to the requirements outlined in
RFC
4962
.
...
RFC5056
   ...
   identity in TLS is discussed at length in [
RFC6125
].

   A number of the threats listed in
Section 4
demand protection of the
   access token content and a standardized solution, for example, in the
   form of a JSON-based format, is available with the JWT [
RFC7519
].
6
.  Threat Mitigation
...
Internet-Draft         OAuth 2.0 PoP Architecture              July 2016
best practices can be found in [
RFC6819
] and in the security
...
RFC6750
   ...
   [
RFC7525
].  Encrypting the token content itself is another
...
and
Section 6.3
as well.  Furthermore, the client has to
...
know that it was provided by the client?  It has to authenticate the
   ...
   [
RFC5246
], or pre-shared secrets within TLS [
RFC4279
].  The choice of
...
Section 6.2
, is to replace authentication with the proof-of-
   ...
   approach can be found with the OAuth 1.0 MAC token [
RFC5849
], and
   Kerberos [
RFC4120
] when utilizing the AP_REQ/AP_REP exchange (see
   also [
I-D.hardjono-oauth-kerberos
] for a comparison between Kerberos
   and OAuth).
...
Section 6.1
, is that the client has to be careful to whom it
...
Sender Constraint:
      ...
      The weak point with this approach, which is briefly described in
Section 6.2
, is to setup the authentication infrastructure such
   ...
   Key Confirmation:
      ...
      The weak point with this approach, see
Section 6.3
, is the
...
The proof-of-possession security concept assumes that the
   ...
   For example, with the JSON Web Token (JWT) [
RFC7519
] a standardized
...
from a client but has to consult the authorization server.  This can,
   ...
   [
RFC7662
]).  Figure 4 shows the protocol interaction graphically.
...
Bradley, Tony Nadalin, Klaas Wierenga, Thomas Hardjono, Brian
   ...
   In the appendix of this document we reuse content from [
RFC4962
] and
...
I-D.ietf-oauth-signed-http-request
              ...
              Signing HTTP Requests for OAuth",
draft-ietf-oauth-signed-
http-request-02
...
RFC4347
              ...
              Security",
RFC 4347
, DOI 10.17487/RFC4347, April 2006,
              <
...
,
RFC 4962
, DOI 10.17487/RFC4962, July 2007,
              <
...
Hunt, et al.             Expires January 9, 2017               [Page 22]
Internet-Draft         OAuth 2.0 PoP Architecture              July 2016
Hannes Tschofenig
   ARM Limited
   Hall in Tirol  6060
