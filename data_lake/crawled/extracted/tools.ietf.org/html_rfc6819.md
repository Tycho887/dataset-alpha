---
{
  ...
  "date": "",
  "length": 154251,
  "crawled_at": "2026-04-23T20:57:15"
}
---
...
RFC 6819
OAuth 2.0 Security               January 2013
Table of Contents
1
. Introduction ....................................................
6
...
4.1.4. Threat: End-User Credentials Phished Using
                  Compromised or Embedded Browser ....................
19
4.1.5
...
4.4.1.2. Threat: Obtaining Authorization "codes"
                           from Authorization Server Database ........
26
...
4.4.1.7. Threat: Authorization "code" Leakage
                           through Counterfeit Client ................
30
4.4.1.8
. Threat: CSRF Attack against redirect-uri ..32
...
35
4.4.1.13
. Threat: Code Substitution (OAuth Login) ..36
4.4.2
. Implicit Grant .....................................
37
...
48
4.6.6
. Threat: Leak of Confidential Data in HTTP Proxies ..48
...
54
5.1.5.4
. Limit Number of Usages or One-Time Usage ..55
...
5.2.3.2. Require User Consent for Public
                           Clients without Secret ....................
60
5.2.3.3. Issue a "client_id" Only in
                           Combination with "redirect_uri" ...........
61
...
5.2.4.2
. Informed Decisions Based on Transparency ..63
...
5.3.1. Don't Store Credentials in Code or
                  Resources Bundled with Software Packages ...........
65
...
This security considerations document only considers clients bound to
   a particular deployment as supported by [
RFC6749
].  Such deployments
...
[RFC6749], Section 4.3
), the mechanism used by authorization
...
Section 3.1
) or authorization server shared secret/public key
      (assertion-based design; see
Section 3.1
)
...
The "state" parameter is used to link requests and callbacks to
   prevent cross-site request forgery attacks (see
Section 4.4.1.8
)
...
Deployment-independent "client_id" with pre-registered "redirect_uri"
      ...
      secret cannot be protected appropriately (see
Section 4.1.1
).  Due
...
This section gives a comprehensive threat model of OAuth 2.0.
   ...
   refers to a detailed description in
Section 5
.
...
Attack: Obtain a Deployment-Specific Secret:
   ...
   o  Web server: Apply standard web server protection measures (for
      config files and databases) (see
Section 5.3.2
).
...
4.1.4
.  Threat: End-User Credentials Phished Using Compromised or
Embedded Browser
...
4.3.2
.  Threat: Obtaining Access Tokens from Authorization Server
Database
...
Section 5.1.4.1.1
   ...
   o  Store access token hashes only (
Section 5.1.4.1.3
).
...
Countermeasures:
   ...
   o  The transmission of client credentials must be protected using
      transport-layer mechanisms such as TLS (see
Section 5.1.1
).
...
4.3.4
.  Threat: Obtaining Client Secret from Authorization Server
Database
...
Countermeasures:
   ...
   o  As per the core OAuth spec, the authorization server as well as
      ...
      using transport-layer mechanisms such as TLS (see
Section 5.1.1
).
...
4.4.1.2
.  Threat: Obtaining Authorization "codes" from Authorization
Server Database
...
4.4.1.3
.  Threat: Online Guessing of Authorization "codes"
...
A hostile party could impersonate the client site and get access to
   ...
   o  The redirect URI of the client should point to an HTTPS-protected
      ...
      redirect URI using server authentication (see
Section 5.1.2
).
...
4.4.1.7
.  Threat: Authorization "code" Leakage through Counterfeit
Client
...
Section 5.2.3.5
   ...
   o  For native applications, one could also consider using deployment-
      specific client ids and secrets (see
Section 5.2.3.4
), along with
      the binding of authorization "codes" to "client_ids" (see
Section 5.2.4.4
) to detect such an attack because the attacker
...
Section 4.4.2
) or resource owner password credentials (see
Section 4.4.3
).
4.4.1.8
...
Section 5.2.2.6
   ...
   o  For older browsers, JavaScript frame-busting (see [
Framebusting
])
...
An attacker who owns a botnet can locate the redirect URIs of clients
   ...
   parameter (see
Section 4.4.1.8
) is deployed on the client side.  With
...
An attacker could obtain the token from the browser's history.  Note
   ...
   o  Use short expiry time for tokens (see
Section 5.1.5.3
).  Reduced
...
CSRF attacks (see
Section 4.4.1.8
) also work against the redirect URI
...
Section 5.2.2.2
   ...
   o  As per the core OAuth specification, the authorization server must
      ...
      layer mechanisms such as TLS (see
Section 5.1.1
).
...
4.4.3.3
.  Threat: Client Obtains Refresh Token through Automatic
Authorization
All interaction with the resource owner is performed by the client.
   Thus it might, intentionally or unintentionally, happen that the
...
Section 5.1.1
   ...
   o  Use alternative authentication means that do not require the
      ...
      Message Authentication Code).
4.4.3.5
.  Threat: Obtaining User Passwords from Authorization Server
Database
...
Countermeasures:
   ...
   o  Utilize secure password policy (
Section 5.1.4.2.1
).
...
[RFC6749], Section 3
   ...
   this grant type are similar to those described in
Section 4.4.3
.
4.5
.  Refreshing an Access Token
...
Section 5.1.5.3
      ...
      leaks.
4.5.2
.  Threat: Obtaining Refresh Token from Authorization Server
Database
...
An attacker may try to guess valid refresh token values and send it
   ...
   o  For handle-based designs (
Section 5.1.4.2.2
).

   o  For assertion-based designs (
Section 5.1.5.9
).

   o  Bind token to client id, because the attacker would guess the
      matching client id, too (see
Section 5.1.5.8
).

   o  Authenticate the client; this adds another element that the
      attacker has to guess (see
Section 5.2.3.4
).
4.5.4
.  Threat: Refresh Token Phishing by Counterfeit Authorization
Server
An attacker could try to obtain valid refresh tokens by proxying
   ...
   o  Utilize server authentication (as described in
Section 5.1.2
).
...
An attacker could attempt to replay valid requests in order to obtain
   ...
   o  The resource server should utilize transport security measures
      (e.g., TLS) in order to prevent such attacks (see
Section 5.1.1
).
   ...
   o  Alternatively, the resource server could employ signed requests
      (see
Section 5.4.3
) along with nonces and timestamps in order to
...
Section 5.4.2
   ...
   o  Restrict the token scope (see
Section 5.1.5.1
) and/or limit the
      token to a certain resource server (
Section 5.1.5.5
).
...
An OAuth HTTP authentication scheme as discussed in [
RFC6749
] is
   optional.  However, [
RFC2616
] relies on the Authorization and
...
Section 5.1.5.3
      ...
      enforcing one-time token usage (see
Section 5.1.5.4
).
5
.  Security Considerations
...
This is applicable to all requests sent from the client to the
   ...
   TLS [
RFC5246
].  A virtual private network (VPN), e.g., based on IPsec
   VPNs [
RFC4301
...
Administrators should undertake industry best practices to protect
   the storage of credentials (for example, see [
OWASP
]).  Such
...
When creating secrets not intended for usage by human users (e.g.,
   ...
   number sequence (see [
RFC4086
] for best current practice) generated
...
Authorization servers in multi-service environments may consider
   ...
   [
OASIS.saml-core-2.0-os
]) use the Audience element for this purpose.
...
The authorization server may require the binding of authentication
   ...
   Equipment Identity [
IMEI
] is one example of such an identifier; there
...
For newer browsers, avoidance of iFrames can be enforced on the
   ...
   [
X-Frame-Options
]).  This header can have two values, "DENY" and
...
5.2.3.1
.  Don't Issue Secrets to Clients with Inappropriate Security
Policy
...
An authorization server should require all clients to register their
   ...
   defined in [
RFC6749
].  The way that this registration is performed is
   ...
   o  Authorization "code" leakage through counterfeit web site: allows
      ...
      redirect to an end-user authorization endpoint (
Section 4.4.1.7
).
...
5.2.3.7
.  Use Strong Client Authentication (e.g., client_assertion/
client_token)
By using an alternative form of authentication such as client
   assertion [
OAuth-ASSERTIONS
], the need to distribute a
...
5.2.4.1
.  Automatic Processing of Repeated Authorizations Requires
Client Validation
Authorization servers should NOT automatically process repeat
   ...
   authentication assertion certificate (
Section 5.2.3.7
) or validation
   of a pre-registered redirect URI (
Section 5.2.3.5
).
5.2.4.2
...
5.3.1
.  Don't Store Credentials in Code or Resources Bundled with
Software Packages
Because of the number of copies of client software, there is limited
   ...
   users the trustworthiness of the client.
5.3.2
.  Use Standard Web Server Protection Measures (for Config Files
and Databases)
...
OAuth, as a security protocol, is distinctive in that its flow
   ...
   o  End users must understand what they are being asked to approve
      (see
Section 5.2.4.2
).  Users often do not have the expertise to
...
Framebusting
              ...
              Security and Privacy Workshop, May 2010, <
http://elie.im/
...
>.
   ...
   [
IMEI
]     3GPP, "International Mobile station Equipment Identities
...
OAuth-REVOCATION
   ...
   [
OPENID
]   "OpenID Foundation Home Page", <
http://openid.net/
>.

   [
OWASP
]    "Open Web Application Security Project Home Page",
              <
https://www.owasp.org/
>.
