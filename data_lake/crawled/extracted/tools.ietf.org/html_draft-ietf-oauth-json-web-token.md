...
21
Jones, et al.             Expires June 12, 2015                 [Page 2]
Internet-Draft            JSON Web Token (JWT)             December 2014
11.1
. Trust Decisions  . . . . . . . . . . . . . . . . . . . . .
21
...
Authors' Addresses . . . . . . . . . . . . . . . . . . . . . . . .
34
Jones, et al.             Expires June 12, 2015                 [Page 3]
Internet-Draft            JSON Web Token (JWT)             December 2014
1
.  Introduction
JSON Web Token (JWT) is a compact claims representation format
   ...
   to be transmitted as a JavaScript Object Notation (JSON) [
RFC7159
]
...
JWS
   ...
   [
JWE
] structure, enabling the claims to be digitally signed or MACed
...
2
.  Terminology
These terms defined by the JSON Web Signature (JWS) [
JWS
]
   ...
   These terms defined by the JSON Web Encryption (JWE) [
JWE
]
   specification are incorporated into this specification: "JSON Web
   ...
   [
RFC4949
] are incorporated into this specification: "Ciphertext",
   "Digital Signature" "Message Authentication Code (MAC)", and
   "Plaintext".

   These terms are defined by this specification:
Jones, et al.             Expires June 12, 2015                 [Page 4]
Internet-Draft            JSON Web Token (JWT)             December 2014
JSON Web Token (JWT)
...
Jones, et al.             Expires June 12, 2015                 [Page 5]
Internet-Draft            JSON Web Token (JWT)             December 2014
canonicalizations applied.
   ...
   NumericDate
      ...
      Edition [
POSIX.1
] definition "Seconds Since the Epoch", in which
      each day is accounted for by exactly 86400 seconds, other than
      that non-integer values can be represented.  See
RFC 3339
[
RFC3339
...
[
RFC7159
], the JSON object consists of zero
   ...
   structural characters, in accordance with
...
Jones, et al.             Expires June 12, 2015                 [Page 6]
Internet-Draft            JSON Web Token (JWT)             December 2014
{"typ":"JWT",
      "alg":"HS256"}
...
Jones, et al.             Expires June 12, 2015                 [Page 7]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
The JWT Claims Set represents a JSON object whose members are the
   ...
   duplicate member name, as specified in
Section 15.12
(The JSON
   Object) of ECMAScript 5.1 [
ECMAScript
].
...
The following Claim Names are registered in the IANA JSON Web Token
   Claims registry defined in
Section 10.1
.  None of the claims defined
...
Jones, et al.             Expires June 12, 2015                 [Page 8]
Internet-Draft            JSON Web Token (JWT)             December 2014
4.1.1
.  "iss" (Issuer) Claim
...
Jones, et al.             Expires June 12, 2015                 [Page 9]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
Section 10.1
or be a Public Name: a value that contains a Collision-
...
For a JWT object, the members of the JSON object represented by the
   ...
   Header values apply.
Jones, et al.             Expires June 12, 2015                [Page 10]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
JWE
   ...
   by JWT applications to declare the MIME Media Type [
IANA.MediaTypes
]
...
JWE
   ...
   for compatibility with legacy implementations.  See
Appendix A.2
for
...
Jones, et al.             Expires June 12, 2015                [Page 11]
Internet-Draft            JSON Web Token (JWT)             December 2014
application receiving them SHOULD verify that their values are
   ...
   JWT.
Section 10.4.1
of this specification registers the "iss" (issuer),
...
To support use cases in which the JWT content is secured by a means
   ...
   Algorithms (JWA) [
JWA
]; it is an Unsecured JWS with the JWT Claims
...
Jones, et al.             Expires June 12, 2015                [Page 12]
Internet-Draft            JSON Web Token (JWT)             December 2014
eyJpc3MiOiJqb2UiLA0KICJleHAiOjEzMDA4MTkzODAsDQogImh0dHA6Ly9leGFt
     cGxlLmNvbS9pc19yb290Ijp0cnVlfQ
...
To create a JWT, the following steps are performed.  The order of the
   ...
   3.  Create a JOSE Header containing the desired set of Header
       Parameters.  The JWT MUST conform to either the [
JWS
] or [
JWE
]
...
JWS
] for creating a JWS MUST
          be followed.

       *  Else, if the JWT is a JWE, create a JWE using the Message as
          the JWE Plaintext; all steps specified in [
JWE
] for creating a
          JWE MUST be followed.
Jones, et al.             Expires June 12, 2015                [Page 13]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
RFC7159
   ...
   6.   Determine whether the JWT is a JWS or a JWE using any of the
        methods described in Section 9 of [
JWE
].
...
JWS
        ...
        *  Else, if the JWT is a JWE, follow the steps specified in
           [
JWE
] for validating a JWE.  Let the Message be the JWE
           Plaintext.
Jones, et al.             Expires June 12, 2015                [Page 14]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
RFC7159
   ...
   will not matter.)
Jones, et al.             Expires June 12, 2015                [Page 15]
Internet-Draft            JSON Web Token (JWT)             December 2014
8
.  Implementation Requirements
This section defines which algorithms and features of this
   ...
   (JWA) [
JWA
], only HMAC SHA-256 ("HS256") and "none" MUST be
...
Jones, et al.             Expires June 12, 2015                [Page 16]
Internet-Draft            JSON Web Token (JWT)             December 2014
and a reference to the specification that defines it.  This
   specification registers the Claim Names defined in
Section 4.1
.
...
Jones, et al.             Expires June 12, 2015                [Page 17]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
Jones, et al.             Expires June 12, 2015                [Page 18]
Internet-Draft            JSON Web Token (JWT)             December 2014
o  Change Controller: IESG
   o  Specification Document(s):
Section 4.1.3
...
Jones, et al.             Expires June 12, 2015                [Page 19]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
Section 4.1
in the IANA JSON Web Signature and Encryption Header
   Parameters registry defined in [
JWS
] for use by Claims replicated as
   Header Parameters in JWEs, per
Section 5.3
.
10.4.1
.  Registry Contents
...
Section 4.1.2
of [[ this document ]]
Jones, et al.             Expires June 12, 2015                [Page 20]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
While syntactically the signing and encryption operations for Nested
   ...
   specification.
Jones, et al.             Expires June 12, 2015                [Page 21]
Internet-Draft            JSON Web Token (JWT)             December 2014
12
.  Privacy Considerations
...
Jones, et al.             Expires June 12, 2015                [Page 22]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
RFC 6755
, October 2012.
Jones, et al.             Expires June 12, 2015                [Page 23]
Internet-Draft            JSON Web Token (JWT)             December 2014
[
RFC6838
...
, January 2013.
   ...
   [
SWT
]      Hardt, D. and Y. Goland, "Simple Web Token (SWT)",
...
Section 6.1
and Appendices A.1, A.2, and A.3 of [
JWS
].
A.1
.  Example Encrypted JWT
...
to the
   ...
   Claims Set from
Section 3.1
as the plaintext value, the computation
...
JWE
   ...
   purposes only) is:
Jones, et al.             Expires June 12, 2015                [Page 24]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
of
   [
JWS
].  Therefore, its computation is not repeated here.  This
...
of [
JWE
], other than that different JOSE Header,
   Plaintext, JWE Initialization Vector, and Content Encryption Key
   values are used.  (The RSA key used is the same.)

   The Payload used is the octets of the ASCII [
RFC20
] representation of
Jones, et al.             Expires June 12, 2015                [Page 25]
Internet-Draft            JSON Web Token (JWT)             December 2014
the JWT at the end of
Appendix A.2.1
of [
JWS
] (with all whitespace
...
SAML 2.0 [
OASIS.saml-core-2.0-os
] provides a standard for creating
...
Jones, et al.             Expires June 12, 2015                [Page 26]
Internet-Draft            JSON Web Token (JWT)             December 2014
complexity.
   ...
   using the JSON [
RFC7159
] object encoding syntax.  It also supports
...
The authors acknowledge that the design of JWTs was intentionally
   influenced by the design and simplicity of Simple Web Tokens [
SWT
]
...
JSS
   ...
   Applications [
CanvasApp
], all of which influenced this draft.

   This specification is the work of the OAuth Working Group, which
   includes dozens of active and dedicated participants.  In particular,
   the following individuals contributed ideas, feedback, and wording
   that influenced this specification:
Jones, et al.             Expires June 12, 2015                [Page 27]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
Jones, et al.             Expires June 12, 2015                [Page 28]
Internet-Draft            JSON Web Token (JWT)             December 2014
o  Changed the
RFC 6755
reference to be informative, based upon
      related IESG review feedback on
draft-ietf-oauth-saml2-bearer
.

   -27

   o  Removed unused reference to
RFC 4648
.
...
Jones, et al.             Expires June 12, 2015                [Page 29]
Internet-Draft            JSON Web Token (JWT)             December 2014
-22
...
reference to be normative.
   ...
   o  Updated the JSON reference to
RFC 7159
.
...
Jones, et al.             Expires June 12, 2015                [Page 30]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
Jones, et al.             Expires June 12, 2015                [Page 31]
Internet-Draft            JSON Web Token (JWT)             December 2014
...
Jones, et al.             Expires June 12, 2015                [Page 32]
Internet-Draft            JSON Web Token (JWT)             December 2014
o  Changed from using the term "byte" to "octet" when referring to 8
   ...
   o  Applied changes made by the RFC Editor to
RFC 6749
's registry
...
Jones, et al.             Expires June 12, 2015                [Page 33]
Internet-Draft            JSON Web Token (JWT)             December 2014
applied".
...
Jones, et al.             Expires June 12, 2015                [Page 34]
Internet-Draft            JSON Web Token (JWT)             December 2014
Authors' Addresses
