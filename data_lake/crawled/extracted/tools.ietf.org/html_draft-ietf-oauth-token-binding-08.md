---
{
  ...
  "url": "https://tools.ietf.org/html/draft-ietf-oauth-token-binding-08",
  ...
  "date": "",
  "length": 69327,
  "crawled_at": "2026-04-23T20:48:11"
}
---
...
4
2.1
.  Example Token Binding for Refresh Tokens  . . . . . . . .
4
3
.  Token Binding for Access Tokens . . . . . . . . . . . . . . .
6
3.1
.  Access Tokens Issued from the Authorization Endpoint  . .
7
3.1.1.  Example Access Token Issued from the Authorization
               Endpoint  . . . . . . . . . . . . . . . . . . . . . .
8
3.2
.  Access Tokens Issued from the Token Endpoint  . . . . . .
9
3.2.1.  Example Access Token Issued from the Token Endpoint .   9
3.3
.  Protected Resource Token Binding Validation . . . . . . .
11
3.3.1
.  Example Protected Resource Request  . . . . . . . . .
11
3.4
.  Representing Token Binding in JWT Access Tokens . . . . .
11
3.5
.  Representing Token Binding in Introspection Responses . .
12
4
.  Token Binding Metadata  . . . . . . . . . . . . . . . . . . .
13
4.1
.  Token Binding Client Metadata . . . . . . . . . . . . . .
13
4.2
.  Token Binding Authorization Server Metadata . . . . . . .
13
5
...
19
6.2
.  Token Bound JWTs for Client Authentication  . . . . . . .
20
6.3
.  Token Bound JWTs for as Authorization Grants  . . . . . .
20
7
...
JWT
], the term "User Agent" defined
   by
RFC 7230
[
RFC7230
], and the terms "Provided", "Referred", "Token
...
Token Binding of refresh tokens is a straightforward first-party
   ...
   HTTP [
RFC8473
].  It cryptographically binds the refresh token to the
   ...
   over HTTP
[RFC8473], Section 4
, and the access token case described
   ...
   hash of it, such a SHA-256 hash [
SHS
]) in the refresh token itself,
...
A native application client makes the following access token request
   ...
   [
BCP212
].  The base64url-encoded representation of the exported
...
Token Binding for access tokens cryptographically binds the access
   ...
   manner to that described in Token Binding over HTTP
[RFC8473],
   Section 4
(Federation Use Cases).  It also builds upon the mechanisms
...
OpenID.Core
] use case, HTTP redirects are used
...
For access tokens returned directly from the authorization endpoint,
   such as with the implicit grant defined in OAuth 2.0
[RFC6749],
   Section 4.2
, the Token Binding ID of the client's TLS channel to the
...
The access token is bound to the Referred Token Binding ID from the
   ...
   in
Section 3.4
, contains the SHA-256 hash of the Token Binding ID as
...
Section 3.2.1
would look something like the following.  The
...
If the access token is represented as a JWT, the token binding
   ...
   bound OpenID Connect ID Tokens [
OpenID.TokenBinding
].  That
   specification defines the new JWT Confirmation Method
RFC 7800
[
RFC7800
] member "tbh" (token binding hash) to represent the SHA-256
...
There are two variations for Token Binding of an authorization code.
   ...
   framework of Proof Key for Code Exchange (PKCE) [
RFC7636
], which
...
Upon receipt of the authorization code, the client sends the access
   ...
   [
RFC8471
] is negotiated on the TLS connection between the client and
   the authorization server and the "Sec-Token-Binding" header, as
   defined in Token Binding over HTTP [
RFC8473
], is included in the
   ...
   continues processing as normal (as defined by OAuth 2.0 [
RFC6749
]).
...
The "Sec-Token-Binding" header contains sufficient information for
   ...
   original authorization request.  However, PKCE [
RFC7636
] requires
...
RFC7636
   ...
   Binding-ID" header, as defined in Token Binding over HTTP
[RFC8473],
   Section 5.3
.
...
with all trailing
   ...
   normal (as defined by OAuth 2.0 [
RFC6749
...
6
.  Token Binding JWT Authorization Grants and Client Authentication
The JWT Profile for OAuth 2.0 Client Authentication and Authorization
   Grants [
RFC7523
] defines the use of bearer JWTs as a means for
...
6.1
.  JWT Format and Processing Requirements
In addition the requirements set forth in
Section 3 of RFC 7523
[
RFC7523
], the following criteria must also be met for token bound
...
Section 2.2 of RFC 7523
[
RFC7523
] with one exception: the value of the
...
RFC8414
], and in OAuth 2.0 Dynamic Client
   Registration Protocol [
RFC7591
].  The values "private_key_jwt" and
   "client_secret_jwt" are designated by OpenID Connect [
OpenID.Core
] as
   authentication method values for bearer JWT client authentication
   using asymmetric and symmetric JWS [
RFC7515
...
To use a token bound JWT for an authorization grant, the client uses
   the parameter values and encodings from
Section 2.1 of RFC 7523
[
RFC7523
] with one exception: the value of the "grant_type" is
...
7.2
.  Binding of Refresh Tokens
Section 6 of RFC 6749
[
RFC6749
] requires that a refresh token be
...
tokens issued to public clients, however, do not have the benefit of
   ...
   Clients MUST declare through dynamic (
Section 4.1
) or static
...
>.
   ...
   [
RFC7636
]  Sakimura, N., Ed., Bradley, J., and N. Agarwal, "Proof Key
...
RFC 7662
, DOI 10.17487/RFC7662, October 2015,
              <
...
>.
   ...
   [
RFC8471
]  Popov, A., Ed., Nystroem, M., Balfanz, D., and J. Hodges,
              "The Token Binding Protocol Version 1.0",
RFC 8471
,
...
>.
   ...
   [
RFC8472
]  Popov, A., Ed., Nystroem, M., and D. Balfanz, "Transport
              Layer Security (TLS) Extension for Token Binding Protocol
              Negotiation",
RFC 8472
, DOI 10.17487/RFC8472, October
              2018, <
...
RFC8473
              ...
              J. Hodges, "Token Binding over HTTP",
RFC 8473
,
...
[[ to be removed by the RFC Editor before publication as an RFC ]]
   ...
   o  Update reference to AS metadata, which is now
RFC 8414
.
   ...
   o  Update to latest references for tokbind drafts and
draft-ietf-
oauth-discovery
.

   o  Update reference to Implementation Considerations in
draft-ietf-
tokbind-https
, which is
section 6
...
Internet-Draft           OAuth 2.0 Token Binding            October 2018
o  Use the boilerplate from
RFC 8174
.

   o  Update reference for
draft-ietf-tokbind-https
to -12 and
draft-
ietf-oauth-discovery
to -09.
   ...
   o  Update references for
draft-ietf-tokbind
- negotiation(-10),
      protocol(-16), and https(-10), as well as
draft-ietf-oauth-
discovery
(-07), and
BCP212
/RFC8252 OAuth 2.0 for Native Apps.
   ...
   o  Define how to convey token binding information of an access token
      via
RFC 7662
OAuth 2.0 Token Introspection (note that the
...
token-binding-00.pdf
   ...
   o  Created the initial working group version from
draft-jones-oauth-
token-binding-00
.
