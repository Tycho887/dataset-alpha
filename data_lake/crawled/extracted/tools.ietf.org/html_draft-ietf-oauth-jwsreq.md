---
{
  ...
  "url": "https://tools.ietf.org/html/draft-ietf-oauth-jwsreq",
  ...
  "date": "",
  "length": 73362,
  "crawled_at": "2026-04-23T20:51:31"
}
---
...
Expires: October 10, 2021                                         Yubico
                                                                M. Jones
                                                               Microsoft
                                                           April 8, 2021
...
Abstract
   ...
   The authorization request in OAuth 2.0 described in
RFC 6749
utilizes
...
Sakimura, et al.        Expires October 10, 2021                [Page 1]
Internet-Draft                  OAuth JAR                     April 2021
...
.  Explicit Endpoints . . . . . . . . . . . . . . . . . . .
19
Sakimura, et al.        Expires October 10, 2021                [Page 2]
Internet-Draft                  OAuth JAR                     April 2021
10.4
.  Risks Associated with request_uri  . . . . . . . . . . .
19
10.4.1
.  DDoS Attack on the Authorization Server  . . . . . .
20
10.4.2
.  Request URI Rewrite  . . . . . . . . . . . . . . . .
20
...
23
11.2.2
.  Tracking using Request Object URI  . . . . . . . . .
23
12
. Acknowledgements  . . . . . . . . . . . . . . . . . . . . . .
24
...
1
.  Introduction
The Authorization Request in OAuth 2.0 [
RFC6749
] utilizes query
   ...
   As the result, the Authorization Request of [
RFC6749
] has
   ...
   (a)  the communication through the user agents is not integrity
        ...
        protection failure)
Sakimura, et al.        Expires October 10, 2021                [Page 3]
Internet-Draft                  OAuth JAR                     April 2021
...
RFC7516
   ...
   [
RFC6749
] flows.  The "request" parameter is a JSON Web Token (JWT)
   [
RFC7519
] whose JWT Claims Set holds the JSON encoded OAuth 2.0
   authorization request parameters.  Note that, in contrast to
RFC
7519
, the elements of the Claims Set are encoded OAuth Request
   Parameters [
IANA.OAuth.Parameters
], supplemented with only a few of
   the IANA-managed JSON Web Token Claims [
IANA.JWT.Claims
] - in
   ...
   The JWT [
RFC7519
] can be passed to the authorization endpoint by
   reference, in which case the parameter "request_uri" is used instead
   of the "request".
Sakimura, et al.        Expires October 10, 2021                [Page 4]
Internet-Draft                  OAuth JAR                     April 2021
Using JWT [
RFC7519
] as the request encoding instead of query
...
Sakimura, et al.        Expires October 10, 2021                [Page 5]
Internet-Draft                  OAuth JAR                     April 2021
2
.  Terminology
...
Absolute URI that references the set of parameters comprising an
   ...
   referenced by the URI are a Request Object (
Section 2.1
), unless the
...
Sakimura, et al.        Expires October 10, 2021                [Page 6]
Internet-Draft                  OAuth JAR                     April 2021
4
.  Request Object
A Request Object (
Section 2.1
   ...
   process the OAuth 2.0 [
RFC6749
] authorization request except the
   ...
   potentially outside of a closed ecosystem, per
section 8.1 of
   [RFC8259]
...
RFC3629
   ...
   any extension parameters.  This JSON [
RFC8259
] object constitutes the
   JWT Claims Set defined in JWT [
RFC7519
].  The JWT Claims Set is then
   signed or signed and encrypted.

   To sign, JSON Web Signature (JWS) [
RFC7515
] is used.  The result is a
   JWS signed JWT [
RFC7519
   ...
   [
RFC7519
] specification.  The value of "aud" should be the value of
   the Authorization Server (AS) "issuer" as defined in
RFC8414
[
RFC8414
].
   ...
   To encrypt, JWE [
RFC7516
] is used.  When both signature and
...
RFC8414
   ...
   The Request Object MAY be sent by value as described in
Section 5.1
or by reference as described in
Section 5.2
.  "request" and
   "request_uri" parameters MUST NOT be included in Request Objects.
Sakimura, et al.        Expires October 10, 2021                [Page 7]
Internet-Draft                  OAuth JAR                     April 2021
A Request Object (
Section 2.1
) has the media type [
RFC2046
   ...
   base64url [
RFC7515
] encoding and signing.  Note that it includes the
...
Sakimura, et al.        Expires October 10, 2021                [Page 8]
Internet-Draft                  OAuth JAR                     April 2021
...
The client constructs the authorization request URI by adding the
   ...
   request
      ...
      (
Section 2.1
) that holds authorization request parameters stated
      in
section 4
of OAuth 2.0 [
RFC6749
...
[
RFC3986
] that is the Request Object URI
      (
Section 2.2
...
RFC6749
   ...
   client_id
      REQUIRED.  OAuth 2.0 [
RFC6749
] "client_id".  The value MUST match
      the "request" or "request_uri" Request Object's (
Section 2.1
)
      "client_id".
...
Sakimura, et al.        Expires October 10, 2021                [Page 9]
Internet-Draft                  OAuth JAR                     April 2021
...
Sakimura, et al.        Expires October 10, 2021               [Page 10]
Internet-Draft                  OAuth JAR                     April 2021
...
RFC7230
   ...
   case, it MUST be a URN, as specified in
RFC8141
[
RFC8141
].
...
Sakimura, et al.        Expires October 10, 2021               [Page 11]
Internet-Draft                  OAuth JAR                     April 2021
It is possible for the Request Object to include values that are to
   ...
   refer to 5.1.4.2.2 of [
RFC6819
] and Good Practices for Capability
   URLs [
CapURLs
].  It is RECOMMENDED that it be removed after a
...
Sakimura, et al.        Expires October 10, 2021               [Page 12]
Internet-Draft                  OAuth JAR                     April 2021
...
If the request object is encrypted, the Authorization Server MUST
   decrypt the JWT in accordance with the JSON Web Encryption [
RFC7516
]
   specification.
...
The Authorization Server MUST validate the signature of the JSON Web
   Signature [
RFC7515
] signed Request Object.  If a "kid" Header
   Parameter is present, the key identified MUST be the key used, and
   MUST be a key associated with the client.  The signature MUST be
   validated using a key associated with the client and the algorithm
   specified in the "alg" Header Parameter.  Algorithm verification MUST
   be performed, as specified in Sections
3.1
and
3.2
of [
...
Sakimura, et al.        Expires October 10, 2021               [Page 13]
Internet-Draft                  OAuth JAR                     April 2021
"invalid_request_object" error to the client in response to the
   authorization request.
6.3
...
The Authorization Server MUST extract the set of Authorization
   ...
   validates the request as specified in OAuth 2.0 [
RFC6749
].
   ...
   to the authorization request, as specified in
Section 5.2
of OAuth
   2.0 [
RFC6749
...
Sakimura, et al.        Expires October 10, 2021               [Page 14]
Internet-Draft                  OAuth JAR                     April 2021
To protect against information disclosure and tampering,
   ...
   ID [
RFC6125
], to avoid man-in-the-middle attacks.  The rules and
...
BCP195
   ...
   o  SRV-ID and URI-ID as described in
Section 6.5 of [RFC6125]
MUST
...
and this
   ...
   o  Parameter Usage Location: authorization request
   o  Change Controller: IETF
Sakimura, et al.        Expires October 10, 2021               [Page 15]
Internet-Draft                  OAuth JAR                     April 2021
o  Specification Document(s):
...
Section 10.5
of this document.
Sakimura, et al.        Expires October 10, 2021               [Page 16]
Internet-Draft                  OAuth JAR                     April 2021
9.3
.  OAuth Dynamic Client Registration Metadata Registry
...
Sakimura, et al.        Expires October 10, 2021               [Page 17]
Internet-Draft                  OAuth JAR                     April 2021
o  Change controller: IETF
...
RFC8725
] need to be considered.  Also, there are
   several academic papers such as [
BASIN
] that provide useful insight
...
When sending the authorization request object through "request"
   parameter, it MUST either be signed using JWS [
RFC7515
] or signed
   then encrypted using JWS [
RFC7515
] and JWE [
RFC7516
] respectively,
...
Sakimura, et al.        Expires October 10, 2021               [Page 18]
Internet-Draft                  OAuth JAR                     April 2021
...
Although this specification does not require them, research such as
   [
BASIN
] points out that it is a good practice to explicitly state the
...
RFC8414
   ...
   In [
RFC6749
], while Redirection URI is included in the Authorization
   Request, others are not.  As a result, the same applies to
   Authorization Request Object.
10.4
.  Risks Associated with request_uri
The introduction of "request_uri" introduces several attack
   possibilities.  Consult the security considerations in
Section 7 of
   RFC3986
[
RFC3986
] for more information regarding risks associated
   with URIs.
Sakimura, et al.        Expires October 10, 2021               [Page 19]
Internet-Draft                  OAuth JAR                     April 2021
10.4.1
.  DDoS Attack on the Authorization Server
A set of malicious client can launch a DoS attack to the
   ...
   location, (b) check the media type of the response is "application/
...
Unless the protocol used by client and the server is locked down to
   use OAuth JAR, it is possible for an attacker to use
RFC6749
requests
   ...
   the request object uses "alg":"none" when this client metadata value
   is "true".  If omitted, the default value is "false".
Sakimura, et al.        Expires October 10, 2021               [Page 20]
Internet-Draft                  OAuth JAR                     April 2021
When the value of it as a server metadata is "true", then the server
   ...
   conform to this specification.  It MUST also reject the request if
   the request object uses "alg":"none" when this server metadata value
   is "true".  If omitted, the default value is "false".
   ...
   server.  Use of signing algorithm metadata is described in
Section 4
.
10.6
.  TLS Security Considerations
...
BCP195
].  This supersedes the TLS version
   recommendations in OAuth 2.0 [
RFC6749
].
10.7
.  Parameter Mismatches
...
.  A simple way to prevent this is to never
...
Sakimura, et al.        Expires October 10, 2021               [Page 21]
Internet-Draft                  OAuth JAR                     April 2021
...
When the Client is being granted access to a protected resource
   ...
   Server need to adhere to Privacy Principles.
RFC 6973
Privacy
...
RFC6973
   ...
   Framework [
RFC6749
] and The OAuth 2.0 Authorization Framework: Bearer
   Token Usage [
RFC6750
] and are not specific to this specification.  In
...
When the Client is being granted access to a protected resource
   ...
   (1)  (Certification Process) The trusted third-party service examines
        ...
        is certified, then they are issued a client credential to
        authenticate against to push request objects to the trusted
        third-party service to get the "request_uri".
Sakimura, et al.        Expires October 10, 2021               [Page 22]
Internet-Draft                  OAuth JAR                     April 2021
(2)  (Translation Process) The client uses the client credential that
        ...
        the claims that the client is eligible for, per prior step.
...
This specification allows extension parameters.  These may include
   ...
   sensitive parameter, the Client SHOULD JWE [
RFC7516
] encrypt the
   request object.
   ...
   security policy unless the Request Object itself is JWE [
RFC7516
]
...
Sakimura, et al.        Expires October 10, 2021               [Page 23]
Internet-Draft                  OAuth JAR                     April 2021
...
Sakimura, et al.        Expires October 10, 2021               [Page 24]
Internet-Draft                  OAuth JAR                     April 2021
-32
...
Section 3
Removed WAP
Sakimura, et al.        Expires October 10, 2021               [Page 25]
Internet-Draft                  OAuth JAR                     April 2021
o
Section 4
.  Clarified authorization request object parameters,
      removed extension parameters from examples

   o
Section 4
.  Specifies application/oauth.authz.req+jwt as mime-type
      fore request objects

   o
Section 5.2.1
Added reference to Capability URLs

   o
Section 5.2.3
.  Added entropy fragment to example request

   o
Section 8
.  Replaced "subjectAltName dnsName" with "DNS-ID"

   o
Section 9
.  Registers authorization request parameters in JWT
      Claims Registry.

   o
Section 9
.  Registers application/oauth.authz.req in IANA mime-
      types registry

   o
Section 10.1
.  Clarified encrypted request objects are "signed
      then encrypted" to maintain consistency

   o
Section 10.2
.  Clarifies trust between AS and TFP

   o
Section 10.3
.  Clarified endpoints subject to the practice

   o
Section 10.4
Replaced "redirect_uri" to "request_uri"
...
for risks
   ...
   o
Section 10.4.1
.d Deleted "do" to maintain grammar flow

   o
Section 10.4.1
, 10.4.2 Replaced "application/jose" to
      "application/jwt"

   o
Section 12.1
.  Extended description for submitting authorization
      request to TFP to obtain request object

   o
Section 12.2.2
.  Replaced per-user Request Object URI with static
      per-user Request URIs

   o
Section 13
.  Combined OAuth WG contributors together
   ...
   o  AD comments
Sakimura, et al.        Expires October 10, 2021               [Page 26]
Internet-Draft                  OAuth JAR                     April 2021
o
Section 5.2.1
.  s/Requiest URI/Request URI/
...
Sakimura, et al.        Expires October 10, 2021               [Page 27]
Internet-Draft                  OAuth JAR                     April 2021
-12
...
Request Object - Clarify
   ...
   o  fixes #51 - OPSDIR Review :
Section 4
Request Object - 'It is
      fine' => 'It is recommended'

   o  fixes #47 - OPSDIR Review : Introduction - 'over- the- wire' =>
      'over-the-wire'
Sakimura, et al.        Expires October 10, 2021               [Page 28]
Internet-Draft                  OAuth JAR                     April 2021
...
in
Section 11
in addition to ISO 29100

   o  #24: SECDIR review:
Section 4
-- Confusing requirements for
      sign+encrypt

   o  #25: SECDIR review:
Section 6
-- authentication and integrity need
      not be provided if the requestor encrypts the token?

   o  #26: SECDIR Review:
Section 10
-- why no reference for JWS
      algorithms?

   o  #27: SECDIR Review:
Section 10.2
- how to do the agreement between
      client and server "a priori"?

   o  #28: SECDIR Review:
Section 10.3
- Indication on "large entropy"
...
- Typo
   ...
   o  #30: SECDIR Review:
Section 10.4
- typos and missing articles
Sakimura, et al.        Expires October 10, 2021               [Page 29]
Internet-Draft                  OAuth JAR                     April 2021
o  #31: SECDIR Review:
Section 10.4
- Clearer statement on the lack
      of endpoint identifiers needed

   o  #32: SECDIR Review:
Section 11
- ISO29100 needs to be moved to
      normative reference

   o  #33: SECDIR Review:
Section 11
- Better English and Entropy
      language needed
...
.
   ...
   o  Applied changes proposed by Hannes on 2016-06-29 on IETF OAuth
      list recorded as
https://bitbucket.org/Nat/oauth-jwsreq/
issues/12/
.
   ...
   o  Better English.
Sakimura, et al.        Expires October 10, 2021               [Page 30]
Internet-Draft                  OAuth JAR                     April 2021
...
Sakimura, et al.        Expires October 10, 2021               [Page 31]
Internet-Draft                  OAuth JAR                     April 2021
-03
...
>.
Sakimura, et al.        Expires October 10, 2021               [Page 32]
Internet-Draft                  OAuth JAR                     April 2021
[
RFC6125
...
>.
Sakimura, et al.        Expires October 10, 2021               [Page 33]
Internet-Draft                  OAuth JAR                     April 2021
[
RFC8259
...
[
BASIN
]    Basin, D., Cremers, C., and S. Meier, "Provably Repairing
...
>.
Sakimura, et al.        Expires October 10, 2021               [Page 34]
Internet-Draft                  OAuth JAR                     April 2021
[
RFC6838
...
RFC 7591
, DOI 10.17487/RFC7591, July 2015,
              <
...
Sakimura, et al.        Expires October 10, 2021               [Page 35]
Internet-Draft                  OAuth JAR                     April 2021
Michael B. Jones
   Microsoft
   One Microsoft Way
...
Sakimura, et al.        Expires October 10, 2021               [Page 36]