---
{
  ...
  "date": "",
  "length": 88655,
  "crawled_at": "2026-04-23T20:51:43"
}
---
...
8
4.1
. Calculating Secondary Keys with Vary .......................
9
4.2
. Freshness .................................................
11
...
32
5.5.7
. Warning: 299 - "Miscellaneous Persistent Warning" ..32
6
. History Lists ..................................................
32
7
...
, if the response can be reused without "validation"
   ...
   can be freshened by validation (
Section 4.3
) or if the origin is
   unavailable (
Section 4.2.4
...
A cache MUST NOT store a response to any request, unless:
   ...
   o  the "no-store" cache directive (see
Section 5.2
) does not appear
      in request or response header fields, and

   o  the "private" response directive (see
Section 5.2.2.6
) does not
      appear in the response, if the cache is shared, and

   o  the Authorization header field (see
...
Section 3.2
   ...
   o  the response either:
      ...
      *  contains an Expires header field (see
Section 5.3
), or

      *  contains a max-age response directive (see
Section 5.2.2.8
), or
...
Section 5.2.2.9
      ...
      *  contains a Cache Control Extension (see
Section 5.2.3
) that
...
A response message is considered complete when all of the octets
   indicated by the message framing ([
RFC7230
]) are received prior to
...
RFC7233
]) and combining the successful
   response with the stored entry, as defined in
Section 3.3
.  A cache
...
Section 4.2 of [RFC7235]
   ...
   directives (
Section 5.2.2
) have such an effect: must-revalidate,
   public, and s-maxage.
...
Note that cached responses that contain the "must-revalidate" and/or
   ...
   (
Section 4.2.4
) by shared caches.  In particular, a response with
...
A response might transfer only a partial representation if the
   ...
   Range specifiers ([
RFC7233
]).  After several such transfers, a cache
...
When presented with a request, a cache MUST NOT reuse a stored
   ...
   o  The presented effective request URI (
...
o  the presented request does not contain the no-cache pragma
      (
Section 5.4
), nor the no-cache cache directive (
Section 5.2.1
),
...
) to the origin server; i.e., a cache is
...
Section 5.2.2.8
   ...
   o  If the Expires response header field (
Section 5.3
) is present, use
...
Section
6.1 of [RFC7231]
), and those responses without explicit freshness
...
Section 5.5.4
) in the response if its current_age is more than 24
   hours and such a warning is not already present.

      Note:
Section 13.9 of [RFC2616]
prohibited caches from calculating
...
value is the sum of the time that the response has been resident in
   ...
   age_value
      ...
      (
Section 5.1
), in a form appropriate for arithmetic operation; or
      0, if not available.
...
for the definition of the Date header field,
   ...
   now
      ...
      performing the calculation".  A host ought to use NTP ([
RFC5905
])
...
Section 5.2.2
   ...
   max-stale request directive; see
Section 5.2.1
).
   ...
   (see
Section 5.5.1
) in stale responses.  Likewise, a cache SHOULD
   generate a 112 warn-code (see
Section 5.5.3
) in stale responses if
...
).  One or more entity-tags, indicating one
...
Section 3.2 of
   [RFC7232]
) indicates that the client wants to validate one or more of
...
present in the selected stored response, but the cache recorded it as
   ...
   defined in [
RFC7233
], also needs to evaluate a received If-Range
   header field (
Section 3.2 of [RFC7233]
) with respect to its selected
   stored response.
4.3.3
.  Handling a Validation Response
...
Section 2.1
      of [RFC7232]
), then that strong validator identifies the selected
...
Section 5.5 of
   [RFC7230]
) as well as the URI(s) in the Location and Content-Location
...
for
   ...
   request containing Authorization, and
Section 3
for details of how
...
.
   ...
   directive (
Section 5.2.2.8
), a recipient MUST ignore the Expires
   field.  Likewise, if a response includes the s-maxage directive
   (
Section 5.2.2.9
), a shared cache recipient MUST ignore the Expires
...
,
   ...
   [
RFC5861
]  Nottingham, M., "HTTP Cache-Control Extensions for Stale
              Content",
RFC 5861
, April 2010.
...
Section 4.1
   ...
   performing invalidation have been clarified.  (
Section 4.4
)
...
Section 5.3
   ...
   compatibility; future pragmas are deprecated.  (
Section 5.4
)

   Some requirements regarding production and processing of the Warning
   header fields have been relaxed, as it is not widely implemented.
   Furthermore, the Warning header field no longer uses
RFC 2047
