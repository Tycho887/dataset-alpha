---
{
  ...
  "date": "",
  "length": 43923,
  "crawled_at": "2026-04-23T20:57:11"
}
---
...
13
5.1
.  Limits to the Maximum Number of Connections  . . . . . . .
13
5.2
...
15
5.6
.  Impact on Intermediary Entities  . . . . . . . . . . . . .
16
6
...
RFC2616
   ...
   which are often grouped under the common label "Comet" [
COMET
],
...
Section 2
analyzes the HTTP long polling technique.
Section 3
analyzes the
   HTTP streaming technique.
Section 4
provides an overview of the
   specific technologies that use the server-push technique.
Section 5
...
packet, next packet, negative ack, retransmit).  When HTTP
      pipelining (see
Section 5.2
) is available, the latency due to the
...
The Bayeux protocol [
BAYEUX
] was developed in 2006-2007 by the Dojo
...
described in [
JSONP
].  In this last case, client-to-server messages
...
BOSH, which stands for Bidirectional-streams Over Synchronous HTTP
   [
BOSH
], was developed by the XMPP Standards Foundation in 2003-2004.
   ...
   Messaging and Presence Protocol as described in [
RFC6120
]).  BOSH
...
BOSH is able to work reliably both when network conditions force
   ...
   POST with pipelining is discouraged in
RFC 2616
, BOSH employs various
...
, recommends that a single user client
   ...
   any resource between frames.  The new HTTP specification [
HTTPBIS
]
...
HTTP [
RFC2616
] permits optional request pipelining over persistent
   ...
   fact, the use of HTTP POST with pipelining is discouraged in
RFC 2616
...
Most proxies work well with HTTP long polling because a complete HTTP
   ...
   JavaScript in an HTTP/1.1 chunk as described in
Section 3
.  This
...
In accordance with [
RFC2616
], the server responds to a request it has
...
This document is meant to describe current usage of HTTP to enable
   ...
   browser into executing the code [
CORS
].
...
[
BAYEUX
]          Russell, A., Wilkins, G., Davis, D., and M.
...
BOSH
   ...
   [
COMET
]           Russell, A., "Comet: Low Latency Data for the
                     Browser", March 2006, <
...
2006/03/comet-low-latency-data-for-the-browser/ >.
   ...
   [
COOKIE
]          Barth, A., "HTTP State Management Mechanism", Work
...
>.
   ...
   [
HTTPBIS
]         Fielding, R., Ed., Gettys, J., Mogul, J., Nielsen,
...
, September 1981.
   ...
   [
WD-eventsource
]  Hickson, I., "Server-Sent Events", W3C Working
