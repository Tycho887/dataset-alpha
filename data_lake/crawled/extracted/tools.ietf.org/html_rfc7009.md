---
{
  ...
  "date": "",
  "length": 22967,
  "crawled_at": "2026-04-23T20:51:37"
}
---
...
1
.  Introduction
The OAuth 2.0 core specification [
RFC6749
] defines several ways for a
...
Implementations MUST support the revocation of refresh tokens and
   ...
   MUST conform to the rules given in
[RFC6749], Section 3.1
.  Clients
...
Since requests to the token revocation endpoint result in the
   ...
   server MUST use Transport Layer Security (TLS) [
RFC5246
] in a version
   compliant with
[RFC6749], Section 1.6
...
Section 5.2 of
   [RFC6749]
.  The following additional error code is defined for the
...
This specification establishes the "OAuth Token Type Hints" registry.
   Possible values of the parameter "token_type_hint" (see
Section 2.1
)
   are registered with a Specification Required ([
RFC5226
]) after a two-
...
of the OAuth core specification
   [
RFC6749
] and the OAuth threat model document [
RFC6819
].
...
[RFC6819],
   Section 4.4.1.11
).  Specifically, invalid token type hints may
