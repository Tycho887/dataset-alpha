---
{
  ...
  "date": "",
  "length": 105988,
  "crawled_at": "2026-04-23T20:54:42"
}
---
...
RFC 7516
JSON Web Encryption (JWE)               May 2015
Table of Contents
1
.  Introduction  . . . . . . . . . . . . . . . . . . . . . . . .
4
...
RFC7159
].  The JWE cryptographic mechanisms
...
JWA
   ...
   described in the separate JSON Web Signature (JWS) [
JWS
]
   specification.
...
In the JWE Compact Serialization, a JWE is represented as the
   ...
   See
Section 7.1
for more information about the JWE Compact
   Serialization.
3.2
.  JWE JSON Serialization Overview
In the JWE JSON Serialization, one or more of the JWE Protected
   ...
   See
Section 7.2
for more information about the JWE JSON
   Serialization.
...
NIST.800-38D
] algorithm with a 256-bit key to
...
Appendix A
for additional examples, including examples using the JWE
...
For a JWE, the members of the JSON object(s) representing the JOSE
   ...
   Section 4 of [
JWS
].  The rules about handling Header Parameters that
...
This parameter has the same meaning, syntax, and processing rules as
   the "alg" Header Parameter defined in Section 4.1.1 of [
JWS
], except
...
The "zip" (compression algorithm) applied to the plaintext before
   ...
   o  "DEF" - Compression with the DEFLATE [
RFC1951
] algorithm
...
.  "jku" (JWK Set URL) Header Parameter
This parameter has the same meaning, syntax, and processing rules as
   the "jku" Header Parameter defined in Section 4.1.2 of [
JWS
], except
...
.  "jwk" (JSON Web Key) Header Parameter
This parameter has the same meaning, syntax, and processing rules as
   the "jwk" Header Parameter defined in Section 4.1.3 of [
JWS
], except
...
.  "kid" (Key ID) Header Parameter
This parameter has the same meaning, syntax, and processing rules as
   the "kid" Header Parameter defined in Section 4.1.4 of [
JWS
], except
...
.  "x5u" (X.509 URL) Header Parameter
This parameter has the same meaning, syntax, and processing rules as
   the "x5u" Header Parameter defined in Section 4.1.5 of [
JWS
], except
   that the X.509 public key certificate or certificate chain [
RFC5280
]
...
.  "x5c" (X.509 Certificate Chain) Header Parameter
This parameter has the same meaning, syntax, and processing rules as
   the "x5c" Header Parameter defined in Section 4.1.6 of [
JWS
], except
   that the X.509 public key certificate or certificate chain [
RFC5280
...
This parameter has the same meaning, syntax, and processing rules as
   the "x5t" Header Parameter defined in Section 4.1.7 of [
JWS
], except
...
Parameter
This parameter has the same meaning, syntax, and processing rules as
   the "x5t#S256" Header Parameter defined in Section 4.1.8 of [
JWS
],
...
.  "typ" (Type) Header Parameter
This parameter has the same meaning, syntax, and processing rules as
   the "typ" Header Parameter defined in Section 4.1.9 of [
JWS
], except
...
.  "cty" (Content Type) Header Parameter
This parameter has the same meaning, syntax, and processing rules as
   the "cty" Header Parameter defined in Section 4.1.10 of [
JWS
], except
...
.  "crit" (Critical) Header Parameter
This parameter has the same meaning, syntax, and processing rules as
   the "crit" Header Parameter defined in Section 4.1.11 of [
JWS
],
...
The message encryption process is as follows.  The order of the steps
   ...
   2.   When Key Wrapping, Key Encryption, or Key Agreement with Key
        Wrapping are employed, generate a random CEK value.  See
RFC
4086
[
RFC4086
...
17.  Compute the encoded Authentication Tag value BASE64URL(JWE
   ...
   19.  Create the desired serialized output.  The Compact Serialization
        ...
        JWE JSON Serialization is described in
Section 7.2
.
5.2
.  Message Decryption
...
2.   Base64url decode the encoded representations of the JWE
   ...
   3.   Verify that the octet sequence resulting from decoding the
        ...
        of a completely valid JSON object conforming to
RFC 7159
[
RFC7159
...
The string comparison rules for this specification are the same as
   those defined in Section 5.3 of [
JWS
].
6
.  Key Identification
The key identification methods for this specification are the same as
   those defined in Section 6 of [
JWS
], except that the key being
...
value that would have been computed for the same parameter in the JWE
   ...
   See
Appendix A.4
for an example JWE using the general JWE JSON
...
The "recipients" member MUST NOT be present when using this syntax.
   ...
   See
Appendix A.5
for an example JWE using the flattened JWE JSON
   Serialization syntax.
8
.  TLS Requirements
The Transport Layer Security (TLS) requirements for this
   specification are the same as those defined in Section 8 of [
JWS
].
9
.  Distinguishing between JWS and JWE Objects
...
See Section 10.2 of [
JWS
] for security considerations on key
...
[
RFC3766
] for information on determining strengths
...
[
RFC3218
] should be consulted for specific countermeasures to attacks
...
Encrypt the CEK with the recipient's public key using the RSAES-OAEP
   ...
   RSA key represented in JSON Web Key [
JWK
] format below (with line
...
Encrypt the CEK with the recipient's public key using the
   ...
   example uses the RSA key represented in JSON Web Key [
JWK
] format
...
Encrypt the CEK with the shared symmetric key using the AES Key Wrap
   algorithm to produce the JWE Encrypted Key.  This example uses the
   symmetric key represented in JSON Web Key [
JWK
] format below:
...
This section contains an example using the flattened JWE JSON
   ...
   recipient of the previous example in
Appendix A.4
.
...
JSE
   ...
   [
JSMS
], both of which significantly influenced this document.  This
   document attempts to explicitly reuse as many of the relevant
   concepts from XML Encryption 1.1 [
W3C.REC-xmlenc-core1-20130411
] and
RFC 5652
[
RFC5652
] as possible, while utilizing simple, compact JSON-
   based data structures.
   ...
   reuse of text from [
JSMS
] in this document; and to Eric Rescorla for
