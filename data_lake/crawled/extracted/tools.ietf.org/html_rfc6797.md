---
{
  ...
  "date": "",
  "length": 101862,
  "crawled_at": "2026-04-23T20:57:13"
}
---
...
RFC 6797
HTTP Strict Transport Security (HSTS)    November 2012
Copyright Notice
...
13
5.1
. HSTS Host Declaration .....................................
13
5.2
. HSTS Policy ...............................................
13
5.3
. HSTS Policy Storage and Maintenance by User Agents ........
14
5.4
...
23
9.1
. ERU Fundamental Definitions ...............................
23
9.2
. Determining the Effective Request URI .....................
24
9.2.1
. Effective Request URI Examples .....................
24
10
...
27
11.4
. Implications of includeSubDomains ........................
28
...
32
14.1
. Underlying Secure Transport Considerations ...............
32
14.2
. Non-Conformant User Agent Implications ...................
33
14.3. Ramifications of HSTS Policy Establishment Only over
            Error-Free Secure Transport ..............................
33
14.4
. The Need for includeSubDomains ...........................
34
14.5
. Denial of Service ........................................
35
...
HTTP [
RFC2616
] may be used over various transports, typically the
   ...
   [
RFC6101
] and its successor, Transport Layer Security (TLS) [
RFC5246
]
   were developed in order to provide channel-oriented security and are
   typically layered between application protocols and TCP.  [
RFC2818
]
   ...
   example, cookies ([
RFC6265
]) may be flagged as Secure.  UAs are to
...
Section 3.1 of [RFC2818]
   ...
   to as "click(ing) through" security [
GoodDhamijaEtAl05
]
   [
SunshineEgelmanEtAl09
   ...
   Jackson and Barth proposed an approach, in [
ForceHTTPS
], to enable
   ...
   [
ForceHTTPS
].  For example, rather than using a cookie to convey
   ...
   This specification also incorporates notions from [
JacksonBarth2008
]
...
This specification begins with an overview of the use cases, policy
   effects, threat models, and requirements for HSTS (in
Section 2
).
   Then,
Section 3
...
defines
   ...
   formally specified in Sections
5
through
15
.
...
When a user browses the web on a local wireless network (e.g., an
   ...
   wireless network itself is secured [
BeckTews09
].  Freely available
   wireless sniffing toolkits (e.g., [
Aircrack-ng
]) enable such passive
   ...
   [
ForceHTTPS
].  For example, there exist widely available tools, such
   as Firesheep (a web browser extension) [
Firesheep
], that enable their
   ...
   through URIs constructed with the "https" scheme [
RFC2818
].  This can
...
]) refers to the notion termed "mixed
...
6.  UAs need to disallow security policy application to peer domains,
   ...
   7.  UAs need to prevent users from "clicking through" security
       ...
       transport exceptions is acceptable.  See also
Section 12.1
("No
       User Recourse").
...
("Bootstrap MITM
          ...
          fully meet the first core requirement; see
Section 12
("User
...
These ancillary requirements are also derived from the overall
   ...
   1.  Disallow "mixed security context" loads (see
Section 2.3.1.3
).
...
Unicode
   ...
   domain name:
      ...
      is also referred to as "DNS name" and is defined in [
RFC1035
] to
      ...
      ABNF for URI" in [
RFC3986
], and the host component from the Host
      HTTP header field production in
Section 14.23 of [RFC2616]
.
...
RFC4949
   ...
   UA:
      ...
      actively manipulated by a user [
RFC2616
].
...
("Strict-Transport-Security Response
   Header Field Processing") for details.  Additionally,
Section 6.2
...
Section 8.3
("URI Loading and Port Mapping") for details.
6
.  Syntax
...
Section 7
("Server Processing Model") then details how hosts employ
   this header field to declare their HSTS Policy, and
Section 8
("User
...
(which includes a notion of "implied linear
...
The REQUIRED "max-age" directive specifies the number of seconds,
   ...
   Host.  See also
Section 8.1.1
("Noting an HSTS Host - Storage
   Model").  The delta-seconds production is specified in [
RFC2616
].
   ...
   quoted-string unescaping, if necessary) is defined as:
    ...
    delta-seconds = <1*DIGIT, defined in
[RFC2616], Section 3.3.2
>
...
If an HSTS Host receives an HTTP request message over a non-secure
   ...
   (
Section 10.3.2 of [RFC2616]
), and a Location header field value
...
("Internationalized Domain Names for Applications (IDNA): Dependency
   ...
   outlined in
Section 10
("Domain Name IDNA-Canonicalization") prior to
   the processing specified in this section.

      NOTE:  [
RFC3490
] is referenced due to its ongoing relevance to
...
If an HTTP response, received over a secure transport, includes an
   STS header field, conforming to the grammar specified in
Section 6.1
...
If the substring matching the host production from the Request-URI
   ...
   the IP-literal or IPv4address productions from
Section 3.2.2 of
   [RFC3986]
, then the UA MUST NOT note this host as a Known HSTS Host.
...
A given domain name may match a Known HSTS Host's domain name in one
   ...
   of which fashion:
      ...
      continuing right-to-left.  See also
...
Whenever the UA prepares to "load" (also known as "dereference") any
   "http" URI [
RFC3986
] (including when following HTTP redirects
   [
RFC2616
]), the UA MUST first determine whether a domain name is
   ...
   3.  Else, if the substring is non-null and syntactically matches the
       IP-literal or IPv4address productions from
Section 3.2.2 of
       [RFC3986]
...
("User Agent Implementation Advice"))
   ...
   employ, such as via Certificate Revocation Lists (CRLs) [
RFC5280
], or
   via the Online Certificate Status Protocol (OCSP) [
RFC2560
], as well
   as via TLS server identity checking [
RFC6125
].
8.5
.  HTTP-Equiv <Meta> Element Attribute
...
This section specifies how an HSTS Host must construct the Effective
   ...
   Host header field, and connection context ([
RFC2616
], Sections
3.2.1
,
...
An IDNA-canonicalized domain name is the output string generated by
   ...
   "NR-LDH labels" (see
Section 2 of [RFC5890]
) concatenated using some
   ...
   2.  When implementing IDNA2008, convert, validate, and test each
       ...
       strings, using the procedures defined in Sections
5.3
through
5.5
of [
RFC5891
       ...
       the "ToASCII" conversion in
Section 4 of [RFC3490]
(see also the
       definition of "equivalence of labels" in
Section 2 of [RFC3490]
).
...
Non-conformant UAs ignore the Strict-Transport-Security header field;
   ...
   in
Section 2.3.1
("Threats Addressed").  Please refer to
Section 14.2
...
HTTP Strict Transport Security (HSTS)    November 2012
11.3
.  Using HSTS in Conjunction with Self-Signed Public-Key
Certificates
...
RFC2560
   ...
   <
http://crl-and-ocsp.ca.example.com/
...
o  offer plain HTTP-based services at a different domain name, e.g.,
   ...
   o  utilize an alternative approach to distributing certificate status
      ...
      TLS extension [
RFC6066
], often colloquially referred to as "OCSP
      Stapling").
...
11.4.2
.  Considerations for Offering Web Applications at Subdomains of
an HSTS Host
...
Failing secure connection establishment on any warnings or errors
   (per
Section 8.4
("Errors in Secure Transport Establishment")) should
...
This specification is fashioned to be independent of the secure
   ...
   requirements in
Section 2
("Overview") in fact presume TLS or SSL as
...
("Threats Addressed").
   ...
   site development and deployment bugs (see
Section 2.3.1.3
) as they
   would if they were not asserting HSTS Policy.
14.3
.  Ramifications of HSTS Policy Establishment Only over Error-Free
Secure Transport
The user agent processing model defined in
Section 8
("User Agent
...
The rationale behind this is that if there is a "man in the middle"
   ...
   entity -- it could cause various mischief (see also
Appendix A
("Design Decision Notes") item 3, as well as
Section 14.6
("Bootstrap
...
("Denial of Service")).
   ...
   The above discussion relates to the recommendation in
Section 12
...
("HSTS Policy
      ...
      *  If the web application has an HTTP response splitting
         vulnerability [
CWE-113
] (which can be abused in order to
...
Bootstrap MITM (man-in-the-middle) vulnerability is a vulnerability
   ...
   various attacks (see Section 5.3 of [
ForceHTTPS
]).
...
An attacker could conceivably obtain users' login credentials
   ...
   such as DNS Security Extensions [
RFC4033
], plus techniques to block
...
Internet security relies in part on the DNS and the domain names it
   ...
   specifications (e.g., as noted in
Section 10
("Domain Name IDNA-
   ...
   Additionally, IDNA2008 [
RFC5890
] differs from IDNA2003 [
RFC3490
] in
...
RFC3490
   ...
   they normatively reference.  Additionally, [
RFC5894
] provides
...
HTTP Strict Transport Security (HSTS)    November 2012
Appendix B
.  Differences between HSTS Policy and Same-Origin Policy
HSTS Policy has the following primary characteristics:
   ...
   In contrast, the Same-Origin Policy (SOP) [
RFC6454
] has the following
