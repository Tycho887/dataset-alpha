---
{
  ...
  "url": "https://tools.ietf.org/html/rfc8485",
  ...
  "date": "",
  "length": 47477,
  "crawled_at": "2026-04-23T20:57:29"
}
---
...
8
3
.  Communicating Vector Values to RPs  . . . . . . . . . . . . .
9
3.1
...
18
Appendix A
.  Vectors of Trust Default Component Value Definitions  19
A.1
.  Identity Proofing . . . . . . . . . . . . . . . . . . . .
19
A.2
...
Methods for measuring trust in digital identity transactions have
   ...
   Prior to the third edition [
SP-800-63-3
] published in 2017, NIST
   Special Publication 800-63 [
SP-800-63-2
] used a single scalar
...
RFC 8485
Vectors of Trust                October 2018
as the one proposed by NISITIR 8112 [
NISTIR-8112
].  The RP can use
...
Appendix A
.  It is expected that trust frameworks will provide
...
Section 7
.  It is anticipated that trust framework definitions will
...
identity subject of a given transaction.  While this information is
   ...
   very useful when the RP requests a login (see
Section 4
) and when
...
Section 5
) to give the components and component values context.  The
   ...
   specification's definitions (see
Appendix A
).  A different vector
...
In OpenID Connect [
OpenID
], the IdP MUST send the vector as a string
   within the "vot" (vector of trust) claim in the ID token.  The
   trustmark (see
Section 5
) that applies to this vector MUST be sent as
   ...
   management" could look like this JSON object [
RFC8259
] payload of the
   ID token.
...
Section 3.1
.  Processing and fulfillment of these requests are in the
...
In OpenID Connect [
OpenID
], the client MAY request a partial set of
   ...
   array of strings [
RFC8259
], each string identifying an acceptable set
...
https://www.rfc-editor.org/info/rfc8485
> is used as the
   trustmark to reference the values defined in
Appendix A
.
...
Vectors of Trust is meant to be a flexible and reusable framework for
   ...
   usable default set of values in
Appendix A
, it is anticipated that
   many situations will require an extension of this specification for
   their own use.

   Component categories such as those defined in
Section 2
are intended
   ...
   needed by using the registry defined in
Section 7
.  The registry
   ...
   The values of components such as those defined in
Appendix A
are
...
Section 2
, though combinations of both types
   ...
   MUST be referenced by a unique trustmark URL (see
Section 5
) to allow
...
This specification adds the following values to the "OAuth Token
   Introspection Response" registry established by [
RFC7662
].
...
The vector of trust value needs to be cryptographically protected in
   ...
   [
BCP195
].  The vector of trust value must be associated with a
...
Vectors of Trust                October 2018
Appendix A
.  Vectors of Trust Default Component Value Definitions
...
>.  All
   ...
   SHOULD define their own component values as described in
Section 6
.
