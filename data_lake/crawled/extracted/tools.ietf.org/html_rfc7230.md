---
{
  ...
  "date": "",
  "length": 203042,
  "crawled_at": "2026-04-23T20:51:39"
}
---
...
(on HTTP
   ...
   establish a tunnel, previously defined in
RFC 2817
, and defines the
...
Section 5.1
   ...
   similar to that used by Internet mail [
RFC5322
...
Section 3.1.1
   ...
   metadata (
Section 3.2
), an empty line to indicate the end of the
...
RFC1919
] or "captive portal") differs from
...
Section 2 of [RFC7231]
   ...
   the path-abempty rule of
RFC 3986
, which allows for an empty path to
...
.
   ...
   port, whether or not an HTTP server is present.  See
Section 9.1
for
...
RFC 7230
HTTP/1.1 Message Syntax and Routing         June 2014
HTTP response message, as described in
...
resource identifiers indicate the same authority (the same host
   ...
   Cookie protocol [
RFC6265
], can allow information set by one service
...
.
   ...
   by splitting on whitespace (see
Section 3.5
), since no whitespace is
...
RFC6265
      ...
      special case while processing header fields.  (See
Appendix A.2.3
of [
Kri2001
] for details.)
...
Section 4.3.6 of
   [RFC7231]
) switch to tunnel mode instead of having a message body.
...
The Transfer-Encoding header field lists the transfer coding names
   ...
   body.  Transfer codings are defined in
Section 4
.
...
Section 9.5
       ...
       (
Section 9.4
) and ought to be handled as an error.  A sender MUST
...
A trailer allows the sender to include additional fields at the end
   ...
   conditionals in
Section 5 of [RFC7231]
), authentication (e.g., see
   [
RFC7235
] and [
RFC6265
]), response control data (e.g., see
Section
7.1 of [RFC7231]
...
The "compress" coding is an adaptive Lempel-Ziv-Welch (LZW) coding
   [
Welch
] that is commonly produced by the UNIX file compression
...
The "gzip" coding is an LZ77 coding with a 32-bit Cyclic Redundancy
   ...
   program [
RFC1952
].  A recipient SHOULD consider "x-gzip" to be
   equivalent to "gzip".
4.3
.  TE
...
Section 4
), and/or the keyword "trailers".  A client MUST NOT send
...
RFC7231
   ...
   semantics.  A URI reference (
Section 2.7
) is typically used as an
...
RFC7234
] and the request can be satisfied
...
Once an inbound connection is obtained, the client sends an HTTP
   request message (
Section 3
) with a request-target derived from the
...
When making a request to a proxy, other than a CONNECT or server-wide
   ...
   "forwarding" of messages are defined in
Section 5.7
.
...
Example 2: the following message received over a TLS-secured TCP
   ...
   non-public content, or poison a cache.  See
Section 9
for security
...
HTTP does not include a request identifier for associating a given
   ...
   when one or more informational responses (1xx, see
Section 6.2 of
   [RFC7231]
...
As described in
Section 2.3
, intermediaries can serve a variety of
...
Section 4
   ...
   A proxy MUST NOT transform the payload (
Section 3.3 of [RFC7231]
) of
...
, the specific connection protocols to be
   ...
   (
Section 2.7.1
) indicates a default connection of TCP over IP, with a
...
Section 4.2.1 of [RFC7231]
   ...
   connection (see the TCP reset problem described in
Section 6.6
).
...
The following is an example response to the above hypothetical
   ...
   field (
Section 6.1
) that contains an "upgrade" connection option, in
...
Then the following are valid values for example-list (not including
   ...
   one non-empty element is required by the example-list production:
     ...
     ",   ,"
Appendix B
shows the collected ABNF for recipients after the list
...
Section 4.3
|
   | Trailer           | http     | standard |
Section 4.4
|
   | Transfer-Encoding | http     | standard |
Section 3.3.1
|
   | Upgrade           | http     | standard |
Section 6.7
|
   | Via               | http     | standard |
Section 5.7.1
...
Section
4.1 of [RFC5226]
), and MUST conform to the purpose of transfer coding
...
The "HTTP" entry in the upgrade token registry has been updated with
   ...
   |       |                      | Tokens               |             |
   +-------+----------------------+----------------------+-------------+
   | HTTP  | Hypertext Transfer   | any DIGIT.DIGIT      |
Section 2.6
|
   |       | Protocol             | (e.g, "2.0")         |             |
   +-------+----------------------+----------------------+-------------+
...
This section is meant to inform developers, information providers,
   ...
   semantics and payloads are addressed in [
RFC7231
].
9.1
.  Establishing Authority
...
hypertext, possibly aided by userinfo obfuscating the authority
   component (see
Section 2.7.1
).  User agents can reduce the impact of
   ...
   URI scheme (
Section 2.7.1
) relies on the user's local name resolution
...
Section 2.7.2
   ...
   matches the target URI's authority component (see [
RFC2818
]).
...
Because HTTP uses mostly textual, character-delimited fields, parsers
   ...
   minimum size limits on request-line (
Section 3.1.1
) and header fields
   (
Section 3.2
...
) or a request payload that is too
   large (
Section 6.5.11 of [RFC7231]
...
Response splitting (a.k.a, CRLF injection) is a common technique,
   ...
   requests to responses on persistent connections [
Klein
].  This
...
metadata header fields.  Historically, the lack of a single integrity
   ...
   between a complete and incomplete response message (
Section 3.4
) when
...
, May 1996.
   ...
   [
RFC1952
]     Deutsch, P., Gailly, J-L., Adler, M., Deutsch, L., and
...
USASCII
   ...
   [
Welch
]       Welch, T., "A Technique for High-Performance Data
                 Compression", IEEE Computer 17(6), June 1984.
11.2
.  Informative References
[
BCP115
]      Hansen, T., Hardie, T., and L. Masinter, "Guidelines
...
, September 2004.
   ...
   [
Georgiev
]    Georgiev, M., Iyengar, S., Jana, S., Anubhai, R.,
...
>.
   ...
   [
Linhart
]     Linhart, C., Klein, A., Heled, R., and S. Orrin, "HTTP
...
, March 1996.
   ...
   [
RFC1945
]     Berners-Lee, T., Fielding, R., and H. Nielsen,
...
, November 1996.
   ...
   [
RFC2068
]     Fielding, R., Gettys, J., Mogul, J., Nielsen, H., and
...
, March 2005.
   ...
   [
RFC4559
]     Jaganathan, K., Zhu, L., and J. Brezak, "SPNEGO-based
...
HTTP has been in use since 1990.  The first version, later referred
   ...
   metadata.  HTTP/1.0, as defined by [
RFC1945
], added a range of
...
The requirements that clients and servers support the Host header
   field (
Section 5.4
), report an error if it is missing from an
   HTTP/1.1 request, and accept absolute URIs (
Section 5.3
) are among
...
HTTP/1.1 introduces the Transfer-Encoding header field
   (
Section 3.3.1
).  Transfer codings need to be decoded prior to
...
Section 2.7.1
   ...
   previously, it was done in
Section 2.4 of [RFC2818]
.  Furthermore, it
   implies end-to-end security.  (
Section 2.7.2
)
...
Section 3.2
   ...
   specifically defined in the ABNF.  (
Section 3.2.3
)
...
Section 3.3.3
   ...
   The "identity" transfer coding token has been removed.  (Sections
3.3
and 4)
...
Section 4.2.2
   ...
   The segment + query components of
RFC 3986
have been used to define
   the request-target, instead of abs_path from
RFC 1808
.  The
...
Section 5.7.1
   ...
   by-hop in this specification doesn't exempt them.  (
Section 6.1
)
...
Section 6.3
   ...
   other than 101 (this was incorporated from [
RFC2817
]).  Furthermore,
   the ordering in the field value is now significant.  (
Section 6.7
)

   Empty list elements in list productions (e.g., a list header field
   containing ", ,") have been deprecated.  (
Section 7
)
