---
{
  ...
  "date": "",
  "length": 134596,
  "crawled_at": "2026-04-23T20:57:10"
}
---
...
5
1.6
.  Generalization from Current Technologies . . . . . . . . .
6
1.7
.  Scope  . . . . . . . . . . . . . . . . . . . . . . . . . .
7
...
.  Contributors . . . . . . . . . . . . . . . . . . . . . . . . .
33
Saint-Andre & Hodges         Standards Track                    [Page 2]
RFC 6125
Service Identity                  March 2011
9
.  Acknowledgements . . . . . . . . . . . . . . . . . . . . . . .
...
The visible face of the Internet largely consists of services that
   ...
   with an application service using Transport Layer Security [
TLS
] or
   Datagram Transport Layer Security [
DTLS
], it references some notion
...
Saint-Andre & Hodges         Standards Track                    [Page 3]
RFC 6125
Service Identity                  March 2011
...
.
   ...
   o  Service providers might want to first read the recommendations for
      requesting of server certificates in
Section 5
.

   o  Software implementers might want to first read the recommendations
      for verification of server identity in
Section 6
.

   The sections on terminology (
Section 1.8
), naming of application
   services (
Section 2
), document scope (
Section 1.7
...
PKIX
   ...
   Furthermore, [
PKIX
] also governs any certificate-related topic on
...
PKIX
   ...
   under
Appendix B
.  However, the procedures described here can be
...
To orient the reader, this section provides an informational overview
   ...
   o  Move toward including and checking DNS domain names via the
      ...
      dNSName.
Saint-Andre & Hodges         Standards Track                    [Page 5]
RFC 6125
Service Identity                  March 2011
...
This document attempts to generalize best practices from the many
   ...
   o  The Internet Message Access Protocol [
IMAP
] and the Post Office
      Protocol [
POP3
...
HTTP-TLS
   ...
   o  The Lightweight Directory Access Protocol [
LDAP
]; see also
      [
LDAP-AUTH
...
LDAP-TLS
   ...
   o  The Simple Mail Transfer Protocol [
SMTP
]; see also [
SMTP-AUTH
] and
...
SMTP-TLS
   ...
   o  The Extensible Messaging and Presence Protocol [
XMPP
]; see also
      [
XMPP-OLD
   ...
   o  The Network News Transfer Protocol [
NNTP
]; see also [
NNTP-TLS
]

   o  The NETCONF Configuration Protocol [
NETCONF
]; see also
      [
NETCONF-SSH
...
SYSLOG-DTLS
   ...
   o  The Session Initiation Protocol [
SIP
]; see also [
SIP-CERTS
]

   o  The Simple Network Management Protocol [
SNMP
]; see also [
SNMP-TLS
]

   o  The General Internet Signalling Transport [
GIST
]
Saint-Andre & Hodges         Standards Track                    [Page 6]
RFC 6125
Service Identity                  March 2011
...
The following topics are out of scope for this specification:
   ...
   o  Identifiers other than fully qualified DNS domain names.
      ...
      existence of private internets [
PRIVATE
], host mobility, multiple
      ...
      specification.
Saint-Andre & Hodges         Standards Track                    [Page 7]
RFC 6125
Service Identity                  March 2011
Furthermore, we focus here on application service identities, not
      ...
      document discusses Uniform Resource Identifiers [
URI
] only as a
...
X.520
   ...
   o  Security protocols other than [
TLS
], [
DTLS
], or the older Secure
      ...
      PKIX certificates at times (e.g., IPsec [
IPSEC
]), their use cases
   ...
   o  Keys or certificates employed outside the context of PKIX-based
      ...
      based on or similar to OpenPGP [
OPENPGP
], or use self-signed
      ...
      Status Protocol [
OCSP
] to check CA-issued certificates.  However,
   ...
   o  Certification authority policies, such as:
      ...
      *  What types or "classes" of certificates to issue and whether to
         ...
         character in "Extended Validation" certificates [
EV-CERTS
]).
Saint-Andre & Hodges         Standards Track                    [Page 8]
RFC 6125
Service Identity                  March 2011
*  Whether to issue certificates based on IP addresses (or some
   ...
   o  Resolution of DNS domain names.
      ...
      Naming Authority Pointer (NAPTR) DNS resource records [
NAPTR
], and
...
Saint-Andre & Hodges         Standards Track                    [Page 9]
RFC 6125
Service Identity                  March 2011
application service type:  A formal identifier for the application
      ...
      of a Uniform Resource Identifier scheme [
URI
] or a DNS SRV Service
      [
DNS-SRV
].
...
PKIX
      ...
      *  CN-ID = a Relative Distinguished Name (RDN) in the certificate
         ...
         separated letter-digit-hyphen labels); see [
PKIX
] and also
         [
LDAP-SCHEMA
...
URI
   ...
   interactive client:  A software agent or device that is directly
      ...
      security and application protocols, such as [
WSC-UI
], often refer
   ...
   pinning:  The act of establishing a cached name association between
      ...
      certificate, as described under
Section 6.6.2
.  (A similar
      definition of "pinning" is provided in [
WSC-UI
].)

   PKIX:  PKIX is a short name for the Internet Public Key
      Infrastructure using X.509 defined in
RFC 5280
[
PKIX
], which
...
Saint-Andre & Hodges         Standards Track                   [Page 11]
RFC 6125
Service Identity                  March 2011
...
PKIX
      ...
      subject field (see Section 4.1.2.6 of [
PKIX
]).
...
TLS
] negotiation.  In this specification
...
Saint-Andre & Hodges         Standards Track                   [Page 13]
RFC 6125
Service Identity                  March 2011
...
For the purposes of this specification, the name of an application
   ...
   the following forms:
Saint-Andre & Hodges         Standards Track                   [Page 14]
RFC 6125
Service Identity                  March 2011
1.  A "traditional domain name", i.e., a fully qualified DNS domain
       name or "FQDN" (see [
DNS-CONCEPTS
]) all of whose labels are "LDH
       labels" as described in [
IDNA-DEFS
].  Informally, such labels are
       constrained to [
US-ASCII
] letters, digits, and the hyphen, with
...
2.3
.  Subject Naming in PKIX Certificates
In theory, the Internet Public Key Infrastructure using X.509 [
PKIX
]
   employs the global directory service model defined in [
X.500
] and
   [
X.501
].  Under that model, information is held in a directory
   ...
   group) of attribute-type-and-value pairs (see also [
LDAP-DN
]), each
   of which asserts some attribute about the entry.

   In practice, the certificates used in [
X.509
] and [
PKIX
] borrow key
...
certificate contains a subject alternative name ("subjectAltName")
   ...
   the subject (see Section 4.2.1.6 of [
PKIX
]).  The subjectAltName
...
Saint-Andre & Hodges         Standards Track                   [Page 16]
RFC 6125
Service Identity                  March 2011
...
Confusion sometimes arises from different renderings or encodings of
   ...
   Distinguished Encoding Rules (DER) specified in [
X.690
].  However,
   ...
   [
X.509
], the same as for a Distinguished Name (DN) [
X.501
]) is an
...
LDAP-DN
   ...
   instead use the terms provided under
Section 1.8
; in particular, we
   do not use the term "(most specific) Common Name field in the subject
   field" from [
HTTP-TLS
] and instead state that a CN-ID is a Relative
...
Saint-Andre & Hodges         Standards Track                   [Page 17]
RFC 6125
Service Identity                  March 2011
3
.  Designing Application Protocols
This section provides guidelines for designers of application
   ...
   o  Does your technology use DNS SRV records to resolve the DNS domain
      ...
      means of SRV-IDs as defined in [
SRVNAME
].)
...
Saint-Andre & Hodges         Standards Track                   [Page 18]
RFC 6125
Service Identity                  March 2011
...
HTTP
       ...
       since [
HTTP-TLS
] does not describe usage of a URI-ID for HTTP
       ...
       "sip" but not "sips" or "tel" for SIP as described in [
SIP-SIPS
],
...
Section 7.4
.
Saint-Andre & Hodges         Standards Track                   [Page 19]
RFC 6125
Service Identity                  March 2011
...
EMAIL-SRV
   ...
   URI-ID of "sip:voice.example.edu" (see [
SIP-CERTS
]) along with a
   ...
   "_xmpp-server.im.example.org" (see [
XMPP
]), a DNS-ID of
   "im.example.org", and an XMPP-specific "XmppAddr" of "im.example.org"
   (see [
XMPP
]).  It might also include a CN-ID of "im.example.org" for
   backward compatibility with deployed infrastructure.
Saint-Andre & Hodges         Standards Track                   [Page 20]
RFC 6125
Service Identity                  March 2011
5
.  Requesting Server Certificates
This section provides rules and guidelines for service providers
   ...
   "pop3", "pop3s", and "submission" as described in [
EMAIL-SRV
]).
6
.  Verifying Service Identity
...
Saint-Andre & Hodges         Standards Track                   [Page 21]
RFC 6125
Service Identity                  March 2011
...
Saint-Andre & Hodges         Standards Track                   [Page 22]
RFC 6125
Service Identity                  March 2011
the data via [
DNSSEC
], or obtaining the data from a third-party
...
.  In this
   ...
   o  If a server for the application service type is typically
      ...
      include an SRV-ID.
Saint-Andre & Hodges         Standards Track                   [Page 23]
RFC 6125
Service Identity                  March 2011
o  If a server for the application service type is typically
...
A web browser that is connecting via HTTPS to the website at
   ...
   [
EMAIL-SRV
]), DNS-IDs of "example.net" and "mail.example.net", and,
   as a fallback, CN-IDs of "example.net" and "mail.example.net".  (A
Saint-Andre & Hodges         Standards Track                   [Page 24]
RFC 6125
Service Identity                  March 2011
legacy email user agent would not support [
EMAIL-SRV
] and therefore
   ...
   identifier: a URI-ID of "sip:voice.example.edu" (see [
SIP-CERTS
]).
...
Saint-Andre & Hodges         Standards Track                   [Page 25]
RFC 6125
Service Identity                  March 2011
DNS-ID of "www.example.com" would result in a DNS domain name
   ...
   o  For a reference identifier of type SRV-ID, the DNS domain name
      ...
      application protocol of IMAP as explained in [
EMAIL-SRV
]).
...
URI
      ...
      [
URI
] limits verification to DNS domain names, thereby
      ...
      the URI (as explained in [
URI
]).  As an example, a URI-ID of "sip:
...
Saint-Andre & Hodges         Standards Track                   [Page 26]
RFC 6125
Service Identity                  March 2011
defined under
Section 2.2
...
If the DNS domain name portion of a reference identifier is a
   ...
   clarified by [
DNS-CASE
] (e.g., "WWW.Example.Com" would be lower-cased
...
IDNA-DEFS
   ...
   checking the domain name.  In accordance with [
IDNA-PROTO
], A-labels
...
Saint-Andre & Hodges         Standards Track                   [Page 27]
RFC 6125
Service Identity                  March 2011
2.  If the wildcard character is the only character of the left-most
   ...
   3.  The client MAY match a presented identifier in which the wildcard
       ...
       character is embedded within an A-label or U-label [
IDNA-DEFS
] of
...
When a client checks identifiers of type SRV-ID and URI-ID, it MUST
   ...
   application service type portion (as described under
Section 6.3
),
   then checking both the DNS domain name portion (as described under
Section 6.4
) and the application service type portion as described in
...
Saint-Andre & Hodges         Standards Track                   [Page 28]
RFC 6125
Service Identity                  March 2011
...
The application service name portion of an SRV-ID (e.g., "imaps")
   ...
   [
DNS-SRV
].  Note that the "_" character is prepended to the service
   identifier in DNS SRV records and in SRV-IDs (per [
SRVNAME
]), and
...
The scheme name portion of a URI-ID (e.g., "sip") MUST be matched in
   a case-insensitive manner, in accordance with [
URI
].  Note that the
...
If the client does not find a presented identifier matching any of
   ...
   "pinning" is explained under
Section 1.8
), and the presented
   certificate matches the pinned certificate (including the context as
   described under
Section 7.1
), then the service identity check has
   succeeded.
Saint-Andre & Hodges         Standards Track                   [Page 29]
RFC 6125
Service Identity                  March 2011
6.6.3
.  Case #3: No Match Found, No Pinned Certificate
...
As defined under
Section 1.8
, a certificate is said to be "pinned" to
...
Saint-Andre & Hodges         Standards Track                   [Page 30]
RFC 6125
Service Identity                  March 2011
...
This document states that the wildcard character '*' SHOULD NOT be
   ...
   technologies (such as those excerpted under
Appendix B
).  Several
...
HTTPSbytes
   ...
   o  Specifications for existing application technologies are not clear
      ...
      *  included as all or part of more than one label (e.g.,
      ...
      algorithms.
Saint-Andre & Hodges         Standards Track                   [Page 31]
RFC 6125
Service Identity                  March 2011
o  There is no specification that defines how the wildcard character
      may be embedded within the A-labels or U-labels [
IDNA-DEFS
] of an
...
Allowing internationalized domain names can lead to the inclusion of
   ...
   for discussion, see for example [
IDNA-DEFS
].
7.4
.  Multiple Identifiers
A given application service might be addressed by multiple DNS domain
   ...
   A more recent approach, formally specified in [
TLS-EXT
], is for the
...
Saint-Andre & Hodges         Standards Track                   [Page 32]
RFC 6125
Service Identity                  March 2011
time why this specification states that they SHOULD NOT (instead of
   ...
   o  At least one significant technology community of interest
      explicitly allows multiple CN-IDs [
EV-CERTS
].
...
,
                    January 2006.
Saint-Andre & Hodges         Standards Track                   [Page 34]
RFC 6125
Service Identity                  March 2011
[
DNSSEC
...
,
   ...
   [
LDAP
]           Sermersheim, J., "Lightweight Directory Access
...
, October 2002.
   ...
   [
NETCONF
]        Enns, R., Ed., "NETCONF Configuration Protocol",
RFC 4741
, December 2006.
...
,
   ...
   [
PKIX-OLD
]       Housley, R., Ford, W., Polk, T., and D. Solo,
...
, October 2008.
   ...
   [
SMTP-AUTH
]      Siemborski, R., Ed. and A. Melnikov, Ed., "SMTP
...
, December 2002.
   ...
   [
SNMP-TLS
]       Hardaker, W., "Transport Layer Security (TLS)
...
, October 2010.
   ...
   [
SYSLOG-TLS
]     Miao, F., Ed., Ma, Y., Ed., and J. Salowey, Ed.,
...
, June 1999.
   ...
   [
WSC-UI
]         Saldhana, A. and T. Roessler, "Web Security Context:
...
, March 2011.
   ...
   [
XMPP-OLD
]       Saint-Andre, P., Ed., "Extensible Messaging and
                    Presence Protocol (XMPP): Core",
RFC 3920
,
                    October 2004.
Saint-Andre & Hodges         Standards Track                   [Page 39]
RFC 6125
Service Identity                  March 2011
Appendix A
.  Sample Text
At the time of this writing, two application technologies reuse the
   recommendations in this specification: email [
EMAIL-SRV
] and XMPP
   [
XMPP
].  Here we include the text from [
XMPP
] to illustrate the
   ...
   o  Support for the DNS-ID identifier type [
PKIX
] is REQUIRED in XMPP
   ...
   o  Support for the SRV-ID identifier type [
SRVNAME
] is REQUIRED for
   ...
   ######
Saint-Andre & Hodges         Standards Track                   [Page 40]
RFC 6125
Service Identity                  March 2011
The text regarding certificate verification is as follows:
   ...
   ######
Saint-Andre & Hodges         Standards Track                   [Page 41]
RFC 6125
Service Identity                  March 2011
Appendix B
.  Prior Art
...
Saint-Andre & Hodges         Standards Track                   [Page 42]
RFC 6125
Service Identity                  March 2011
...
Saint-Andre & Hodges         Standards Track                   [Page 43]
RFC 6125
Service Identity                  March 2011
...
Saint-Andre & Hodges         Standards Track                   [Page 44]
RFC 6125
Service Identity                  March 2011
...
-
3.1.3.3
explain how to compare values of
...
of type dNSName) or for which the mapping is performed in a secure
   manner (e.g., using [
DNSSEC
], or using user- or admin-configured
   host-to-address/address-to-host lookup tables).

   The server's identity may also be verified by comparing the reference
   identity to the Common Name (CN) [
LDAP-SCHEMA
] value in the last
...
, below, with the exception
   ...
   Encoding (ACE) format as specified in
Section 4 of RFC 3490
[
IDNA2003
] before comparison with subjectAltName values of type
...
Saint-Andre & Hodges         Standards Track                   [Page 46]
RFC 6125
Service Identity                  March 2011
...
IPv6
].  For IP Version 4, as specified in
RFC 791
, the octet
   string will contain exactly four octets.  For IP Version 6, as
   specified in
RFC 2460
, the octet string will contain exactly sixteen
...
Saint-Andre & Hodges         Standards Track                   [Page 47]
RFC 6125
Service Identity                  March 2011
...
TLS
   ...
   ######
Saint-Andre & Hodges         Standards Track                   [Page 48]
RFC 6125
Service Identity                  March 2011
B.5
.  XMPP (2004)
...
XMPP-OLD
   ...
   Case #1:  The peer contains an End Entity certificate which appears
      ...
      anchor (as described in Section 6.1 of [
PKIX
]).
   ...
   1.  Verify the peer certificate according to the rules of [
PKIX
].
       The certificate SHOULD then be checked against the expected
       identity of the peer following the rules described in [
HTTP-TLS
],
   ...
   ######
Saint-Andre & Hodges         Standards Track                   [Page 49]
RFC 6125
Service Identity                  March 2011
Although [
XMPP-OLD
...
Saint-Andre & Hodges         Standards Track                   [Page 50]
RFC 6125
Service Identity                  March 2011
...
Saint-Andre & Hodges         Standards Track                   [Page 51]
RFC 6125
Service Identity                  March 2011
...
Saint-Andre & Hodges         Standards Track                   [Page 52]
RFC 6125
Service Identity                  March 2011
######
...
PKIX
   ...
   o  Implementations MAY support matching a locally configured IP
      ...
      converted to an octet string as specified in [
PKIX
],
Section
4.2.1.6
.  A match occurs if this octet string is equal to the
      value of iPAddress in the subjectAltName extension.

   ######
Saint-Andre & Hodges         Standards Track                   [Page 53]
RFC 6125
Service Identity                  March 2011
B.9
.  SIP (2010)
...
SIP-CERTS
   ...
   as SIP domain identities:
      ...
      [
DNS-CASE
].  Implementations MUST handle Internationalized Domain
      Names (IDNs) in accordance with Section 7.2 of [
PKIX
].
      ...
      Implementations MUST match the values in their entirety:
         ...
         "foo.example.com."
            ...
            [
HTTP-TLS
] allows the dNSName component to contain a
            wildcard; e.g., "DNS:*.example.com".  [
PKIX
], while not
            disallowing this explicitly, leaves the interpretation of
            wildcards to the individual specification.  [
SIP
] does not
   ...
   ######
Saint-Andre & Hodges         Standards Track                   [Page 54]
RFC 6125
Service Identity                  March 2011
B.10
.  SNMP (2010)
...
SNMP-TLS
   ...
   validation [
PKIX
] to a configured trust anchor, and an active row
...
In 2010, [
GIST
] specified the following text regarding application
   service identity verification in the General Internet Signalling
   Transport:
Saint-Andre & Hodges         Standards Track                   [Page 55]
RFC 6125
Service Identity                  March 2011
######
   ...
   discussed in
Section 4.4.2
.  This section defines the identity
   ...
   *.example.com, etc., but not example.com.
   ...
   peer.  Nodes SHOULD implement the algorithm in Section 6 of [
PKIX
]
   ...
   ######
Saint-Andre & Hodges         Standards Track                   [Page 56]
RFC 6125
Service Identity                  March 2011
Authors' Addresses
