---
{
  ...
  "url": "https://tools.ietf.org/html/rfc7592",
  ...
  "date": "",
  "length": 37531,
  "crawled_at": "2026-04-23T20:57:24"
}
---
...
RFC 7592
OAuth 2.0 Dynamic Registration Management      July 2015
Copyright Notice
...
13
Appendix A
.  Registration Tokens and Client Credentials . . . . .
15
A.1
.  Credential Rotation . . . . . . . . . . . . . . . . . . .
16
...
In order for an OAuth 2.0 client to utilize an OAuth 2.0
   ...
   Protocol" [
RFC7591
] describes how an OAuth 2.0 client can be
...
endpoint is communicated to the client through the
   ...
   as specified in
Section 3
.  The client MUST use its registration
...
RFC6750
   ...
   layer security mechanism, as described in
Section 5
.
   ...
   HTTP methods [
RFC7231
].  If an authorization server does not support
...
To update a previously registered client's registration with an
   ...
   json".  The HTTP entity payload is a JSON [
RFC7159
] document
...
If the client attempts to set an invalid metadata field and the
   ...
   server responds with an error as described in [
RFC7591
].
2.3
.  Client Delete Request
...
This specification extends the client information response defined in
   "OAuth 2.0 Client Dynamic Registration" [
RFC7591
], which states that
   ...
   fully qualified URL of the client configuration endpoint (
Section 2
)
...
While the client secret can expire, the registration access token
   ...
   Bearer Token Usage specification [
RFC6750
].
   ...
   endpoint.  The server MUST support TLS 1.2 [
RFC5246
] and MAY support
...
BCP195
   ...
   guessing attack of this token, such as described in
Section 5.2 of
   [RFC6750]
and
...
The authorization server MUST provide the client with the fully
   ...
   Information Response, as specified in
Section 3
.  The authorization
