---
{
  ...
  "date": "",
  "length": 152590,
  "crawled_at": "2026-04-23T20:57:19"
}
---
...
RFC 7518
JSON Web Algorithms (JWA)               May 2015
Table of Contents
1
.  Introduction  . . . . . . . . . . . . . . . . . . . . . . . .
4
...
.  Introduction
This specification registers cryptographic algorithms and identifiers
   to be used with the JSON Web Signature (JWS) [
JWS
], JSON Web
   Encryption (JWE) [
JWE
...
JWK
   ...
   specifications utilize JSON-based [
RFC7159
...
The table below is the set of "alg" (algorithm) Header Parameter
   ...
   See
Appendix A.1
for a table cross-referencing the JWS digital
...
Hash-based Message Authentication Codes (HMACs) enable one to use a
   ...
   validating HMACs is provided in
RFC 2104
[
RFC2104
].
...
Section 5.3.4
(Security Effect of the HMAC
   Key) of NIST SP 800-117 [
NIST.800-107
], which states that the
   effective security strength is the minimum of the security strength
   of the key and two times the size of the internal hash value.)

   The HMAC SHA-256 MAC is generated per
RFC 2104
, using SHA-256 as the
   ...
   value per
RFC 2104
, using SHA-256 as the hash algorithm "H", using
...
RFC3447
   ...
   (commonly known as PKCS #1), using SHA-2 [
SHS
] hash functions.
...
The ECDSA P-256 SHA-256 digital signature for a JWS is validated as
   ...
   2.  Split the 64-octet sequence into two 32-octet sequences.  The
       ...
       OctetString Conversion defined in
Section 2.3.7
of SEC1 [
SEC1
]
...
JWSs MAY also be created that do not provide integrity protection.
   ...
   acceptance on a per-object basis.  See
Section 8.5
for security
...
RFC3447
   ...
   in
Appendix A.2.1 of RFC 3447
are used.  (Those default parameters
   ...
   of NIST SP 800-57 [
NIST.800-57
], which requires 112 bits of security
...
RFC3394
   ...
   the default initial value specified in
Section 2.2.3.1
of that
   document.
...
This section defines the specifics of directly performing symmetric
   ...
   Refer to the security considerations on key lifetimes in
Section 8.2
and AES GCM in
Section 8.4
when considering utilizing direct
   encryption.
4.6
...
RFC6090
   ...
   the Concat KDF, as defined in Section 5.8.1 of [
NIST.800-56A
].  The
...
The "epk" (ephemeral public key) value created by the originator for
   ...
   JSON Web Key [
JWK
] public key value.  It MUST contain only public key
...
NIST.800-56A
   ...
   a manner similar to "Diffie-Hellman Key Agreement Method" [
RFC2631
]:
   ...
   mode in
RFC 2631
) and the "apv" parameter SHOULD NOT be present.

   See
Appendix C
for an example key agreement computation using this
   method.
4.7
.  Key Encryption with AES GCM
...
Section 6.2 of [RFC2898]
, then by encrypting the JWE CEK using the
   derived key.
...
The "p2s" (PBES2 salt input) Header Parameter encodes a Salt Input
   ...
   encryption operation; see
RFC 4086
[
RFC4086
] for considerations on
...
The table below is the set of "enc" (encryption algorithm) Header
   ...
   See
Appendix A.3
for a table cross-referencing the JWE "enc"
...
This section defines a family of authenticated encryption algorithms
   built using a composition of AES [
AES
] in Cipher Block Chaining (CBC)
   mode [
NIST.800-38A
] with PKCS #7 padding operations per
Section 6.3
   of [RFC5652]
and HMAC ([
RFC2104
...
and
5.2.2.2
define the generic encryption and
   decryption algorithms.  Sections
5.2.3
through
5.2.5
define instances
...
.  The value T, from the previous step, is
       ...
       the operation halts.  (But see Section 11.5 of [
JWE
] for security
...
SHS
   ...
   [
RFC4868
].  For encryption, it uses AES in the CBC mode of operation
   as defined in Section 6.2 of [
NIST.800-38A
], with PKCS #7 padding and
   a 128-bit IV value.
...
A JSON Web Key (JWK) [
JWK
] is a JSON data structure that represents a
...
The "crv" (curve) parameter identifies the cryptographic curve used
   with the key.  Curve values from [
DSS
...
.  Additional "crv" values can be
   ...
   SEC1 [
SEC1
] point compression is not supported for any of these three
   curves.
6.2.1.2
.  "x" (X Coordinate) Parameter
...
of SEC1 [
SEC1
].  The length of this octet string
...
6.3
.  Parameters for RSA Keys
JWKs can represent RSA [
RFC3447
] keys.  In this case, the "kty"
   member value is "RSA".  The semantics of the parameters defined below
   are the same as those defined in Sections
3.1
and
3.2
of
...
The "oth" (other primes info) parameter contains an array of
   ...
   OtherPrimeInfo parameters in
Appendix A.1.2 of RFC 3447
[
RFC3447
],
...
o  Compression Algorithm Value: "DEF"
   ...
   o  Change Controller: IESG
   o  Specification Document(s): JSON Web Encryption (JWE) [
JWE
]
7.4
.  JSON Web Key Types Registry
...
7.5
.  JSON Web Key Parameters Registration
This section registers the parameter names defined in Sections
6.2
,
...
Many algorithms have associated security considerations related to
   ...
   [
NIST.800-57
] for specific guidance on key lifetimes.
8.3
.  RSAES-PKCS1-v1_5 Security Considerations
While
Section 8 of RFC 3447
[
RFC3447
] explicitly calls for people not
...
Section 3
of "Twenty Years of Attacks on the
   RSA Cryptosystem" [
Boneh99
], must not be used.
8.4
...
Keys used with AES GCM must follow the constraints in Section 8.3 of
   [
NIST.800-38D
], which states: "The total number of invocations of the
...
Receiving agents that validate signatures and sending agents that
   ...
   enforce upper limits on the key sizes they accept.
Section 5.6.1
...
preventing reuse is to always generate at least one new piece of key
   ...
   considerations noted in this document as well as from
RFC 4086
[
RFC4086
].
...
[
RFC2898
] to derive cryptographic keys from user-supplied
   passwords.
   ...
   [
NIST.800-63-2
] contains guidelines for estimating password entropy,
...
See Section 10.1 of [
JWS
] for security considerations on key entropy
   and random values.
8.10
.  Differences between Digital Signatures and MACs
See Section 10.5 of [
JWS
] for security considerations on differences
...
See Section 11.3 of [
JWE
] for security considerations on using
   matching algorithm strengths.
8.12
.  Adaptive Chosen-Ciphertext Attacks
See Section 11.4 of [
JWE
] for security considerations on adaptive
...
Passwords obtained from users are likely to require preparation and
   ...
   applications perform the steps outlined in [
PRECIS
] to prepare a
...
>.
   ...
   [
RFC4868
]  Kelly, S. and S. Frankel, "Using HMAC-SHA-256,
...
JCA
              ...
              Guide", 2014, <
http://docs.oracle.com/javase/8/docs/techno
...
RFC 2631
, DOI 10.17487/RFC2631, June 1999,
              <
...
RFC 3275
, DOI 10.17487/RFC3275, March 2002,
              <
...
.  They are
   ...
   version of [
AEAD-CBC-SHA
], demonstrating that the cryptographic
...
This example uses ECDH-ES Key Agreement and the Concat KDF to derive
   the CEK in the manner described in
Section 4.6
.  In this example, the
...
CanvasApp
   ...
   Encryption" [
JSE
], and "JavaScript Message Security Format" [
JSMS
],
   ...
   [
AEAD-CBC-SHA
] specification, upon which the AES_CBC_HMAC_SHA2
