---
{
  ...
  "date": "",
  "length": 230932,
  "crawled_at": "2026-04-23T20:51:41"
}
---
...
Section 4
   ...
   request header fields (
Section 5
), the meaning of status codes to
   indicate a machine-readable response (
Section 6
), and the meaning of
...
Section 7 of
   [RFC7230]
, that allows for compact definition of comma-separated
   lists using a '#' operator (similar to how the '*' operator indicates
   repetition).
Appendix C
describes rules imported from other
   documents.
Appendix D
...
Section 5.3 of
   [RFC7230]
).  When a request is received, the server reconstructs an
   effective request URI for the target resource (
Section 5.5 of
   [RFC7230]
).
...
Considering that a resource could be anything, and that the uniform
   ...
   abstraction is called a representation [
REST
].
...
RFC 7231
HTTP/1.1 Semantics and Content            June 2014
data and metadata for evaluating conditional requests [
RFC7232
] and
...
HTTP uses charset names to indicate or negotiate the character
   encoding scheme of a textual representation [
RFC6365
].  A charset is
...
RFC2045
   ...
   older mail transfer protocols do not apply to HTTP (see
Appendix A
).
...
MIME provides for a number of "multipart" types -- encapsulations of
   ...
   multipart types share a common syntax, as defined in
Section 5.1.1 of
   [RFC2046]
...
Section 8.4
.  They are used in the Accept-Encoding (
Section 5.3.4
)
...
The following content-coding values are defined by this
   specification:
      ...
      compress (and x-compress): See
Section 4.2.1 of [RFC7230]
.
...
Section 5.5 of [RFC7230]
   ...
   of the same name defined for MIME body parts in
Section 4 of
   [RFC2557]
.  However, its appearance in an HTTP message has some
...
Section 4.3.2
   ...
   the server.  For a state-changing request like PUT (
Section 4.3.4
) or
   POST (
Section 4.3.3
...
Section 4.3.4
   ...
   representation in the payload of a POST request (
Section 4.3.3
)
...
|
   | Content-Range     |
Section 4.2 of [RFC7233]
|
   | Trailer           |
Section 4.4 of [RFC7230]
|
   | Transfer-Encoding |
Section 3.3.1 of [RFC7230]
|
...
When content negotiation preferences are sent by the user agent in a
   ...
   negotiation fields of
Section 5.3
and implicit characteristics, such
...
Section 5
   ...
   (
Section 5.2
) to make the requested action conditional on the current
...
RFC7232
   ...
   better visibility and reuse in network-based systems [
REST
].  Once
...
+---------+-------------------------------------------------+-------+
   ...
   Registry" maintained by IANA, as defined in
Section 8.1
.
...
Section 5.1.2
).  A proxy MUST NOT generate a Max-Forwards header
...
Section 5.7.1 of
   [RFC7230]
) is of particular interest, since it acts as a trace of the
...
|
   | Host              |
Section 5.4 of [RFC7230]
|
   | Max-Forwards      |
Section 5.1.2
|
   | Pragma            |
Section 5.4 of [RFC7234]
|
   | Range             |
Section 3.1 of [RFC7233]
|
   | TE                |
Section 4.3 of [RFC7230]
|
   +-------------------+--------------------------+
...
corresponding request-line and header section to the next inbound
   ...
   the client to begin sending the message body.
      ...
      publication of HTTP/1.1 [
RFC2068
] as both the means to request an
...
5.2
.  Conditionals
The HTTP conditional request header fields [
RFC7232
] allow a client
...
this specification consists of a comparison between a set of
   ...
   (
Section 7.2
).  Hence, these preconditions evaluate whether the state
...
The following request header fields are sent by a user agent to
   ...
   in
Section 3.4.1
.  The preferences sent in these fields apply to any
...
RFC6265
   ...
   +---------------------+--------------------------+
   | Authorization       |
Section 4.2 of [RFC7235]
|
   | Proxy-Authorization |
Section 4.4 of [RFC7235]
|
   +---------------------+--------------------------+
5.5
.  Request Context
...
Section 3.2 of
   [RFC7230]
), which together identify the user agent software and its
...
Note that this list is not exhaustive -- it does not include
   ...
   list of status codes is maintained by IANA.  See
Section 8.2
for
   details.
6.2
...
HTTP-date is case sensitive.  A sender MUST NOT generate additional
   ...
   Format constructs with the corresponding name ([
RFC5322
],
Section
3.3
...
Section 5.3
   ...
   across users is constrained by the field definition (
Section 4.2 of
   [RFC7235]
).  Likewise, an origin server might use Cache-Control
   directives (
Section 5.2 of [RFC7234]
) to supplant Vary if it
...
The "Hypertext Transfer Protocol (HTTP) Method Registry" defines the
   namespace for the request method token (
Section 4
).  The method
...
Section 4
   ...
   o  Safe ("yes" or "no", see
Section 4.2.1
)

   o  Idempotent ("yes" or "no", see
Section 4.2.2
)
...
Section 5.2
   ...
   some use for partial response semantics ([
RFC7233
]), it ought to
   document this, too.
...
The "Hypertext Transfer Protocol (HTTP) Status Code Registry" defines
   the namespace for the response status-code token (
Section 6
).  The
...
>.
   ...
   Codes previously defined in
Section 7.1 of [RFC2817]
.
8.2.1
.  Procedure
...
Header fields are key:value pairs that can be used to communicate
   ...
   connection (i.e., control data).  See
Section 3.2 of [RFC7230]
for a
...
Section 7 of
   [RFC7230]
as necessary, and are usually constrained to the range of
...
The "Message Headers" registry has been updated with the following
   ...
   +-------------------+----------+----------+-----------------+
   | Accept            | http     | standard |
Section 5.3.2
|
   | Accept-Charset    | http     | standard |
Section 5.3.3
|
   | Accept-Encoding   | http     | standard |
Section 5.3.4
|
   | Accept-Language   | http     | standard |
Section 5.3.5
|
   | Allow             | http     | standard |
Section 7.4.1
|
   | Content-Encoding  | http     | standard |
Section 3.1.2.2
|
   | Content-Language  | http     | standard |
Section 3.1.3.2
|
   | Content-Location  | http     | standard |
Section 3.1.4.2
|
...
Section 7.1.1.2
|
   | Expect            | http     | standard |
Section 5.1.1
|
   | From              | http     | standard |
Section 5.5.1
|
   | Location          | http     | standard |
Section 7.1.2
|
   | Max-Forwards      | http     | standard |
Section 5.1.2
|
   | MIME-Version      | http     | standard |
Appendix A.1
|
   | Referer           | http     | standard |
Section 5.5.2
|
   | Retry-After       | http     | standard |
Section 7.1.3
|
   | Server            | http     | standard |
Section 7.4.2
|
   | User-Agent        | http     | standard |
Section 5.5.3
|
   | Vary              | http     | standard |
Section 7.1.4
...
Content coding registrations MUST include the following fields:
   ...
   codings (
Section 4 of [RFC7230]
), unless the encoding transformation
...
,
   ...
   [
OWASP
]    van der Stock, A., Ed., "A Guide to Building Secure Web
...
, November 1996.
   ...
   [
RFC2068
]  Fielding, R., Gettys, J., Mogul, J., Nielsen, H., and T.
...
, January 1997.
   ...
   [
RFC2295
]  Holtman, K. and A. Mutz, "Transparent Content Negotiation
              in HTTP",
RFC 2295
, March 1998.
...
, August 1998.
   ...
   [
RFC2557
]  Palme, F., Hopmann, A., Shelness, N., and E. Stefferud,
...
, June 1999.
   ...
   [
RFC2774
]  Frystyk, H., Leach, P., and S. Lawrence, "An HTTP
              Extension Framework",
RFC 2774
, February 2000.
...
RFC2045
   ...
   However,
RFC 2045
is focused only on email; applications of HTTP have
...
HTTP is not a MIME-compliant protocol.  However, messages can include
   ...
   conformance with the MIME protocol (as defined in [
RFC2045
]).
...
The Expect header field's extension mechanism has been removed due to
   widely-deployed broken implementations.  (
Section 5.1.1
)
...
Section 6.4
   ...
   to rewrite the method from POST to GET.  (Sections
6.4.2
and
6.4.3
)
...
Section 7.1.2
   ...
   required to always trust its value.  (
Section 7.4.1
)
...
[RFC7230], Section 2.7
>
     absolute-URI  = <absolute-URI, see
[RFC7230], Section 2.7
>
     comment       = <comment, see
[RFC7230], Section 3.2.6
>
     field-name    = <comment, see
[RFC7230], Section 3.2
>
     partial-URI   = <partial-URI, see
[RFC7230], Section 2.7
>
Fielding & Reschke           Standards Track                   [Page 93]
RFC 7231
HTTP/1.1 Semantics and Content            June 2014
quoted-string = <quoted-string, see
[RFC7230], Section 3.2.6
>
     token         = <token, see
[RFC7230], Section 3.2.6
