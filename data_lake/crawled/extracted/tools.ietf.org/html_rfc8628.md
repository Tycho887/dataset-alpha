---
{
  ...
  "date": "",
  "length": 45825,
  "crawled_at": "2026-04-23T20:51:53"
}
---
...
RFC 8628
OAuth 2.0 Device Grant              August 2019
Copyright Notice
...
This OAuth 2.0 [
RFC6749
] protocol extension enables OAuth clients to
...
The client makes a device authorization request to the device
   ...
   the "application/x-www-form-urlencoded" format, per
Appendix B of
   [RFC6749]
...
.
   ...
   (TLS) protocol [
RFC8446
] and implement the best practices of
BCP 195
[
RFC7525
].
...
Section 3.4
), care is needed to avoid overloading the capacity of the
...
After receiving a successful authorization response, the client
   ...
   [
RFC8446
] session.  The authorization server prompts the end user to
...
When "verification_uri_complete" is included in the authorization
   response (
Section 3.2
), clients MAY present this URI in a non-textual
...
Section 3.3
is still followed, with the optimization that the user
...
If the user has approved the grant, the token endpoint responds with
   a success response defined in
Section 5.1 of [RFC6749]
; otherwise, it
   responds with an error, as defined in
Section 5.2 of [RFC6749]
.
...
It is possible for the device flow to be initiated on a device in an
   ...
   interaction step (see
Section 3.3
) and to confirm that the device is
   ...
   "verification_uri_complete" optimization discussed in
Section 3.3.1
,
...
draft-recordon-oauth-v2-device
, authored by David Recordon and Brent
