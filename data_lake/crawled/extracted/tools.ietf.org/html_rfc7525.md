---
{
  ...
  "date": "",
  "length": 59400,
  "crawled_at": "2026-04-23T20:57:20"
}
---
...
RFC5246
   ...
   Security Layer (DTLS) [
RFC6347
] are widely used to protect data
...
RFC7465
] encryption algorithms, which together
   have been the most widely deployed ciphers, have been attacked in the
   context of TLS.  A companion document [
RFC7457
] provides detailed
   ...
   be deployed in environments defined in
Section 5
.  In fact, this
   ...
   exception of unauthenticated TLS (see
Section 5
).  Other
...
RFC6176
   ...
   o  Implementations MUST NOT negotiate SSL version 3.
      ...
      Rationale: SSLv3 [
RFC6101
] was an improvement over SSLv2 and
      ...
      which (e.g., renegotiation_info [
RFC5746
]) are security-critical.
      In addition, with the emergence of the POODLE attack [
POODLE
],
...
RFC 7525
TLS Recommendations                  May 2015
o  Implementations SHOULD NOT negotiate TLS version 1.0 [
RFC2246
];
...
RFC4346
   ...
   o  Implementations MUST support TLS 1.2 [
RFC5246
] and MUST prefer to
...
DTLS, an adaptation of TLS for UDP datagrams, was introduced when TLS
   ...
   o  Implementations SHOULD NOT negotiate DTLS version 1.0 [
RFC4347
].
...
Clients that "fall back" to lower versions of the protocol after the
   ...
   attacks has been defined recently in [
RFC7507
].)
3.2
.  Strict TLS
The following recommendations are provided to help prevent SSL
   Stripping (an attack that is summarized in
Section 2.1 of [RFC7457]
):
...
RFC5077
   ...
   o  Ticket keys MUST be changed regularly, e.g., once every week, so
      as not to negate the benefits of forward secrecy (see
Section 6.3
for details on forward secrecy).
   ...
   (
Section 4
) recommends the use of cipher suites that provide forward
...
RFC5746
   ...
   certificates received over a connection.  The [
triple-handshake
]
...
RFC7465
   ...
   o  Implementations MUST NOT negotiate cipher suites offering less
      ...
      Rationale: Based on [
RFC3766
], at least 112 bits of security is
...
o  Implementations SHOULD NOT negotiate cipher suites based on RSA
   ...
   o  Implementations MUST support and prefer to negotiate cipher suites
      ...
      which attacks can be successful.  See
Section 6.3
for a detailed
      discussion.
4.2
...
RFC5116
   ...
   to be explicitly configured in server software.  (See [
BETTERCRYPTO
]
...
Clients SHOULD include TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256 as the
   ...
   suite(s) prescribed by TLS.  To maximize interoperability,
RFC 5246
mandates implementation of the TLS_RSA_WITH_AES_128_CBC_SHA cipher
   ...
   [
Krawczyk2001
], since it uses an AEAD mode of operation.)
   ...
   example, [
RFC6460
] defines a profile that uses the
...
RFC4492
].  For interoperability, clients
   and servers SHOULD support the NIST P-256 (secp256r1) curve
   [
RFC4492
].  In addition, clients SHOULD send an ec_point_formats
...
Rationale: For various reasons, in practice, DH keys are typically
   ...
   [
RFC3766
], it is better to use keys longer than that for the "DHE"
   family of cipher suites.  A DH key of 1926 bits would be roughly
   equivalent to a 100-bit symmetric key [
RFC3766
] and a DH key of 2048
...
for additional information on the
   ...
   As noted in [
RFC3766
], correcting for the emergence of a TWIRL
   ...
   the "Transport Layer Security (TLS) Parameters" registry [
IANA-TLS
])
   contains 160-bit elliptic curves that are considered to be roughly
   equivalent to only an 80-bit symmetric key [
ECRYPT-II
].  Curves of
   ...
   of the SHA-256 hash algorithm is RECOMMENDED (see [
CAB-Baseline
] for
...
Section 4.2
.  Some implementations are severely limited in the length
   ...
   1.  Elliptic Curve DHE with appropriately negotiated parameters
       ...
       (MAC) algorithm stronger than HMAC-SHA1 [
RFC5289
]
...
.
   ...
   suites.  Its use has been shown to be insecure in [
PatersonRS11
].
...
RFC6120
   ...
   Standards Process [
RFC2026
] are expected at minimum to conform to the
...
o  Authentication: an endpoint of the TLS communication is
   ...
   security [
RFC7435
], TLS is sometimes used without authentication.  As
   discussed in
Section 5.2
, considerations for opportunistic security
...
RFC7435
] and is often motivated by a desire
...
RFC6125
   ...
   NOT be used as a reference identifier [
RFC6125
] even when it matches
   the presented certificate.  This proviso does not apply if the host
   name is discovered securely (for further discussion, see [
DANE-SRV
]
   and [
DANE-SMTP
   ...
   certification path in accordance with [
RFC5280
] (see also [
RFC6125
]).
...
.  AES-GCM
Section 4.2
above recommends the use of the AES-GCM authenticated
   encryption algorithm.  Please refer to
Section 11 of [RFC5246]
for
   general security considerations when using TLS 1.2, and to
Section 6
   of [RFC5288]
...
Forward secrecy (also called "perfect forward secrecy" or "PFS" and
   defined in [
RFC4949
]) is a defense against an attacker who records
   ...
   o  A long-term key used on a device as a default key [
Heninger2012
].
...
For performance reasons, many TLS implementations reuse Diffie-
   ...
   o  TLS implementations that reuse exponents should test the DH public
      ...
      time of writing.  See [
RFC6989
] for recipient tests required of
...
RFC 7525
TLS Recommendations                  May 2015
o  The On-Line Certification Status Protocol (OCSP) [
RFC6960
]
   ...
   o  The TLS Certificate Status Request extension (
Section 8 of
      [RFC6066]
), commonly called "OCSP stapling", resolves the
   ...
   o  OCSP stapling as defined in [
RFC6066
] does not extend to
      intermediate certificates used in a certificate chain.  Although
      the Multiple Certificate Status extension [
RFC6961
] addresses this
...
RFC6960
   ...
   2.  Both the status_request extension defined in [
RFC6066
] and the
       status_request_v2 extension defined in [
RFC6961
] (This might
...
RFC6961
   ...
   the DANE-TLSA resource record [
RFC6698
] is used to signal to a client
...
DANE-SMTP
              ...
              opportunistic DANE TLS", Work in Progress,
draft-ietf-
...
>.
   ...
   [
POODLE
]   US-CERT, "SSL 3.0 Protocol Vulnerability and POODLE
...
>.
   ...
   [
RFC6460
]  Salter, M. and R. Housley, "Suite B Profile for Transport
              Layer Security (TLS)",
RFC 6460
, January 2012,
              <
...
SESSION-HASH
              ...
              in Progress,
draft-ietf-tls-session-hash-05
, April 2015.
...
Acknowledgments
   ...
   Offered by Browsers" [
Smith2013
].  Finally, thanks to all others who
