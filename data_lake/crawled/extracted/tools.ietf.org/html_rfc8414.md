---
{
  ...
  "date": "",
  "length": 53165,
  "crawled_at": "2026-04-23T20:51:48"
}
---
...
4
2.1
. Signed Authorization Server Metadata .......................
8
3
...
OpenID.Discovery
   ...
   that "OAuth 2.0 Dynamic Client Registration Protocol" [
RFC7591
]
...
OpenID.Registration
   ...
   known location as a JSON [
RFC8259
] document, which declares its
...
This metadata can be communicated either in a self-asserted fashion
   ...
   represented as claims in a JSON Web Token (JWT) [
JWT
].  In the JWT
...
RFC7591
   ...
   discovered, for instance, through the use of WebFinger [
RFC7033
], as
   described in
Section 2
...
Authorization servers can have metadata describing their
   ...
   Authorization Server Metadata" registry established in
Section 7.1
:
   ...
   issuer
      ...
      location that is ".well-known" according to
RFC 5785
[
RFC5785
]
      derived from this issuer identifier, as described in
Section 3
.
...
RFC6749
   ...
   jwks_uri
      OPTIONAL.  URL of the authorization server's JWK Set [
JWK
]
...
RFC7591
   ...
   response_modes_supported
      ...
      [
OAuth.Responses
].  If omitted, the default is "["query",
      "fragment"]".  The response mode value "form_post" is also defined
      in "OAuth 2.0 Form Post Response Mode" [
OAuth.Post
].
...
does not support Dynamic Client Registration, then information on
   ...
   ui_locales_supported
      ...
      represented as a JSON array of language tag values from
BCP 47
[
RFC5646
].  If omitted, the set of supported languages and scripts
      is unspecified.
...
JWT
] used to authenticate the client at
...
RFC7662
   ...
   introspection_endpoint_auth_methods_supported
      ...
      [
IANA.OAuth.Parameters
] or those registered in the IANA "OAuth
      Access Token Types" registry [
IANA.OAuth.Parameters
].  (These
      values are and will remain distinct, due to
Section 7.2
.)  If
...
In addition to JSON elements, metadata values MAY also be provided as
   a "signed_metadata" value, which is a JSON Web Token (JWT) [
JWT
] that
...
Authorization servers supporting metadata MUST make a JSON document
   containing metadata as specified in
Section 2
available at a path
...
[
RFC5785
].  The well-known URI suffix used MUST
...
URI string "/.well-known/oauth-authorization-server", which is the
   ...
   "openid-configuration".  As described in
Section 5
, despite the
...
The response is a set of claims about the authorization server's
   ...
   subset of the metadata values defined in
Section 2
.  Other claims MAY
   also be returned.
...
USA15
] MUST NOT be applied at any point to
...
The identifiers "/.well-known/openid-configuration", "op_policy_uri",
   ...
   [
OpenID.Core
] family of specifications that were originally defined
   by "OpenID Connect Discovery 1.0" [
OpenID.Discovery
].  Despite the
   ...
   authorization server metadata location defined in
Section 3
is
   equivalent to the corresponding transformation defined in
Section 4
of "OpenID Connect Discovery 1.0" [
OpenID.Discovery
], provided that
...
TLS certificate checking MUST be performed by the client, as
   described in
Section 6.1
, when making an authorization server
...
RFC7033
              ...
              "WebFinger",
RFC 7033
, DOI 10.17487/RFC7033, September
              2013, <
...
>.
   ...
   [
USA15
]    Davis, M., Ed. and K. Whistler, Ed., "Unicode
