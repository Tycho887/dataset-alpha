---
{
  ...
  "date": "",
  "length": 217437,
  "crawled_at": "2026-04-23T20:54:37"
}
---
...
PKCS1
   ...
   discussed in [
PKCS1
], the DigestInfo MUST be DER-encoded [
X680
]
   [
X690
...
struct {
   ...
   The MAC is generated as described in
Section 6.2.3.1
.
...
appropriate to use the partially implicit nonce technique described
   in Section 3.2.1 of [
AEAD
]; with record_iv_length being the length of
   ...
   in
Section 6.3
), and the explicit part is included in
...
The Record Protocol requires an algorithm to generate keys required
   by the current connection state (see
Appendix A.6
) from the security
...
The master secret is expanded into a sequence of secure bytes, which
   ...
   client write IV and a server write IV (see
Section 6.2.3.3
).
...
session identifier
   ...
   peer certificate
      X509v3 [
PKIX
] certificate of the peer.  This element of the state
      may be null.
...
insufficient_security
   ...
   New Alert values are assigned by IANA as described in
Section 12
.
7.3
.  Handshake Protocol Overview
...
The extension format is:
   ...
   [
TLSEXT
].  The list of extension types is maintained by IANA as
   described in
Section 12
.
...
enum {
   ...
   hash
      ...
      values indicate support for unhashed data, MD5 [
MD5
], SHA-1,
...
and
7.4.3
describe the appropriate rules.
...
Structure of this message:
   ...
   Note: PKCS #7 [
PKCS7
] is not used as the format for the certificate
   vector because PKCS #6 [
PKCS6
] extended certificates are not used.
...
Meaning of this message:
   ...
   Structure of this message:
      ...
      enum { dhe_dss, dhe_rsa, dh_anon, rsa, dh_dss, dh_rsa
            /* may be extended, e.g., for ECDH -- see [
TLSECC
] */
...
TLSECC
   ...
   may be used with any key.  Currently, DSA [
DSS
] may only be used with
   SHA-1.  Future revisions of DSS [
DSS-3
] are expected to allow the use
...
digest algorithms should be used with each key size.  In addition,
   future revisions of [
PKIX
] may specify mechanisms for certificates to
...
-  The certificates MUST be signed using an acceptable hash/
      signature algorithm pair, as described in
Section 7.4.4
.  Note
...
BLEI
   ...
   [
KPR03
] can be used to attack a TLS server that reveals whether a
   particular message, when decrypted, is properly PKCS#1 formatted,
   contains a valid PreMasterSecret structure, or has the correct
   version number.

   As described by Klima [
KPR03
...
1. Generate a string R of 48 random bytes
   ...
   Klima et al. [
KPR03
] describe some theoretical attacks, and therefore
   ...
   The RSAES-OAEP encryption scheme defined in [
PKCS1
] is more secure
...
When this message will be sent:
   ...
   Structure of this message:
      ...
      Handshake structures (as defined in
Section 7.4
) exchanged thus
      ...
      hashes may be used with any key.  Currently, DSA [
DSS
] may only be
      used with SHA-1.  Future revisions of DSS [
DSS-3
] are expected to
...
When this message will be sent:
   ...
   Structure of this message:
      ...
      defined in
Section 5
, the Hash MUST be the Hash used as the basis
...
, exchanged thus far.
   ...
   may be different from handshake_messages in
Section 7.4.8
because it
...
In the absence of an application profile standard specifying
   ...
   suite TLS_RSA_WITH_AES_128_CBC_SHA (see
Appendix A.5
for the
   definition).
10
...
This document uses several registries that were originally created in
   [
TLS1.1
].  IANA has updated these to reference this document.  The
...
-  TLS ClientCertificateType Identifiers Registry: Future values in
      ...
      Action [
RFC2434
].  Values in the range 64-223 (decimal) inclusive
      are assigned via Specification Required [
RFC2434
].  Values from
...
RFC2434
   ...
   -  TLS Cipher Suite Registry: Future values with the first byte in
      ...
      Action [
RFC2434
].  Values with the first byte in the range 192-254
...
RFC2434
   ...
   -  This document defines several new HMAC-SHA256-based cipher suites,
      whose values (in
Appendix A.5
) have been allocated from the TLS
...
RFC4366
   ...
   allocation policy (unchanged from [
RFC4366
]) is listed below:
...
Section 7.4.1.4
   ...
   -  TLS SignatureAlgorithm Registry: The registry has been initially
      populated with the values described in
Section 7.4.1.4.1
.  Future
...
RFC2434
      ...
      Registry, defined in [
RFC3749
].  IANA has allocated value 0 for
...
[
TLSECC
] adds Elliptic Curve cipher suites to TLS.  This
   document changes some of the structures used in that document.  This
   section details the required changes for implementors of both
RFC
4492
and TLS 1.2.  Implementors of TLS 1.2 who are not implementing
RFC 4492
do not need to read this section.
   ...
   restrictions imposed by future revisions of [
PKIX
].
...
of
RFC 4492
are also relaxed.  As in this document,
...
block cipher
   ...
   Data Encryption Standard
      DES [
DES
] still is a very widely used symmetric encryption
...
of protection.  (The low bit of each key byte is presumed to be
      ...
      operated in a mode [
3DES
] where three independent keys and three
...
DSS-3
   ...
   MD5
      MD5 [
MD5
] is a hashing function that converts an arbitrarily long
...
SHA-256
   ...
   SSL
      Netscape's Secure Socket Layer protocol [
SSL3
].  TLS is based on
      SSL Version 3.0.
...
TLS requires a cryptographically secure pseudorandom number generator
   ...
   [
RANDOM
] provides guidance on the generation of random values.
D.2
.  Certificates and Authentication
...
Section 6.2.1
   ...
   -  Do you ignore the TLS record layer version number in all TLS
      records before ServerHello (see
Appendix E.1
)?
...
FI06
   ...
   -  When using Diffie-Hellman key exchange, do you correctly strip
      leading zero bytes from the negotiated key (see
Section 8.1.2
)?
...
TLS 1.2 clients that wish to support SSL 2.0 servers MUST send
   version 2.0 CLIENT-HELLO messages defined in [
SSL2
].  The message
...
TLS supports three authentication modes: authentication of both
   ...
   master_secret (see
Section 8.1
).  The master_secret is required to
   generate the Finished messages, encryption keys, and MAC keys (see
   Sections
7.4.9
and
6.3
).  By sending a correct Finished message,
...
With RSA, key exchange and server authentication are combined.  The
   ...
   the certificate verify message (see
Section 7.4.8
).  The client signs
...
[
CBCATT
] describes a chosen plaintext attack on TLS that depends on
   knowing the IV for a record.  Previous versions of TLS [
TLS1.0
] used
...
TLS secures transmitted application data via the use of symmetric
   ...
   plays an important role for achieving this goal [
ENCAUTH
].
...
TLS is susceptible to a number of denial-of-service (DoS) attacks.
   ...
   proper TCP SYN randomization is used [
SEQNUM
] by the TCP stack.
...
3DES
   ...
   [
DSS
]      NIST FIPS PUB 186-2, "Digital Signature Standard",
...
SCH
   ...
   [
SHS
]      NIST FIPS PUB 180-2, "Secure Hash Standard", National
...
BLEI
   ...
   [
CBCATT
]   Moeller, B., "Security of CBC Ciphersuites in SSL/TLS:
...
[
DSS-3
]    NIST FIPS PUB 186-3 Draft, "Digital Signature Standard",
              National Institute of Standards and Technology, U.S.
...
, December 2005.
   ...
   [
FI06
]     Hal Finney, "Bleichenbacher's RSA signature forgery based
...
, April 2004.
   ...
   [
KPR03
]    Klima, V., Pokorny, O., Rosa, T., "Attacking RSA-based
              Sessions in SSL/TLS",
...
, May 2003.
   ...
   [
PKCS6
]    RSA Laboratories, "PKCS #6: RSA Extended Certificate
              Syntax Standard", version 1.5, November 1993.

   [
PKCS7
]    RSA Laboratories, "PKCS #7: RSA Cryptographic Message
              Syntax Standard", version 1.5, November 1993.
...
, May 1996.
   ...
   [
SSL2
]     Hickman, Kipp, "The SSL Protocol", Netscape Communications
              Corp., Feb 9, 1995.

   [
SSL3
]     A. Freier, P. Karlton, and P. Kocher, "The SSL 3.0
...
, September 1981.
   ...
   [
TIMING
]   Boneh, D., Brumley, D., "Remote timing attacks are
...
, May 2006.
   ...
   [
TLSEXT
]   Eastlake, D., 3rd, "Transport Layer Security (TLS)
...
[
TLSPGP
]   Mavrogiannopoulos, N., "Using OpenPGP Keys for Transport
