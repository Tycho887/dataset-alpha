---
{
  ...
  "date": "",
  "length": 139082,
  "crawled_at": "2026-04-23T20:56:57"
}
---
...
A Uniform Resource Identifier (URI) provides a simple and extensible
   ...
   in WWW" [
RFC1630
].  The syntax is designed to meet the
...
RFC2396
], which merged "Uniform Resource
   Locators" [
RFC1738
] and "Relative Uniform Resource Locators"
   [
RFC1808
] in order to define a single, generic syntax for all URIs.
   It obsoletes [
RFC2732
], which introduced syntax for an IPv6 address.
   It excludes portions of
RFC 1738
that defined the specific syntax of
...
BCP35
].  Advice for designers of new URI
   schemes can be found in [
RFC2718
].  All significant changes from
RFC
2396
are noted in
Appendix D
.
   ...
   set" in accordance with the definitions provided in [
BCP19
], and
   "character encoding" in place of what [
BCP19
] refers to as a
   "charset".
1.1
...
Resource
   ...
   matching the syntax rule named <URI> in
Section 3
.  It enables
   uniform identification of resources via a separately defined
   extensible set of naming schemes (
Section 3.1
).  How that
...
The URI syntax has been designed with global transcription as one of
   ...
   octets (
Section 2.1
) may be used within a URI to represent characters
...
presented in
Section 5
, defines how such a reference is transformed
   ...
   (complete path segments of "." or "..", as described in
Section 3.3
),
...
This specification uses the Augmented Backus-Naur Form (ABNF)
   notation of [
RFC2234
], including the following core ABNF syntax rules
...
A percent-encoding mechanism is used to represent a data octet in a
   ...
   character (SP).
Section 2.4
describes when percent-encoding and
   decoding is applied.
...
reserved    = gen-delims / sub-delims
   ...
   delimiters of the generic URI components described in
Section 3
.  A
...
production and transmission of URIs: local name and data encoding,
   ...
   character translation of textual identifiers to UTF-8 [
STD63
] (or
...
UCS
   ...
   character encoding [
STD63
]; then only those octets that do not
...
The generic URI syntax consists of a hierarchical sequence of
   ...
   rules for a path (
Section 3.3
), only one of which will match any
   given URI reference.
...
RFC2718
   ...
   also match the <absolute-URI> grammar, as described in
Section 4.3
.
...
A 128-bit IPv6 address is divided into eight 16-bit pieces.  Each
   ...
   range 0 to 255, separated by "."), as described in [
RFC1123
] by
   reference to [
RFC0952
].  Note that other forms of dotted notation may
   be interpreted on some platforms, as described in
Section 7.4
, but
...
STD63
   ...
   encoding [
RFC3490
] prior to name lookup.  URI producers should
...
A URI-reference is either a URI or a relative reference.  If the
   ...
   familiar with regular expressions should see
Appendix B
for an
...
A relative reference takes advantage of the hierarchical syntax
   (
Section 1.2.3
) to express a URI reference relative to the name space
...
This section defines the process of resolving a URI reference within
   ...
   string matching the <URI> syntax rule of
Section 3
.
5.1
.  Establishing a Base URI
The term "relative" implies that a "base URI" exists against which
   ...
   references (
Section 4.4
), relative references are only usable when a
   base URI is known.  A base URI must be established by the parser
   prior to parsing URI references that might be relative.  A base URI
   must conform to the <absolute-URI> syntax rule (
Section 4.3
).  If the
...
This section describes an algorithm for converting a URI reference
   ...
   described in
Section 5.3
, to form the target URI.  This algorithm
...
Section 5.1
and parsed into the five main components described in
Section 3
...
Similarly, parsers must remove the dot-segments "." and ".." when
   ...
   [
RFC1630
].  Its use should be avoided but is allowed for backward
   compatibility.
...
For all URIs, the hexadecimal digits within a percent-encoding
   ...
   defined otherwise by the scheme (see
Section 6.2.3
).
6.2.2.2
...
Section 2.1
   ...
   to an unreserved character, as described in
Section 2.3
.
...
ftp://cnn.example.com&story=breaking_news@10.0.0.1/top_story.htm
   ...
   information on URI-based semantic attacks can be found in [
Siedzik
].
8
.  IANA Considerations
URI scheme names, as defined by <scheme> in
Section 3.1
, form a
   registered namespace that is managed by IANA according to the
   procedures defined in [
BCP35
].  No IANA actions are required by this
   document.
9
.  Acknowledgements
...
, June 1994.
   ...
   [
RFC1736
]  Kunze, J., "Functional Recommendations for Internet
              Resource Locators",
RFC 1736
, February 1995.
...
, December 1999.
   ...
   [
RFC3305
]  Mealling, M. and R. Denenberg, "Report from the Joint
...
, April 2003.
   ...
   [
Siedzik
]  Siedzik, R., "Semantic Attacks: What's in a URL?",
              April 2001, <
...
results in the following subexpression matches:
   ...
   its components by using the algorithm of
Section 5.3
.
Appendix C
.  Delimiting a URI in Context
...
RFC3513
] for definition of an IPv6 literal
   address, which, unfortunately, lacks an ABNF description of
   IPv6address, we created a new ABNF rule for IPv6address that matches
   the text representations defined by
Section 2.2 of [RFC3513]
.
   Likewise, the definition of IPv4address has been improved in order to
   limit each decimal octet to the range 0-255.
Section 6
, on URI normalization and comparison, has been completely
   rewritten and extended by using input from Tim Bray and discussion
   within the W3C Technical Architecture Group.
D.2
.  Modifications
The ad-hoc BNF syntax of
RFC 2396
has been replaced with the ABNF of
   [
RFC2234
...
Section 2
, on characters, has been rewritten to explain what
...
corresponding to unreserved characters.  In general, the terms
   ...
   [
RFC2518
] and with the "about:" scheme used internally by many WWW
...
o
[RFC2396] section 5.2
, step 6a, failed to account for a base URI
      with no path.

   o  Restored the behavior of [
RFC1808
] where, if the reference
   ...
   o  The determination of whether a URI reference is a same-document
      ...
      in
RFC 2396
, especially when normalization is used to reduce
