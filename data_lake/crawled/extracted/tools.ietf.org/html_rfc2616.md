---
{
  ...
  "date": "",
  "length": 412748,
  "crawled_at": "2026-04-23T20:51:35"
}
---
...
166
19.4
Differences Between HTTP Entities and
RFC 2045
Entities ....
167
...
47
   ...
   provided by the Uniform Resource Identifier (URI) [
3
], as a location
   (URL) [
4
...
20
], for indicating the resource to which a
...
9
] as defined by the Multipurpose
...
2
   ...
   and WAIS [
10
] protocols. In this way, HTTP allows basic hypermedia
...
This specification uses a number of terms to refer to the roles
   ...
   message
      ...
      sequence of octets matching the syntax defined in
section 4
and
...
19
], but other ports can be used. This does
...
[
9
]. Implementors will need to be familiar with the
...
OCTET          = <any 8-bit sequence of data>
   ...
   is defined by its associated media type, as described in
section 3.7
.
   ...
   8859-1 [
22
] only when encoded according to the rules of
RFC 2047
[
14
].
...
3
   ...
   combination of Uniform Resource Locators (URL) [
4
] and Names (URN)
   [
20
...
URIs in HTTP can be represented in absolute form or relative to some
   known base URI [
11
], depending upon the context of their use. The two
...
section 5.1.2
   ...
   in URLs SHOULD be avoided whenever possible (see
RFC 1900
[
24
]). If
...
Sun Nov  6 08:49:37 1994       ; ANSI C's asctime() format
   ...
   a fixed-length subset of that defined by
RFC 1123
[
8
] (an update to
...
[
9
]). The second format is in common use, but is based on the
   obsolete
RFC 850
[
12
] date format and lacks a four-digit year.
   HTTP/1.1 clients and servers that parse the date value MUST accept
   all three formats (for compatibility with HTTP/1.0), though they MUST
   only generate the
RFC 1123
format for representing HTTP-date values
   in header fields. See
section 19.3
...
-date | asctime-date
rfc1123
-date = wkday "," SP date1 SP time SP "GMT"
rfc850
-date  = weekday "," SP date2 SP time SP "GMT"
...
19
] MUST represent the character set defined
...
section 4.4
   ...
   values of MIME [
7
], which were designed to enable safe transport of
...
section 14.1
) header fields in order to provide
...
Note that some older HTTP applications do not recognize media type
   ...
   Authority (IANA [
19
]). The media type registration process is
   outlined in
RFC 1590
[
17
]. Use of non-registered media types is
   discouraged.
3.7.1
Canonicalization and Text Defaults
Internet media types are registered with a canonical form. An
   ...
   character set (
section 3.4
) of the data. When no explicit charset
...
40
   ...
   Unlike in
RFC 2046
, the epilogue of any multipart message MUST be
...
[
1
]. In summary, a language tag is composed of 1
...
section 14.26
   ...
   If-Range (
section 14.27
) header fields. The definition of how they
   are used and compared as cache validators is in
section 13.3.3
. An
...
section 6
) messages use the generic
   message format of
RFC 822
[
9
] for transferring entities (the payload
...
section 6.2
   ...
   entity-header (
section 7.1
) fields, follow the same generic format as
   that given in
...
entity, and thus MAY be added or removed by any application along the
   request/response chain. (However,
section 3.6
...
The transfer-length of a message is the length of the message-body as
   ...
   2.If a Transfer-Encoding header field (
section 14.41
) is present and
...
section 3.6
   ...
   3.If a Content-Length header field (
section 14.13
) is present, its
...
section 9.9
   ...
   path of the URI MUST be transmitted (see
section 3.2.1
, abs_path) as
...
30
   ...
   measurements of actual HTTP/1.1 (
RFC 2068
) implementations show good
   results [
39
]. Alternatives have also been explored, for example,
   T/TCP [
27
].
...
A significant difference between HTTP/1.1 and earlier versions of
   ...
   place using the Connection header field (
section 14.10
). Once a close
...
An HTTP/1.1 server MAY assume that a HTTP/1.1 client intends to
   ...
   signaled. See
section 19.6.2
for more information on backward
...
.
   ...
   with an HTTP/1.0 client (but see
RFC 2068
[
33
] for information and
...
8.2.3
Use of the 100 (Continue) Status
The purpose of the 100 (Continue) status (see
section 10.1.1
) is to
...
- If the proxy knows that the version of the next-hop server is
      ...
      - A proxy MUST NOT forward a 100 (Continue) response if the
        ...
        general rule for forwarding of 1xx responses (see
section 10.1
).
8.2.4
...
The server understands and is willing to comply with the client's
   request, via the Upgrade message header field (
section 14.42
), for a
...
section 14.27
   ...
   The response MUST include the following header fields:
      ...
      - Either a Content-Range header field (
section 14.16
) indicating
...
The requested resource MUST be accessed through the proxy given by
   ...
   proxy. 305 responses MUST only be generated by origin servers.
      ...
      Note:
RFC 2068
was not clear that 305 was intended to redirect a
...
section 14.43
   ...
   driven negotiation. See
section 13.6
for use of the Vary header field
   by caches and
section 14.44
for use of the Vary header field by
   servers.
12.2
Agent-driven Negotiation
...
HTTP is typically used for distributed information systems, where
   ...
   "expiration" mechanism for this purpose (see
section 13.2
). The
   latter reduces network bandwidth requirements; we use a "validation"
   mechanism for this purpose (see
section 13.3
).
...
section 13.2
         ...
         of the client, origin server, and cache (see
section 14.9
); if
         ...
         header (see
section 13.1.5
and 14.46), unless such a response
...
Whenever a cache returns a response that is neither first-hand nor
   "fresh enough" (in the sense of condition 2 in
section 13.1.1
), it
   ...
   in
section 14.46
. The warning allows clients to take appropriate
   action.
...
for a more restrictive way to force revalidation.
   ...
   revalidate" cache-control directive (see
section 14.9
).
   ...
   See
section 13.13
for an explanation of the difference between caches
...
; this section describes how to calculate
   ...
   use NTP [
28
] or some similar protocol to synchronize their clocks to
...
section 14.9.3
   ...
   maxage (see
section 14.9.3
) appears in the response, and the response
...
When a cache has a stale entry that it would like to use as a
   ...
   for the entity, and, if they match (see
section 13.3.3
), it responds
...
The ETag response-header field value, an entity tag, provides for an
   ...
   Entity Tags are described in
section 3.11
. The headers used with
   entity tags are described in sections
14.19
,
14.24
,
...
For the purpose of defining the behavior of caches and non-caching
   ...
   (
section 14.10
) to be introduced into HTTP/1.1 (or later).
13.5.2
Non-modifiable Headers
...
. A transparent proxy MUST
   preserve the entity-length (
section 7.2.2
) of the entity-body,
   although it MAY change the transfer-length (
section 4.4
).
...
section
14.46
) MUST be deleted from the cache entry and the forwarded
        response.
...
The Accept-Charset request-header field can be used to indicate what
   ...
   Character set values are described in
section 3.4
. Each charset MAY
...
1#( codings [ ";" "q" "=" qvalue ] )
   ...
   an Accept-Encoding field, using these rules:
      ...
      1. If the content-coding is one of the content-codings listed in
         ...
         accompanied by a qvalue of 0. (As defined in
section 3.9
, a
...
summarizes these defaults
   ...
   public
      ...
      shared cache. (See also Authorization,
section 14.8
, for
      additional details.)
...
The expiration time of an entity MAY be specified by the origin
   server using the Expires header (see
section 14.21
). Alternatively,
...
imaging, scientific data analysis and those using end-to-end
      ...
      listed in
section 13.5.2
as being subject to the no-transform
...
Section 4.4
describes how to determine the length of a message-body
...
23
   ...
   in transit, but is not proof against malicious attacks.)
        ...
        md5-digest   = <base64 of 128 bit MD5 digest as per
RFC 1864
>
   ...
   HTTP extends
RFC 1864
to permit the digest to be computed for MIME
...
Note: while the definition of Content-MD5 is exactly the same for
      HTTP as in
RFC 1864
for MIME entity-bodies, there are several ways
...
.
   ...
   Unlike byte-ranges-specifier values (see
section 14.35.1
), a byte-
...
MUST be followed.
   ...
   cache, SHOULD use a mechanism, such as NTP [
28
], to synchronize its
...
The Expect mechanism is hop-by-hop: that is, an HTTP/1.1 proxy MUST
   ...
   See
section 8.2.3
for the use of the 100 (continue) status.
14.21
Expires
...
and
19.6.1.1
for other requirements relating to
   Host.
14.24
If-Match
The If-Match request-header field is used with a method to make it
   ...
   If-Match header field. Entity tags are defined in
section 3.11
. The
...
section 13.3.3
   ...
   possibly using the Vary mechanism, see
section 14.44
) exists, and
...
The If-None-Match request-header field is used with a method to make
   ...
   See
section 13.3.3
for rules on how to determine if two entities tags
...
for a discussion of
   ...
   a cache, possibly using the Vary mechanism, see
section 14.44
)
...
A server MAY ignore the Range header. However, HTTP/1.1 origin
   ...
   header (see
section 14.27
) in addition to the Range header.
...
section 3.8
   ...
   SHOULD include a Via field (as described in
section 14.45
).
...
section 3.6
   ...
   defined in
section 3.6.1
. This keyword is reserved for use with
...
section 14.10
   ...
   a TE field, using these rules:
      ...
      2. If the transfer-coding being tested is one of the transfer-
         ...
         is accompanied by a qvalue of 0. (As defined in
section 3.9
, a
         qvalue of 0 means "not acceptable.")
...
The Upgrade general-header allows the client to specify what
   ...
   header field (
section 14.10
) whenever Upgrade is present in an
   HTTP/1.1 message.
...
Clients using HTTP rely heavily on the Domain Name Service, and are
   ...
   renumbering is expected to become increasingly common [
24
], the
...
35
   ...
   (see
section 19.5.1
) header in HTTP is derived, has a number of very
   serious security considerations. Content-Disposition is not part of
   the HTTP standard, but since it is widely implemented, we are
   documenting its use and risks for implementors. See
RFC 2183
[
49
]
...
By their very nature, HTTP proxies are men-in-the-middle, and
   ...
   appropriate guidelines for use developed and followed. (
Section
15.1.1
).
...
This specification makes heavy use of the augmented BNF and generic
   constructs defined by David H. Crocker for
RFC 822
[
9
]. Similarly, it
   ...
   Ned Freed for MIME [
7
]. We hope that their inclusion in this
...
The Apache Group, Anselm Baird-Smith, author of Jigsaw, and Henrik
   Frystyk implemented
RFC 2068
early, and we wish to thank them for the
...
, March 1995.
   ...
   [
2
] Anklesaria, F., McCahill, M., Lindner, P., Johnson, D., Torrey,
...
, August 1982.
   ...
   [
10
] Davis, F., Kahle, B., Morris, H., Salem, J., Shen, T., Wang, R.,
...
,
   ...
   [
15
] Nebel, E. and L. Masinter, "Form-based File Upload in HTML",
RFC
1867
, November 1995.
...
22
   ...
   [
23
] Meyers, J. and M. Rose, "The Content-MD5 Header Field",
RFC
1864
, October 1995.
...
, February 1996.
   ...
   [
25
] Deutsch, P., "GZIP file format specification version 4.3",
RFC
1952
, May 1996.
...
.
   ...
   [
31
] Deutsch, P. and J. Gailly, "ZLIB Compressed Data Format
...
, May 1997.
   ...
   [
37
] Palme, J., "Common Internet Message Headers",
RFC 2076
, February
        1997. [jg640]
...
, January 1998. [jg641]
   ...
   [
39
] Nielsen, H.F., Gettys, J., Baird-Smith, A., Prud'hommeaux, E.,
...
44
   ...
   [
45
] Palme, J. and A. Hopmann, "MIME E-mail Encapsulation of
...
, October 1996.
   ...
   [
47
] Masinter, L., "Hyper Text Coffee Pot Control Protocol
        (HTCPCP/1.0)",
RFC 2324
, 1 April 1998.
...
In addition to defining the HTTP/1.1 protocol, this document serves
   ...
   intermixed). The following is to be registered with IANA [
17
].
...
Although this document specifies the requirements for the generation
   ...
   the entity with the labels US-ASCII or ISO-8859-1. See
section 3.7.1
and 3.4.1.
   ...
   and other potential problems with date encodings include:
      ...
      - HTTP/1.1 clients and caches SHOULD assume that an
RFC-850
date
...
discusses mail, and HTTP has a few features that are different from
   those described in
RFC 2045
. These differences were carefully chosen
...
of this document describes the forms
   ...
   HTTP.
RFC 2046
requires that content with a type of "text" represent
...
Introduction of Content-Encoding
RFC 2045
does not include any concept equivalent to HTTP/1.1's
...
45
   ...
   HTTP transports all message-bodies as payload (see
section 3.7.2
) and
...
and
RFC 2068
document protocol elements used by some
...
The requirements that clients and servers support the Host request-
   header, report an error if the Host request-header (
section 14.23
) is
...
.
   ...
   Keep-Alive and Keep-Alive header) is documented in
RFC 2068
. [
33
]
...
This specification has been carefully audited to correct and
   disambiguate key word usage;
RFC 2068
had many problems in respect to
   the conventions laid out in
RFC 2119
[
34
].
...
section 3.5
   ...
   to allow clients to refuse a representation. (
Section 3.9
)
   ...
   by
RFC 2145
. Require proxies to upgrade requests to highest protocol
...
, 14.16)
   ...
   transmission requirements fixes are in sections
8.2
,
10.4.18
,
...
Section 13.5.2
   ...
   Clean up confusion between 403 and 404 responses. (
Section 10.4.4
,
   10.4.5, and 10.4.11)
   ...
   (
Section 13.1.2
, 13.2.4, 13.5.2, 13.5.3, 14.9.3, and 14.46) Warning
...
Transfer-coding had significant problems, particularly with
   ...
   worth fixing [
39
]. TE also solves another, obscure, downward
...
33
   ...
   specification, but not commonly implemented. See
RFC 2068
[
33
].
