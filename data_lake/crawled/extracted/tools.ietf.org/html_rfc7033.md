---
{
  ...
  "date": "",
  "length": 60334,
  "crawled_at": "2026-04-23T20:57:17"
}
---
...
3
3
. Example Uses of WebFinger .......................................
4
3.1
. Identity Provider Discovery for OpenID Connect .............
4
3.2
. Getting Author and Copyright Information for a Web Page ....
5
4
. WebFinger Protocol ..............................................
7
4.1
. Constructing the Query Component of the Request URI.......
7
4.2
. Performing a WebFinger Query..............................
8
4.3
. The "rel" Parameter.......................................
9
...
15
7
. Hosted WebFinger Services ......................................
15
8
. Definition of WebFinger Applications ...........................
16
8.1
. Specification of the URI Scheme and URI ...................
17
8.2
. Host Resolution ...........................................
17
...
18
8.5
. One URI, Multiple Applications ............................
18
8.6
. Registration of Link Relation Types and Properties ........
19
9
...
22
10.2
. JSON Resource Descriptor (JRD) Media Type ................
22
10.3
. Registering Link Relation Types ..........................
24
10.4
. Establishment of the "WebFinger Properties" Registry .....
24
10.4.1
...
2
] methods over a secure
   transport [
12
].  A WebFinger resource returns a JavaScript Object
   Notation (JSON) [
5
] object describing the entity that is queried.
   ...
   relations [
8
] that enable a client to discover, for example, that a
   ...
   Use of WebFinger is illustrated in the examples in
Section 3
and
   described more formally in
Section 4
.
Section 8
describes how
...
1
   ...
   information specified in the value.  In Web Linking [
4
], the link
...
attribute specifies the information that is linked to the entity or
   ...
   needs to be either a single IANA-registered link relation type [
8
] or
   a URI [
6
...
[
6
].  Relative URIs, having
   syntax following that of
...
Suppose Carol wishes to authenticate with a web site she visits using
   OpenID Connect [
15
].  She would provide the web site with her OpenID
...
The WebFinger protocol is used to request information about an entity
   ...
   WebFinger resource is a well-known URI [
3
   ...
   "resource" parameter of the WebFinger URI's query string; see
Section
4.1
for details.

   The host to which a WebFinger query is issued is significant.  If the
   query target contains a "host" portion (
...
.
   ...
   the Internet.  Also, the Cross-Origin Resource Sharing (CORS) [
7
]
...
4.1
.  Constructing the Query Component of the Request URI
A WebFinger URI MUST contain a query component (see
Section 3.4 of
   RFC 3986
).  The query component MUST contain a "resource" parameter
...
Section 2.1 of RFC 3986
.  The encoding is done to conform to the
   query production in
Section 3.4
of that specification, with the
...
A WebFinger client issues a query using the GET method to the well-
   known [
3
] resource identified by the URI whose path component is
...
resource MUST silently ignore any requested representations that it
   ...
   Resource Descriptor (JRD) is "application/jrd+json" (see
Section
10.2
).
   ...
   server might return information about a person's blog, vCard [
14
],
...
4.4
.  The JSON Resource Descriptor (JRD)
The JSON Resource Descriptor (JRD), originally introduced in
RFC 6415
[
16
] and based on the Extensible Resource Descriptor (XRD) format
   [
17
], is a JSON object that comprises the following name/value pairs:
...
The "links" array has any number of member objects, each of which
   represents a link [
4
].  Each of these link objects can have the
   following members:
...
The "titles" object comprises zero or more name/value pairs whose
   names are a language tag [
11
] or the string "und".  The string is
...
4.5
.  WebFinger and URIs
WebFinger requests include a "resource" parameter (see
Section 4.1
)
   ...
   URI: it could be an "acct" URI [
18
], an "http" or "https" URI, a
   "mailto" URI [
19
...
WebFinger resources might not be accessible from a web browser due to
   ...
   (CORS) [
7
], and servers MUST include the Access-Control-Allow-Origin
...
Any application that uses WebFinger MUST specify the URI scheme(s),
   ...
   scheme [
18
].  When trying to obtain the copyright information for a
...
illustrate the use of different
   URI schemes with WebFinger applications.  In the example in
Section
3.1
, WebFinger is used to retrieve information pertinent to OpenID
   Connect.  In the example in
Section 3.2
, WebFinger is used to
...
As explained in
Section 4
, the host to which a WebFinger query is
...
WebFinger defines both subject-specific properties (i.e., properties
   described in
Section 4.4.3
that relate to the URI for which
   information is queried) and link-specific properties (see
Section
4.4.4.5
).  This section refers to subject-specific properties.
...
RFC 7033
WebFinger                 September 2013
Consider this portion of the JRD found in the example in
Section 3.2
.
...
The links returned in a WebFinger response each comprise several
   pieces of information, some of which are optional (refer to
Section
4.4.4
...
Since this specification utilizes Cross-Origin Resource Sharing
   (CORS) [
7
], all of the security considerations applicable to CORS are
   ...
   is valid (as defined in [
12
]) and accept a response only if the
...
Systems or services that expose personal data via WebFinger MUST
   ...
   WebFinger-like services, Balduzzi et al. [
20
] took a large set of
...
Related information:  The query to the WebFinger resource will
   include one or more parameters in the query string; see
Section 4.1
   of RFC 7033
.  Resources at this location are able to return a JSON
   Resource Descriptor (JRD) as described in
Section 4.4 of RFC 7033
.
10.2
.  JSON Resource Descriptor (JRD) Media Type
...
Applications that use this media type:
     ...
     protocol (
RFC 7033
) to enable the exchange of information between a
...
.  Registering Link Relation Types
RFC 5988
established a "Link Relation Types" registry that is reused
   by WebFinger applications.

   Link relation types used by WebFinger applications are registered in
   the "Link Relation Types" registry as per the procedures of
Section
6.2.1 of RFC 5988
.  The "Notes" entry for the registration SHOULD
...
WebFinger utilizes URIs to identify properties of a subject or link
   and the associated values (see Sections
8.3
and
8.6
).  This
...
The IETF has created a mailing list, webfinger@ietf.org, which can be
   ...
   IESG has appointed Designated Experts [
13
...
[
13
]) after a review by the Designated Experts.  The review
...
>.
   ...
   [
16
]  Hammer-Lahav, E., Ed., and B. Cook, "Web Host Metadata",
RFC
6415
, October 2011.
