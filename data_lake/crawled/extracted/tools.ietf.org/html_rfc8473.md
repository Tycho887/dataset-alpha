---
{
  ...
  "url": "https://tools.ietf.org/html/rfc8473",
  ...
  "date": "",
  "length": 62319,
  "crawled_at": "2026-04-23T20:51:52"
}
---
...
4
2.1
.  HTTPS Token Binding Key-Pair Scoping  . . . . . . . . . .
5
3
.  TLS Renegotiation . . . . . . . . . . . . . . . . . . . . . .
6
...
16
7.2
.  Sensitivity of the Sec-Token-Binding Header . . . . . . .
16
7.3
.  Securing Federated Sign-On Protocols  . . . . . . . . . .
17
8
.  Privacy Considerations  . . . . . . . . . . . . . . . . . . .
20
8.1
.  Scoping of Token Binding Key Pairs  . . . . . . . . . . .
20
8.2
.  Lifetime of Token Binding Key Pairs . . . . . . . . . . .
20
8.3
.  Correlation . . . . . . . . . . . . . . . . . . . . . . .
21
...
RFC8471
] defines a Token Binding ID for a
   ...
   HTTP cookie or an OAuth token [
RFC6749
]) to a client, the server can
   ...
   While the Token Binding protocol [
RFC8471
...
Section 8.3 of [RFC7231]
   ...
   TokenBindingMessage, as defined in [
RFC8471
].  The base64url encoding
   uses the URL and filename safe character set described in
Section 5
   of [RFC4648]
, with all trailing padding characters (i.e., "=")
...
established by this TokenBinding is called a "Provided Token
   ...
   specified in
Section 5.3
.  In addition to the latter, or rather than
...
RFC6265
] are typically utilized for state
   ...
   OAuth tokens [
RFC6749
] are utilized to manage authorization for
...
as "a domain that is controlled by a
...
"example.com".  User Agents SHOULD use an up-to-date public suffix
   list, such as the one maintained by Mozilla [
PSL
].
...
RFC7230
   ...
   clearly identify which TLS Exported Keying Material value [
RFC5705
]
...
In a first-party use case (also known as a "same-site" use case), an
   ...
   See
Section 5 of [RFC8471]
for general guidance regarding the binding
...
] (in which the identity token is a SAML
   assertion).
...
To better protect the security of the identity token, the Identity
   ...
   o  Therefore, in the previous step, the Token Provider may want to
      ...
      protocol [
RFC8471
] and uses the corresponding public key of this
...
, when a client receives this header
   field, it should take the TokenBindingID [
RFC8471
] of the provided
...
Token Binding is no longer included in subsequent requests.  (A new
   ...
   other status codes.  As described in
Section 2
, if the client
...
RFC8472
   ...
   specified in [
RFC8471
].  If a Token Provider does not support the key
...
OASIS.saml-prof-2.0-os
   ...
   +------------+------------------------------------------------------+
   | EKM:       | TLS Exported Keying Material [
RFC5705
]               |
   |            |                                                      |
...
HTTPS-based applications may have multi-party use cases other than,
   ...
   of referred Token Bindings, as presented above in
Section 5.3
.
   ...
   NOTE: See
Section 8
("Privacy Considerations") for privacy guidance
...
W3C.REC-DOM-Level-3-Core-20040407
   ...
   name as a "forbidden header name"; see [
fetch-spec
].
7.3
.  Securing Federated Sign-On Protocols
...
Section 4.1 of [RFC8471]
   ...
   Token Binding of the cookie.  See
Section 2.1
above.
...
An application's various communicating endpoints that receive Token
   ...
   application associated with a Token Binding ID; see
Section 6
("Implementation Considerations").
...
Below is the Internet Assigned Numbers Authority (IANA) "Permanent
   Message Header Field Names" registration information per [
RFC3864
].
