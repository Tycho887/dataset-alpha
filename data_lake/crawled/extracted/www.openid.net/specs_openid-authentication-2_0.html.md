---
{
  "title": "Final: OpenID Authentication 2.0 - Final",
  "url": "https://www.openid.net/specs/openid-authentication-2_0.html",
  "domain": "www.openid.net",
  "depth": 2,
...
An Identifier is either a "http" or "https" URI, (commonly
            referred to as a "URL" within this document), or an
XRI
(
Reed, D. and D. McAlpin, “Extensible Resource Identifier (XRI) Syntax V2.0,” .
)
[XRI_Syntax_2.0].  This document defines
...
[RFC2616]
(
Fielding, R., Gettys, J., Mogul, J., Frystyk, H., Masinter, L., Leach, P., and T. Berners-Lee, “Hypertext Transfer Protocol -- HTTP/1.1,” .
)
.
Relying Party:
...
by presenting a User-Supplied Identifier
...
the
            User-Supplied Identifier, the Relying Party
performs discovery
(
Discovery
)
...
(optional)
            ...
            The Relying Party and the OP establish an
association
(
Establishing Associations
)
-- a shared
            secret established using
...
The OP redirects the end user's User-Agent back to the
            Relying Party with either an assertion that
authentication is
            approved
(
Positive Assertions
...
the information received from the OP including
...
Direct communication is initiated by a Relying Party to an
          OP endpoint URL.  It is used for
establishing associations
(
Establishing Associations
)
...
All direct requests are HTTP POSTs, and so
            contain the required fields listed in
Section 4.1.2
(
HTTP Encoding
)
...
5.1.2. 
Direct Response
The body of a response to a
Direct Request
(
Direct Request
)
consists of
            an HTTP Response body in
Key-Value
            Form
(
Key-Value Form Encoding
)
. The content-type of the response SHOULD be
            "text/plain".
...
If a request is malformed or contains invalid arguments,
              ...
              400. The response body MUST be a
Key-Value Form
(
Key-Value Form Encoding
)
...
In indirect communication, messages are passed through the
          ...
          Party or the OP.  Indirect communication is used for
authentication
          requests
(
Requesting Authentication
...
There are two methods for indirect communication: HTTP
          ...
          indirect messages, as specified in
Section 4.1.2
(
HTTP Encoding
)
...
The <form> element's "action" attribute value MUST
            ...
            generate a message as specified in
Section 4.1.2
(
HTTP Encoding
)
...
When generating MAC keys, the recommendations in
[RFC1750]
(
Eastlake, D., Crocker, S., and J. Schiller, “Randomness Recommendations for Security,” .
)
SHOULD be followed.
TOC
6.1. 
...
Determine the list of keys to be signed, according to
              the message to be signed (See
Section 10.1
(
Positive Assertions
)
). The list of keys to be
...
Convert the list of key/value pairs to be signed to an
              octet string by encoding with
Key-Value Form Encoding
(
Key-Value Form Encoding
)
.
Determine the signature algorithm from the
association type
(
Establishing Associations
)
...
[RFC3174]
(
Eastlake, D. and P. Jones, “US Secure Hash Algorithm 1 (SHA1),” .
)
)
...
[FIPS180‑2]
(
U.S. Department of Commerce and National Institute of Standards               and Technology, “Secure Hash Signature Standard,” .
)
...
URL Identifiers MUST then be further normalized by both
              ...
              finally applying the rules in Section 6 of
[RFC3986]
(
Berners-Lee, T., “Uniform Resource Identifiers (URI): Generic Syntax,” .
)
to the final destination URL. This
              final URL MUST be noted by the Relying Party as the
              Claimed Identifier and be used when
requesting
              authentication
(
Requesting Authentication
...
[XRI_Resolution_2.0]
(
Wachob, G., Reed, D., Chasen, L., Tan, W., and S. Churchill, “Extensible Resource Identifier (XRI) Resolution V2.0           - Committee Draft 02,” .
)
...
for definitions).  If more than one set of
...
When the Identifier is an XRI, the <xrd:XRD>
              ...
              MUST be used as the Claimed Identifier (see
Section 11.5
(
Identifying the end user
)
).  This is a vital security
...
section. It is RECOMMENDED
              ...
              this order, as specified in
Section 7.3.2.2
(
Extracting Authentication Data
)
If an OP supports extensions (
Section 12
(
Extensions
)
...
It is RECOMMENDED that a Relying Party form associations if it
        ...
        of creating or storing associations,
Section 11.4.2
(
Verifying Directly with the OpenID Provider
)
...
8.1. 
Association Session Request
An association session is initiated by a
direct request
(
Direct Communication
)
...
Value: "associate"
openid.assoc_type
The preferred association type.  The association
                    type defines the algorithm to be used to sign
                    subsequent messages.
Value: A valid association type from
Section 8.3
(
Association Types
)
...
for more information on
            these parameters.
NOTE: The 'btwoc' function is defined in
Section 4.2
(
Integer Representations
)
...
8.3.1. 
HMAC-SHA1
An association of type "HMAC-SHA1" uses the
HMAC-SHA1
(
Signature Algorithms
)
...
8.3.2. 
HMAC-SHA256
An association of type "HMAC-SHA256" uses the
HMAC-SHA256
(
Signature Algorithms
)
...
In a "no-encryption" association session, the OP sends
            ...
            are using transport layer encryption. See
Section 15.1.1
(
...
The MAC key sent by the OP MUST be the length specified
            for the requested association type, as specified in
Section 6.2
(
Signature Algorithms
)
...
. For information on the
            selection of random values, see
[RFC1750]
(
Eastlake, D., Crocker, S., and J. Schiller, “Randomness Recommendations for Security,” .
)
...
Note: If no association handle is sent, the
                  transaction will take place in
Stateless Mode
(
Verifying Directly with the OpenID Provider
)
...
[RFC3986], section 3.1 for rules about URI matching.
...
When present, the "openid.return_to" URL MUST match the
          "openid.realm", or the OP MUST return an
...
OpenID providers SHOULD verify that the return_to URL
            ...
            party endpoints for the realm by performing
discovery on the relying
            party
(
Discovering OpenID Relying Parties
)
. As always when performing discovery, the
...
,
        ...
        to complete the authentication, the OP SHOULD send a
positive assertion
(
Positive Assertions
)
...
Methods of identifying authorized end users and obtaining
        ...
        beyond the scope of this specification.  See
Section 15.1.2.1
(
Rogue Relying Party Proxying
)
for OpenID Provider security
        considerations.
...
If no association handle is specified, the OP SHOULD use a
        ...
        check the signature of the response via
Direct Verification
(
Verifying Directly with the OpenID Provider
)
...
Note: If neither Identifier is present in the assertion,
                  ...
                  other information in its payload, using
extensions
(
Extensions
)
...
When receiving a negative assertion in response to a
          ...
          Authentication 1.1 can be found in
Section 14
(
OpenID Authentication 1.1 Compatibility
)
...
An assertion has not yet been accepted from this OP with
            the same value for "openid.response_nonce" (
Section 11.3
(
Checking the Nonce
)
...
The signature on the assertion is valid and all fields
            that are required to be signed are signed (
Section 11.4
(
Verifying Signatures
)
...
Protocol Version
openid.ns
This table shows the mapping of
discovered information
(
Discovered Information
)
...
to the OP. To verify the signature,
...
"openid.claimed_id" is not defined by OpenID
                ...
                user.
"openid.identity" MUST be sent in a
authentication
                request
(
Responding to Authentication Requests
...
When responding with a negative assertion to a
                ...
                points to this URL.
The Relying Party MUST accept an
authentication
                response
(
Positive Assertions
...
that are missing the "openid.op_endpoint" parameter.
TOC
14.2.2. 
OpenID Providers
"openid.identity" MUST be sent in a
...
, since it is not part of the OpenID Authentication
                1.1 protocol.
TOC
15. 
Security Considerations
...
. Altering signed fields without the
...
XMLDSIG
(
Eastlake 3rd, D., Reagle Jr., J., and D. Solo, “(Extensible Markup Language) XML-Signature Syntax and           Processing,” .
)
[RFC3275]. The keying material
...
15.4. 
HTTP and HTTPS URL Identifiers
While these types of Identifiers have been
...
[FIPS180-2]
U.S. Department of Commerce and National Institute of Standards
              and Technology, “
...
,” RFC 2631.
[RFC3174]
Eastlake, D. and P. Jones, “
US Secure Hash Algorithm 1 (SHA1)
,” RFC 3174.
[RFC3275]
Eastlake 3rd, D., Reagle Jr., J., and D. Solo, “
...
,” RFC 3986.
[XRI_Resolution_2.0]
Wachob, G., Reed, D., Chasen, L., Tan, W., and S. Churchill, “
