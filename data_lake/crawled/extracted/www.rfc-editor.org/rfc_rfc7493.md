---
{
  ...
  "date": "",
  "length": 12717,
  "crawled_at": "2026-04-23T21:00:28"
}
---
...
RFC 7493
The I-JSON Message Format             March 2015
Table of Contents
1
.  Introduction  . . . . . . . . . . . . . . . . . . . . . . . .
2
...
.  Introduction
RFC 7159
describes the JSON data interchange format, which is widely
...
describes practices that
   ...
   "JSON texts" as defined in
RFC 7159
but with certain extra
...
2
.  I-JSON Messages
An I-JSON message is a JSON text, as defined by
RFC 7159
.
2.1
.  Encoding and Characters
...
Software that implements IEEE 754-2008 binary64 (double precision)
   numbers [
IEEE754
] is generally available and widely used.
...
An I-JSON message can be any JSON value.  However, there are software
   implementations, coded to the older specification [
RFC4627
], which
...
Protocols often contain data items that are designed to contain
   ...
   in [
RFC3339
], with the additional restrictions that uppercase rather
...
, with the same additional restrictions.
4.4
.  Binary Data
When it is required that an I-JSON protocol element contain arbitrary
   ...
   value in base64url; see
Section 5 of [RFC4648]
.
5
.  Security Considerations
All the security considerations that apply to JSON (see
RFC 7159
)
