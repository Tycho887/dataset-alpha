---
{
  ...
  "url": "https://www.rfc-editor.org/rfc/rfc2617",
  ...
  "date": "",
  "length": 76384,
  "crawled_at": "2026-04-23T21:00:21"
}
---
...
RFC 2069
have been removed from this specification due to problems
...
(possibly new) challenge applicable to the proxy for the requested
   ...
   rules found in section 14.8 of [
2
]. Both the Proxy-Authenticate and
...
HTTP Authentication                  June 1999
   ...
   receiving another challenge from the proxy server. See
section 4
for
...
If a server receives a request for an access-protected object, and an
   ...
   domain
     ...
     [
7
], that define the protection space.  If a URI is an abs_path, it
     is relative to the canonical root URL (see
section 1.2
above) of
...
This directive is not meaningful in Proxy-Authenticate headers, for
   ...
   nonce
     ...
     requests.  For more details on the issues involved see
section 4
.
     of this document.
...
stale
   ...
   algorithm
     ...
     difference in usage, see the description in
section 3.2.2.2
.
...
3.2.2.4
Directive values and quoted-string
...
2
   ...
   request line as specified in section 5.1.2 of [
2
]. This may be "*",
...
2
   ...
   when a shared cache (see section 13.7 of [
2
   ...
   request, unless one of two Cache-Control (see section 14.9 of [
2
])
...
contains the state information. The client will retry the request, at
   ...
   Authenticate and Proxy-Authorization headers described in
section 3.6
below.
3.4
Security Protocol Negotiation
...
2
] and their behavior is subject to
   ...
   Authenticate header as defined above in
section 3.2.1
.
...
However, it is significantly stronger than (e.g.) CRAM-MD5, which has
   been proposed for use with LDAP [
10
], POP and IMAP (see
RFC 2195
[
9
]).  It is intended to replace the much weaker and even more
   ...
   (see
section 3.2
above) are  protected.  Most header fields and their
...
section
3.2.2.1
above).  As shown in the example nonce in
section 3.2.1
, the
...
The following code implements the calculations of H(A1), H(A2),
   ...
   the values used in the example of
section 3.5
. It uses the MD5
   implementation from
RFC 1321
...
, August 1998.
   ...
   [
8
]  Kaliski, B.,Robshaw, M., "Message Authentication with MD5",
...
http://www.rsa.com/rsalabs/pubs/cryptobytes/spring95/md5.htm
   ...
   [
9
]  Klensin, J., Catoe, R. and P. Krumviede, "IMAP/POP AUTHorize
...
, September
   ...
   [
10
] Morgan, B., Alvestrand, H., Hodges, J., Wahl, M.,
