---
{
  ...
  "date": "",
  "length": 61793,
  "crawled_at": "2026-04-23T21:00:31"
}
---
...
JWS
   ...
   Initialization Vector" are defined by the JWE specification [
JWE
].
...
NumericDate
      ...
      2013 Edition [
POSIX.1
] definition "Seconds Since the Epoch", in
      which each day is accounted for by exactly 86400 seconds, other
      than that non-integer values can be represented.  See
RFC 3339
[
RFC3339
...
The following Claim Names are registered in the IANA "JSON Web Token
   Claims" registry established by
Section 10.1
.  None of the claims
...
Claim Names can be defined at will by those using JWTs.  However, in
   ...
   by
Section 10.1
or be a Public Name: a value that contains a
...
JWE
   ...
   by JWT applications to declare the media type [
IANA.MediaTypes
] of
...
To support use cases in which the JWT content is secured by a means
   ...
   specification [
JWA
]; it is an Unsecured JWS with the JWT Claims Set
   as its JWS Payload.
6.1
.  Example Unsecured JWT
The following example JOSE Header declares that the encoded object is
...
JWS
       ...
       *  Else, if the JWT is a JWE, create a JWE using the Message as
          the plaintext for the JWE; all steps specified in [
JWE
] for
...
Processing a JWT inevitably requires comparing known strings to
   ...
   the algorithm is, the Unicode [
UNICODE
] string encoding "alg" will be
...
This section defines which algorithms and features of this
   ...
   [
JWA
], only HMAC SHA-256 ("HS256") and "none" MUST be implemented by
...
This section establishes the IANA "JSON Web Token Claims" registry
   ...
   registers the Claim Names defined in
Section 4.1
.
...
10.3.1
.  Registry Contents
This section registers the "application/jwt" media type [
RFC2046
] in
...
JWS
] for use by claims replicated as Header
   Parameters in JWEs, per
Section 5.3
.
...
All of the security issues that are pertinent to any cryptographic
   ...
   employed.  In particular, Sections
10.12
("JSON Security
   Considerations") and 10.13 ("Unicode Comparison Security
   Considerations") of [
JWS
] apply equally to the JWT Claims Set in the
...
>.
   ...
   [
POSIX.1
]  IEEE, "The Open Group Base Specifications Issue 7", IEEE
...
Security Assertion Markup Language (SAML) 2.0
   [
OASIS.saml-core-2.0-os
] provides a standard for creating security
...
RFC3275
   ...
   especially XML Canonicalization [
W3C.REC-xml-c14n-20010315
]
...
Both JWTs and SWTs [
SWT
], at their core, enable sets of claims to be
...
Acknowledgements
   ...
   influenced by the design and simplicity of SWTs [
SWT
] and ideas for
