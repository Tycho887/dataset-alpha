---
{
  "title": "RFC 8126: Guidelines for Writing an IANA Considerations Section in RFCs",
  "url": "https://www.rfc-editor.org/rfc/rfc8126",
  ...
  "date": "",
  "length": 108764,
  "crawled_at": "2026-04-23T21:00:32"
}
---
...
4
1.1
.  Keep IANA Considerations for IANA . . . . . . . . . . . .
4
1.2
.  For Updated Information . . . . . . . . . . . . . . . . .
5
1.3
.  A Quick Checklist Upfront . . . . . . . . . . . . . . . .
5
2
...
12
3.1
.  Documentation Requirements for Registrations  . . . . . .
12
3.2
...
23
4.11
. Using the Well-Known Registration Policies  . . . . . . .
24
4.12
...
27
5.1
.  The Motivation for Designated Experts . . . . . . . . . .
27
5.2
...
34
9.1
.  When There Are No IANA Actions  . . . . . . . . . . . . .
34
9.2
.  Namespaces Lacking Documented Guidance  . . . . . . . . .
35
9.3
.  After-the-Fact Registrations  . . . . . . . . . . . . . .
35
9.4
.  Reclaiming Assigned Values  . . . . . . . . . . . . . . .
35
9.5
.  Contact Person vs Assignee or Owner . . . . . . . . . . .
36
9.6
.  Closing or Obsoleting a Registry/Registrations  . . . . .
37
10
. Appeals . . . . . . . . . . . . . . . . . . . . . . . . . . .
37
...
38
14
. Changes Relative to Earlier Editions of
BCP 26
. . . . . . .
38
14.1
.  2016: Changes in This Document Relative to
RFC 5226
. .
38
14.2
...
Many protocols make use of points of extensibility that use constants
   ...
   in these fields do not have conflicting uses and to promote
   interoperability, their allocations are often coordinated by a
   central record keeper.  The Protocol field in the IP header [
RFC791
]
   and MIME media types [
RFC6838
...
It's useful to be familiar with this document as a whole.  But when
   ...
   1.  Put all the information that IANA will need to know into the
       "IANA Considerations" section of your document (see
Section 1.1
).
...
Section 1.1
   ...
   3.  Note that the IESG has the authority to resolve issues with IANA
       ...
       may ultimately involve an Area Director (see
Section 3.3
).
...
4.12
   ...
   7.  If you're using a policy that requires a designated expert
       (Expert Review or Specification Required), understand
Section 5
...
Section 3.1
   ...
   2.  If the registry has multiple ranges from which assignments can be
       made, make it clear which range is requested (see
Section 3.1
).
...
Section 3.1
).  This will avoid registration conflicts among
       multiple documents.
...
4.  For "reference" fields, use the document that provides the best
       ...
       the relevant documentation (see Sections
3.1
and
7
).
...
Section 3.1
   ...
   7.  If the registration policy for the registry does not already
       ...
       to the registration need to be made later (see
Section 9.5
).

   If you're writing a "bis" document or otherwise making older
   documents obsolete, see
Section 8
.
   ...
   for your document to be finished and approved, see [
RFC7120
].
...
.
   ...
   If you need to update an existing registration, see
Section 3.2
.
...
Registry definitions and registrations within registries often need
   ...
   make the change.  For example, the Media Types registry [
RFC6838
]
...
Often, documents request an assignment in an existing registry (one
   ...
   identify the registry is helpful (see
Section 2.2
).
   ...
   allocation (see
Section 3.4
) or use values that have already been set
...
IANA normally takes its actions when a document is approved for
   ...
   cases.  See [
RFC7120
] for details.  It is usually not necessary to
...
Even when the space is essentially unlimited, it is still often
   ...
   interoperability and security.  See [
RFC6709
].
...
Therefore, it is important to think specifically about the
   ...
   For more discussion of that topic, see
Section 4.12
.
...
Private Use is for private or local use only, with the type and
   ...
   Examples:
      ...
      Site-specific options in DHCP [
RFC2939
]
      Fibre Channel Port Type Registry [
RFC4044
]
      TLS ClientCertificateType Identifiers 224-255 [
RFC5246
]
4.2
.  Experimental Use
Experimental Use is similar to Private Use, but with the purpose
   being to facilitate experimentation.  See [
RFC3692
] for details.
...
>, some registries
   ...
   o  URN namespaces - IANA registers URN Namespace IDs (NIDs
      [
RFC8141
]), and the organization registering an NID is responsible
...
RFC4422
      ...
      LDAP Protocol Mechanisms and LDAP Syntax [
RFC4520
]
4.5
.  Expert Review
For the Expert Review policy, review and approval by a designated
   expert (see
Section 5
) is required.  While this does not necessarily
...
Section 5
is important when deciding on an
...
RFC3748
      ...
      HTTP Digest AKA algorithm versions [
RFC4169
]
      URI schemes [
RFC7595
]
      GEOPRIV Location Types [
RFC4589
]
4.6
.  Specification Required
For the Specification Required policy, review and approval by a
   designated expert (see
Section 5
) is required, and the values and
...
document published outside of the RFC path, including informal
   ...
   rough" (see also the last paragraph of
Section 5.4
).

   As with Expert Review (
Section 4.5
), clear guidance to the designated
...
RFC4124
      ...
      TLS ClientCertificateType Identifiers 64-223 [
RFC5246
]
...
RFC5742
   ...
   Examples:
      ...
      DNSSEC DNS Security Algorithm Numbers [
RFC6014
]
      Media Control Channel Framework registries [
RFC6230
]
...
RFC5378
], have gone through IETF Last Call, and have been
...
RFC4271
      ...
      Mobile Node Identifier option types [
RFC4283
]
      TLS ClientCertificateType Identifiers 0-63 [
RFC5246
]
      DCCP Packet Types [
RFC4340
...
RFC5771
      ...
      IPv4 IGMP Type and Code values [
RFC3228
]
      Mobile IPv6 Mobility Header Type and Option values [
RFC6275
]
4.11
.  Using the Well-Known Registration Policies
Because the well-known policies benefit from both community
   ...
   For example, for media-type registrations [
RFC6838
], a number of
   ...
   (using the numbering from the full list in
Section 4
):
...
Some existing registries have policies that allow provisional
   registration: see URI Schemes [
RFC7595
] and Email Header Fields
   [
RFC3864
].  Registrations that are designated as provisional are
...
The designated expert is responsible for coordinating the appropriate
   ...
   the IANA Considerations sections of [
RFC3748
] and [
RFC3575
] for
...
originally appointed may later become unavailable, the IESG will
   ...
   The normal appeals process, as described in
[RFC2026], Section 6.5.1
,
...
In the years since [
RFC2434
] was published and put to use, experience
   ...
   reason to the contrary (and see also
Section 5.4
).  Reasons that have
...
On occasion, an RFC is issued that obsoletes a previous edition of
   ...
   when
RFC 4637
is to be obsoleted by
draft-ietf-foo-rfc4637bis
.  When
   ...
   For example, suppose
RFC 4637
registered the "BANANA" flag in the
...
RFC 4637
and, because of some
   rearrangement, now documents the flag in
Section 4.1.2
, the IANA
   Considerations of the bis document might contain text such as this:
      ...
      Name      Description          Reference
      --------  -------------------  ---------
      BANANA    Flag for bananas     [[this RFC]],
Section 4.2.1
...
RFC 4637
, IANA is asked to change
      all registration information that references [
RFC4637
] to instead
      reference [[this RFC]].
...
Occasionally, the IETF becomes aware that an unassigned value from a
   ...
   This is part of the reason for the advice in
Section 3.1
about using
...
o  Attempts should be made to contact the original party to which a
   ...
   o  It may be appropriate to write up the proposed action and solicit
      ...
      some of its "Private Use" options [
RFC3942
].
...
Sometimes there is a request to "close" a registry to further
   ...
   normal change controls (see
Section 2.3
).  Any closure, obsolescence,
...
Appeals of protocol parameter registration decisions can be made
   using the normal IETF appeals process as described in
[RFC2026],
   Section 6.5
...
Information concerning possible security vulnerabilities of a
   ...
   document [
BCP72
].  It is the responsibility of the IANA
...
13
.  IANA Considerations
Sitewide, IANA has replaced references to
RFC 5226
with references to
   this document.
14
...
Significant additions:
   ...
   o  Removed
RFC 2119
key words, boilerplate, and reference, preferring
...
,
RFC 2026
, DOI 10.17487/RFC2026, October 1996,
              <
...
, July
              2003, <
http://www.rfc-editor.org/info/bcp72
>.
...
RFC 1591
, DOI 10.17487/RFC1591, March 1994,
              <
...
[
RFC3942
]  Volz, B., "Reclassifying Dynamic Host Configuration
...
, DOI 10.17487/RFC4025, March
              2005, <
http://www.rfc-editor.org/info/rfc4025
>.

   [
RFC4044
]  McCloghrie, K., "Fibre Channel Management MIB",
RFC 4044
,
...
>.
   ...
   [
RFC4124
]  Le Faucheur, F., Ed., "Protocol Extensions for Support of
...
RFC4283
              ...
              (MIPv6)",
RFC 4283
, DOI 10.17487/RFC4283, November 2005,
              <
...
RFC4589
              ...
              Registry",
RFC 4589
, DOI 10.17487/RFC4589, July 2006,
              <
...
RFC5378
              ...
              Contributors Provide to the IETF Trust",
BCP 78
,
RFC 5378
,
...
,
RFC 5742
, DOI 10.17487/RFC5742, December 2009,
              <
...
[
RFC6014
]  Hoffman, P., "Cryptographic Algorithm Identifier
              Allocation for DNSSEC",
RFC 6014
...
Acknowledgments for This Document (2017)
   ...
   his role in this third edition.  Much of the text from
RFC 5226
...
Acknowledgments from the Second Edition (2008)
   ...
   The original acknowledgments section in
RFC 5226
was:
