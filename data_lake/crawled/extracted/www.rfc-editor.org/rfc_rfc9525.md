---
{
  ...
  "date": "",
  "length": 63073,
  "crawled_at": "2026-04-23T21:00:37"
}
---
...
.  That document also governs any
...
.  The TLS handshake protocol can also be used
with different record layers to define secure transport protocols;
at present, the most prominent example is QUIC
[
RFC9000
]
.  The
...
sometimes use certificates based
on PKIX (more precisely, certificates structured via
[
X.509
]
or
specific encodings thereof such as
[
X.690
]
), at least in certain
...
This document discusses Uniform Resource Identifiers
[
URI
]
only to the
...
.  That document provides a profile of the X.509v3
...
For the sake of clarity, and to follow the usage in
[
TLS
]
and related
...
, should be understood in the sense defined in
[
SECTERMS
]
. Such
...
name (e.g.,
bigcompany.example
), an IP address (IPv4 or IPv6), or an identifier
...
characters outside the range described in
[
US-ASCII
]
are prohibited, and internationalized
...
unrestricted
because they can be used in any type of service, such as a
single certificate being used for both the HTTP and IMAP services at the host
"bigcompany.example".  Other identifiers are
restricted
because they can only be used for
...
MUST
specify which identifiers are
mandatory to implement and
SHOULD
provide operational guidance when necessary.
¶
The Common Name RDN
MUST NOT
be used to identify a service because
...
do not change
when the client uses the DNS resource records defined in
[
SVCB-FOR-HTTPS
]
or
...
to look up connection information. However, it is possible
...
names of application services, then the specification
MUST
state that SRV-ID
...
the specification
MUST
state that URI-ID as defined in this document is not
...
A technology
MAY
disallow the use of the wildcard character in presented identifiers. If
it does so, then the specification
MUST
state that wildcard certificates as
...
The certificate
SHOULD
include a DNS-ID as a baseline
...
identifiers of type URI-ID (e.g., this is true of the Session Initiation Protocol
[
SIP
]
as specified by
...
MUST
be that of the protocol associated with the application service type,
and the "host" component
MUST
be the FQDN
of the service.  The application protocol specification
MUST
specify which URI schemes are acceptable in URI-IDs contained in PKIX
certificates used for the application protocol (e.g.,
sip
but not
sips
or
tel
for SIP as described in
[
SIP-SIPS
]
). Typically, this
...
The certificate
MAY
contain more than one DNS-ID, SRV-ID, URI-ID, or IP-ID
...
The certificate
MAY
include other application-specific identifiers
for compatibility with a deployed base, especially identifiers for
types that were defined before publication of
[
SRVNAME
]
or for which
...
DNS SRV lookups on the application service name of
isp.example
. A
...
messenger.example
that services IM addresses of the form
user@messenger.example
and that is
discoverable via DNS SRV lookups on the
messenger.example
domain.  A
certificate for this service might include SRV-IDs of
_xmpp-client.messenger.example
and
_xmpp-server.messenger.example
(see
[
XMPP
]
), as well as a DNS-ID of
messenger.example
.
¶
5.
...
A service provider
SHOULD
request certificates with as few identifiers as
...
SHOULD
request a certificate that includes
...
limit the applicability of certificates using SRV-IDs or URI-IDs, it
SHOULD
...
The client
MUST
construct a list of acceptable reference identifiers
and
MUST
do so independently of the identifiers presented by the
server.
¶
...
service type
sip
from the URI scheme and parse the domain name
college.example
...
by means of DNS SRV records, the reference identifier
SHOULD
be an SRV-ID.
¶
...
.
¶
A web browser connecting to
<https://192.0.2.107/>
would have a single
IP-ID reference identifier of
192.0.2.107
...
would probably be explicitly configured to
connect to
mail.isp.example
, whereas an SRV-aware user agent would derive
isp.example
from an email address of the form
user@isp.example
but might
also accept
mail.isp.example
...
limits the additional domain name validation
(
Section 6.3
...
<sip:voice.college.example>
would be split
into a DNS domain name portion of
voice.college.example
and an application
service type of
sip
...
Section 6.3
.
If the reference identifier produces an IP address, the client
MUST
match the IP
address; see
Section 6.4
.
If an application service type is present, it
MUST
also match the
service type; see
Section 6.5
...
internationalized domain name, as defined in
Section 2
, or not.
For clients
...
identifier
MUST
be performed by comparing the set of domain name labels using
a case-insensitive ASCII comparison, as clarified by
[
DNS-CASE
]
.  For
example,
WWW.BigCompany.Example
would be lower-cased to
www.bigcompany.example
for
comparison purposes.  Each label
MUST
match in order for the names to be
...
internationalized domain name, then the client
MUST
...
or comparing it with others.  In accordance with
[
IDNA-PROTO
]
, A-labels
...
only defines string names, not octet identifiers
...
.  The client
MUST
check (1) the combination of (a) an
application service type of
xmpp-client
and (b) a DNS domain name of
messenger.example
as well as (2)
a DNS domain name of
app.example
.  However, the
client
MUST NOT
check the combination of an application service type of
xmpp-client
and a DNS domain name of
app.example
because it does not
have an SRV-ID of
_xmpp-client.app.example
...
, the underscore "_" is part of the service name in
...
client
MUST
use the matched reference identifier as the validated identity of
the application service.
¶
If the client does not find a presented identifier matching any of the
reference identifiers, then the client
MUST
proceed as follows.
¶
...
then it
SHOULD
terminate the communication attempt with a bad
certificate error and log the error appropriately.  The application
MAY
provide a configuration setting to disable this behavior, but it
MUST NOT
...
user, then it
SHOULD
inform the user of the identity mismatch and
...
Such clients
MAY
give advanced users the option of proceeding
...
The application
MAY
also present the user with the ability to accept the
presented certificate as valid for subsequent connections.  Such ad hoc
"pinning"
SHOULD NOT
restrict future connections to just the pinned
certificate. Local policy that statically enforces a given certificate for a
given peer
SHOULD
be made available only as prior configuration rather than a
...
or buggy hosts. For example, see
[
Defeating-SSL
]
...
As specified in
Section 6.3
, restricting the presented identifiers in certificates to only one
...
to follow best practices to prevent XSS attacks; for example, see
[
XSS
]
, which was
...
As noted in
Section 3
, application protocols can disallow the use of
...
MUST
include both a "scheme" and a "host" component that matches the "reg-name"
rule; if the entry does not include both, it is not a valid URI-ID and
MUST
be
...
specification maintained by the WHATWG
[
URL
]
...
A-labels only (
Section 6.3
). The limited scope of this specification likely
...
); in any case, such concerns are a matter for
...
TLS extensions such as the Server Name Indication (SNI), as discussed in
[
TLS-EXT
],
Section 3
, and ALPN, as discussed in
[
ALPN
]
, provide a way for the application
...
the set of presented identifiers.
[
PKIX
],
Section 4.2.1.10
...
,
SHOULD
take care to ensure that CAs issuing such certificates are
appropriately constrained. This
MAY
take the form of local policy through
agreement with the issuing CA or
MAY
be enforced by the client requiring
...
,
<
https://media.blackhat.com/bh-ad-10/Hansen/Blackhat-AD-2010-Hansen-Sokol-HTTPS-Can-Byte-Me-slides.pdf
>
.
[NAPTR]
Mealling, M.
...
,
"Coded Character Sets - 7-bit American Standard Code for Information Interchange (7-Bit ASCII)"
,
...
Thanks also to
Carsten Bormann
for converting the previous version of this specification to Markdown so that we could more easily use
Martin Thomson's
i-d-template
software.
¶
In addition to discussions within the UTA Working Group, the following people
