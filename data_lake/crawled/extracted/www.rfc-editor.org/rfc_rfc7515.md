---
{
  ...
  "date": "",
  "length": 128973,
  "crawled_at": "2026-04-23T21:00:29"
}
---
...
52
Appendix C
.  Notes on Implementing base64url Encoding without
                Padding ..............................................
54
Appendix D
...
JSON Web Signature (JWS) represents content secured with digital
   ...
   [
RFC7159
] data structures.  The JWS cryptographic mechanisms provide
   integrity protection for an arbitrary sequence of octets.  See
Section 10.5
for a discussion on the differences between digital
   signatures and MACs.
   ...
   [
JWA
] specification and an IANA registry defined by that
   specification.  Related encryption capabilities are described in the
   separate JSON Web Encryption (JWE) [
JWE
] specification.
...
Section 3.2
      ...
      sequence is the empty string.  (See
Appendix C
for notes on
...
JWE
   ...
   are defined by the "Internet Security Glossary, Version 2" [
RFC4949
].
3
.  JSON Web Signature (JWS) Overview
...
Section 2
   ...
   these values (each of which is defined in
Section 2
):
...
In the JWS Compact Serialization, no JWS Unprotected Header is used.
   ...
   See
Section 7.1
for more information about the JWS Compact
   Serialization.
...
In the JWS JSON Serialization, one or both of the JWS Protected
   ...
   values, rather than just one.  See
Section 7.2
...
, including specifying the
   ...
   object is a JSON Web Token [
JWT
] and the JWS Protected Header and the
   JWS Payload are secured using the HMAC SHA-256 [
RFC2104
] [
SHS
]
...
{"iss":"joe",
   ...
   SHA-256 algorithm using the key specified in
Appendix A.1
and
   ...
   See
Appendix A
for additional examples, including examples using the
...
Section 15.12
("The JSON Object") of ECMAScript 5.1 [
ECMAScript
].
...
Section 4.1.11
, all Header Parameters not defined by this
...
Section 9.1
, with meanings as defined in the
   subsections below.
...
RFC3986
   ...
   keys MUST be encoded as a JWK Set [
JWK
...
Section 8
on TLS requirements.  Use of this Header Parameter is
   OPTIONAL.
4.1.3
.  "jwk" (JSON Web Key) Header Parameter
...
RFC5280
   ...
   certificate or certificate chain that conforms to
RFC 5280
[
RFC5280
]
...
RFC4945
   ...
   the certificate MUST use TLS [
RFC2818
] [
RFC5246
]; and the identity of
   the server MUST be validated, as per
Section 6 of RFC 6125
[
RFC6125
].
   Also, see
Section 8
on TLS requirements.  Use of this Header
...
-- not base64url-encoded) DER
   [
ITU.X690.2008
] PKIX certificate value.  The certificate containing
   the public key corresponding to the key used to digitally sign the
   JWS MUST be the first certificate.  This MAY be followed by
   additional certificates, with each subsequent certificate being the
   one used to certify the previous one.  The recipient MUST validate
   the certificate chain according to
RFC 5280
[
RFC5280
] and consider
...
JWA
   ...
   or [
JWA
] for use with JWS, duplicate names, or names that do not
   ...
   specification or [
JWA
] for use with JWS or if any other constraints
...
(
Section 4.2
).  Unlike Public Header Parameter names, Private Header
...
[
RFC7159
]; let the JWS Protected Header be this JSON
       object.
...
JSON Serialization, this restriction includes that the same
   ...
   8.  Validate the JWS Signature against the JWS Signing Input
       ...
       See
Section 10.6
for security considerations on algorithm
...
[
RFC7159
].  Since the only string comparison
   ...
   Also, see the JSON security considerations in
Section 10.12
and the
   Unicode security considerations in
Section 10.13
.
6
.  Key Identification
It is necessary for the recipient of a JWS to be able to determine
   ...
   described in
Section 4.1
or can be identified using methods that are
...
the signature or MAC fails when the algorithm used requires a key
   ...
   Also, see
Appendix D
for notes on possible key selection algorithms.
7
.  Serializations
...
member of the signature/MAC's array element.  The union of these sets
   ...
   See
Appendix A.6
for an example JWS using the general JWS JSON
...
In summary, the syntax of a JWS using the flattened JWS JSON
   ...
   See
Appendix A.7
for an example JWS using the flattened JWS JSON
   Serialization syntax.
8
.  TLS Requirements
Implementations supporting the "jku" and/or "x5u" Header Parameters
   ...
   of this writing, TLS version 1.2 [
RFC5246
] is the most recent
   version.
   ...
   See current publications by the IETF TLS working group, including
RFC
6176
[
RFC6176
], for guidance on the ciphersuites currently considered
...
9.2.1
.  Registry Contents
This section registers the "application/jose" media type [
RFC2046
] in
...
[
RFC6838
], which can be used to indicate that the content
...
See Section 8.1 of [
JWA
] for security considerations on cryptographic
   agility.
10.5
.  Differences between Digital Signatures and MACs
...
The digital signature representations for some algorithms include
   ...
   instance, signatures produced with RSASSA-PKCS1-v1_5 [
RFC3447
] encode
...
Strict JSON [
RFC7159
] validation is a security requirement.  If
   ...
   conforming to the JSON-text syntax defined in
RFC 7159
MUST be
   rejected in their entirety by JSON parsers.
Section 4
of "The JavaScript Object Notation (JSON) Data Interchange
   Format" [
RFC7159
] states, "The names within an object SHOULD be
...
ECMAScript
   ...
   Thus, this specification requires that the "SHOULD" in
Section 4 of
   [RFC7159]
...
[
RFC7159
]).  This means, for instance, that these JSON
...
>.
   ...
   [
RFC6176
]    Turner, S. and T. Polk, "Prohibiting Secure Sockets
...
>.
   ...
   [
JSS
]        Bradley, J. and N. Sakimura, Ed., "JSON Simple Sign",
                September 2010, <
...
>.
   ...
   [
JWE
]        Jones, M. and J. Hildebrand, "JSON Web Encryption
                (JWE)",
RFC 7516
...
This section provides several examples of JWSs.  While the first
   three examples all represent JSON Web Tokens (JWTs) [
JWT
], the
   payload can be any octet sequence, as shown in
Appendix A.4
.
A.1
.  Example JWS Using HMAC SHA-256
...
The following octet sequence, which is the UTF-8 representation used
   ...
   represented in JSON Web Key [
JWK
] format below (with line breaks
...
RFC 7515
JSON Web Signature (JWS)                May 2015
This example uses the RSA key represented in JSON Web Key [
JWK
]
...
This example uses the Elliptic Curve key represented in JSON Web Key
   [
JWK
] format below (with line breaks within values for display
   purposes only):
...
, resulting in the same JWS Signature value; therefore,
   ...
   Protected Header and key are the same as in
Appendix A.3
, resulting
...
This section contains an example using the flattened JWS JSON
   ...
   signature of the previous example in
Appendix A.6
.
...
The JSON array below is an example of a certificate chain that could
   ...
   Parameter, per
Section 4.1.6
(with line breaks within values for
   display purposes only):
...
This appendix describes a set of possible algorithms for selecting
   ...
   supplements the normative information on key location in
Section 6
.
...
UNDEFINED
" that they do not understand.  Any other similar input, in
   which the use of the value "
