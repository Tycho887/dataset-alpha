---
{
  ...
  "date": "",
  "length": 100870,
  "crawled_at": "2026-04-23T20:49:58"
}
---
...
Those challenges go beyond the original scope of
[
RFC6749
]
,
...
were developed in order to support the usage of OAuth in
dynamic scenarios.
¶
...
authorization response by attackers. Public clients MUST use PKCE
[
RFC7636
]
to
this end. For confidential clients, the use of PKCE
[
RFC7636
]
is RECOMMENDED.
...
Section 4.5.3.2
,
confidential clients MAY use the OpenID Connect
nonce
parameter and the
respective Claim in the ID Token
[
OpenID
]
instead. In any case, the PKCE
challenge or OpenID Connect
nonce
MUST be transaction-specific and securely
...
. When asymmetric methods for client
...
the same URI), mix-up attacks (see
Section 4.4
), where the client is tricked
...
redirector (see
Section 4.10.1
) in order to get access to access tokens. This allows
...
comparison as defined in
[
RFC3986
]
, Section 6.2.1. The only exception are
native apps using a
localhost
...
¶
The attacker obtains an authorization code by performing any of
the attacks described above.
¶
He starts a regular OAuth authorization process with the
legitimate client from his device.
¶
The attacker injects the stolen authorization code in the response
...
control the network.
¶
The legitimate client sends the code to the authorization server's
...
nonce
parameter can be used for the same
purpose. The
nonce
value is one-time use and created by the client.
...
nonce
to the authorization code and attests this binding in the ID
...
client properly checks the
nonce
parameter in the ID Token and does
...
MUST validate the
nonce
in the ID Token obtained from the
 token endpoint, even if another ID Token was obtained from the
 authorization response (e.g.,
response_type=code+id_token
), and
¶
MUST ensure that, unless and until that check succeeds, all
...
state
to the code, using token binding
...
using one of the countermeasures discussed in
Section 4.5
.
¶
4.7.
...
. The same protection is provided by
PKCE or the OpenID Connect
nonce
value.
¶
...
identity of the resource owner as communicated in the
sub
Claim
...
or other mechanisms. If a client is able to choose its own
client_id
during registration with the authorization server, then
there is a risk that it can register with the same
sub
value as a
...
Work in Progress
,
Internet-Draft, draft-ietf-oauth-dpop-04
,
4 October 2021
,
<
...
Work in Progress
,
Internet-Draft, draft-ietf-oauth-rar-08
,
18 October 2021
,
<
...
Work in Progress
,
Internet-Draft, draft-ietf-oauth-par-10
,
29 July 2021
,
<
https://datatracker.ietf.org/doc/html/draft-ietf-oauth-par-10
>
.
[arXiv.1704.08539]
Fett, D.
...
Work in Progress
,
Internet-Draft, draft-ietf-oauth-resource-indicators-08
,
11 September 2019
,
<
...
Work in Progress
,
Internet-Draft, draft-ietf-oauth-iss-auth-resp-04
,
2 December 2021
,
<
