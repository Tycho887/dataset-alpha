---
{
  ...
  "url": "https://datatracker.ietf.org/doc/html/draft-ietf-oauth-sd-jwt-vc-13",
  ...
  "date": "",
  "length": 103328,
  "crawled_at": "2026-04-23T20:47:49"
}
---
...
rules to express Verifiable Credentials with JSON payloads with and without selective disclosure based on the SD-JWT
[
...
can in principle be used to express
...
¶
Consumer:
Applications using the Type Metadata specified in
Section 6
are called Consumer. This typically includes Issuers, Verifiers, and Wallets.
¶
...
The
typ
header parameter of the SD-JWT MUST be present. The
typ
value MUST
use
dc+sd-jwt
...
¶
Note that this draft used
vc+sd-jwt
as the value of the
typ
header from its inception in July 2023 until November 2024 when it was changed to
dc+sd-jwt
to avoid conflict with the
vc
media type name registered by the W3C's Verifiable Credentials Data Model draft. In order to facilitate a minimally disruptive transition, it is RECOMMENDED that Verifiers and Holders accept both
vc+sd-jwt
and
dc+sd-jwt
as the value of the
...
selectively disclosable. This specification does not define any
vct
values; instead
...
¶
The following is a non-normative example of how
vct
is used to express
a type:
¶
...
can be associated with rules that define that at least the registered JWT claims
given_name
,
family_name
,
...
, and
address
must appear in the Unsecured Payload. Additionally, the registered JWT claims
email
and
phone_number
, and the private claims
is_over_18
,
is_over_21
, and
...
this claim explicitly indicates the Issuer of the Verifiable Credential
when it is not conveyed by other means (e.g., the subject of the end-entity certificate of an
x5c
header).
¶
nbf
: OPTIONAL. The time before which the Verifiable Credential MUST NOT be
accepted before validating. See
[
RFC7519
]
...
Verifiable Credential is no longer valid. See
[
RFC7519
]
for more
...
¶
cnf
: OPTIONAL unless cryptographic Key Binding is to be supported, in which case it is REQUIRED. Contains the confirmation method identifying the proof of possession key as defined in
[
RFC7800
]
...
. For proof of cryptographic Key Binding, the KB-JWT in the presentation of the SD-JWT MUST be secured by the key identified in this claim.
¶
vct
...
¶
vct#integrity
: OPTIONAL. The hash of the Type Metadata document to provide integrity as defined in
Section 7
.
¶
status
...
status
claim is present and using the
status_list
...
MAY be used.
¶
Binary data in claims SHOULD be encoded as data URIs as defined in
[
RFC2397
]
. Exceptions can be made when data formats are used that already define a text encoding suitable for use in JSON or where an established text encoding is commonly used. For example, images would make use of data URIs, whereas hash digests in base64 encoding do not need to be encoded as such.
¶
An example of a claim containing binary data encoded as a data URI is shown in
Appendix B.1
.
¶
3.2.2.4.
...
In that case, the SD-JWT VC MUST NOT contain the
_sd
...
eGppV1diWk1RR0hWV0tWUTRoYlNJaXJzVmZ1ZWNDRTZ0NGpUOUYySFpRIn19fQ.XURsF
s2def7z9ldt9UDafvkIjtmaR76a-vBqiBcieJuQWwn0gFzxgDGoMv2xcZ3IruzXnugNj
OT__R-luaB_GA~WyIyR0xDNDJzS1F2ZUNmR2ZyeU5STjl3IiwgImdpdmVuX25hbWUiLC
...
¶
Examples of what presentations of SD-JWT VCs might look like are provided in
Section 4.3
.
¶
3.4.
...
employing an Issuer Signature Mechanism (defined in
Section 3.5
) that is permitted for the Issuer according to policy.
¶
If Key Binding is required (refer to the security considerations in Section 9.5 of
[
I-D.ietf-oauth-selective-disclosure-jwt
]
), the Verifier MUST verify the KB-JWT
according to Section 7.3 of
[
I-D.ietf-oauth-selective-disclosure-jwt
]
. To verify
...
¶
JWT VC Issuer Metadata: A mechanism to retrieve the Issuer's public key using web-based resolution. When the value of the
iss
claim of the Issuer-signed JWT is an HTTPS URI, the recipient obtains the public key using the keys from JWT VC Issuer Metadata as defined in
Section 5
.
¶
X.509 Certificates: A mechanism to retrieve the Issuer's public key using the X.509 certificate chain in the SD-JWT header. When the protected header of the Issuer-signed JWT contains the
x5c
parameter, the recipient uses the public key from the end-entity certificate of the certificates from that
x5c
...
is used, whereas support for the JWS
JSON Serialization in Section 8 of
[
I-D.ietf-oauth-selective-disclosure-jwt
]
is OPTIONAL.
...
4.3.
Examples
The following is a non-normative example of a presentation of the SD-JWT shown in
Section 3.3
including a KB-JWT.
In this presentation, the Holder provides only the Disclosures for the
address
and
is_over_65
claims.
...
eGppV1diWk1RR0hWV0tWUTRoYlNJaXJzVmZ1ZWNDRTZ0NGpUOUYySFpRIn19fQ.XURsF
s2def7z9ldt9UDafvkIjtmaR76a-vBqiBcieJuQWwn0gFzxgDGoMv2xcZ3IruzXnugNj
OT__R-luaB_GA~WyJsa2x4RjVqTVlsR1RQVW92TU5JdkNBIiwgImlzX292ZXJfNjUiLC
...
I6ICJodHRwczovL2V4YW1wbGUuY29tL3ZlcmlmaWVyIiwgImlhdCI6IDE3NjI0NDczOT
QsICJzZF9oYXNoIjogImJzSGhVQkdmNDF0RGxfdWg3b0R4MElFZUxMbXhMRGRhY1Rab2
dlaWhVM1kifQ.QF1gFBo_xSNMFtcqyBjIi8_78jqaC_0Hsl66ep3WFzu9ai7y5MJYHa_
y0i0lHwYcbV61RMlZx_mNiDw8drfeaQ
¶
...
hbGciOiAic2hhLTI1NiJ9.hTtikX3aK5wHr8tM9ow2CQ4qottHiHjqtbf9mZPslmWENu
zV0T0FxqipsAxIgjejdfleg8StDytgouCKTxTMCA~WyJsa2x4RjVqTVlsR1RQVW92TU5
...
A successful response MUST use the
200 OK HTTP
and return the JWT VC Issuer
Metadata configuration using the
application/json
content type.
¶
...
¶
issuer
: REQUIRED. The Issuer identifier, which MUST be identical to the
iss
value in the JWT.
¶
jwks_uri
...
document which contains the Issuer's public keys. The value of
this field MUST point to a valid JWK Set document.
¶
jwks
: OPTIONAL. Issuer's JSON Web Key Set
[
RFC7517
]
document value, which
contains the Issuer's public keys. The value of this field MUST be a JSON
object containing a valid JWK Set.
¶
JWT VC Issuer Metadata MUST include either
jwks_uri
or
jwks
in their JWT VC
...
¶
It is RECOMMENDED that the JWT contains a
kid
...
The
issuer
value returned MUST be identical to the
iss
value of the
...
excerpts, the full examples can be found in
Appendix B.2
.
¶
The following is an example of an SD-JWT VC payload, containing a
vct
claim
with the value
...
Section 6.3.1
and the following Type Metadata document is
retrieved from it:
¶
{
  "vct":"https://betelgeuse.example.com/education_credential",
...
A consumer retrieving Type Metadata MUST ensure that the
vct
value in the
SD-JWT VC payload is identical to the
vct
value in the reference to the Type
Metadata (either in the SD-JWT VC itself or in an
extends
property in a Type
Metadata document).
¶
...
vct#integrity
MUST be an "integrity metadata" string as defined in Section
Section 7
.
¶
6.3.2.
...
¶
6.3.3.
Using a Defined Retrieval Method
...
retrieve Type Metadata based on a URN in the
vct
claim.
¶
6.3.4.
...
protection is present in the Type Metadata as defined in
Section 7
, the Consumer MAY assume that the Type Metadata is static and can be cached
indefinitely. Otherwise, the Consumer MUST use the
Cache-Control
...
Section 10.3
apply in order to avoid problems with circular dependencies.
¶
...
The
vct
claim in the SD-JWT VC as defined in
Section 3.2.2.2
and various URIs in the
...
. A Consumer of the respective documents MUST verify the
integrity of the retrieved document as defined in Section 3.3.5 of
[
W3C.SRI
]
.
...
¶
locale
: A language tag as defined in Section 2 of
[
RFC5646
]
...
¶
background_color
: An RGB color value as defined in
[
W3C.CSS-COLOR
]
for the background of the credential.
This property is OPTIONAL.
¶
text_color
: An RGB color value as defined in
...
value for the text of the credential. This property
is OPTIONAL.
¶
8.1.1.1.
Logo Metadata
...
¶
uri
: A URI pointing to the logo image. This property is REQUIRED.
¶
uri#integrity
...
¶
alt_text
: A string containing alternative text for the logo image. This
...
¶
uri
: A URI pointing to the background image. This property is REQUIRED.
¶
uri#integrity
: An "integrity metadata" string as described in
Section 7
...
¶
8.1.2.
Rendering Method "svg_templates"
The
svg_templates
...
described in
Section 8.1.2.1
. This property is REQUIRED if more than
...
SHOULD reject not render the SVG template. If the
svg_id
is present in the
...
credential. This property is OPTIONAL. If omitted, the default value is
false
. See
Section 9.3
for details.
...
Processing of
path
In detail, the array components of
path
...
wildcard selection of array elements. It does not use JSON Path
[
RFC9535
]
as
...
¶
label
: A human-readable label for the claim, intended for end users. This
property is REQUIRED.
¶
description
: A human-readable description for the claim, intended for end
users. This property is OPTIONAL.
¶
9.3.
Claim Mandatory Metadata
The
mandatory
property is a boolean indicating that, if set to
true
, the
claim MUST be included in the credential by the Issuer. If the value is
false
...
¶
9.5.
Extending Claim Metadata
When an SD-JWT VC type extends another type as described in
Section 6.4
, all claim metadata from the extended type MUST be
...
and
mandatory
An extending type can specify an
sd
property for a claim that is marked as
allowed
in the extended type (or where
sd
was omitted), changing it to either
always
or
never
.
However, it MUST NOT change a claim that is marked as
always
or
never
in the extended
...
¶
Similarly, an extending type can set the
mandatory
property of a claim that is
optional in the extended type to
true
, but it MUST NOT change a claim that is
mandatory
in the extended type to
false
.
¶
9.5.2.
...
¶
In this example, the child type inherits the
name
claim metadata from the base type, but overrides the
address.city
claim metadata with its own definition. It also adds a new claim metadata for
nationalities
...
¶
Additional considerations can be found in
[
OWASP_SSRF
]
.
...
as outlined in
Section 3.5
, it is critical that those rules maintain the integrity of the
relationship between the
iss
value of the SD-JWT, if present, and the public keys of the Issuer.
¶
It MUST be ensured that for any given
iss
...
¶
10.3.
Circular "extends" Dependencies of Types
...
¶
10.4.
Robust Retrieval of Type Metadata
In
Section 6.3
, various methods for distributing and retrieving
...
Section 6.3.4
if possible. Such a cache MAY be populated with metadata before
...
¶
10.5.
Risks Associated with Textual Information
Some claims in the SD-JWT VC and properties in the Type Metadata, e.g.,
display
...
¶
10.6.
Credential Type Extension and Issuer Authorization
When processing credentials, it is important to recognize that the ability to extend or reference
an existing credential type (e.g., a well-known identifier such as
urn:ec.eu.x.y.z
) does not
confer any implicit authorization to issue credentials of that type or its extensions. In particular:
¶
Issuer Authorization
: Verifiers and wallets MUST NOT assume that any issuer who issues a credential extending a known type is authorized to do so. The mere presence of an extension or reference to a recognized type (e.g., a national type
urn:de.bla
extending a European PID type) does not validate the issuer's authority.
¶
Rogue Issuers
: Attackers may issue credentials with types that extend or mimic legitimate types (e.g.,
urn:attacker
extending
urn:ec.eu.x.y.z
). Such credentials MUST NOT be accepted solely based on their type hierarchy or extension relationship.
¶
Processing Rules
: Implementations MUST verify the issuer's authorization independently of the credential type or its extensions. This typically involves checking the issuer's identity, trust status, and any relevant accreditation or registry before accepting a credential.
¶
Recommendation:
...
disclosure of claims of a given SD-JWT VC, the
vct
...
¶
In general, Issuers are advised to choose
vct
...
per Holder, e.g.,
https://example.com/issuer/holder-1234
...
Issuers.
¶
Another related concern arises from the use of confirmation methods in the cnf claim that involve retrieving key material from a remote source, especially if that source is controlled by the issuer. This includes, but is not limited to, the use of the x5u parameter in JWKs (
Section 4.6
of [
RFC7517
]
...
payloads and secured by SD-JWT
[
I-D.ietf-oauth-selective-disclosure-jwt
]
. Other specifications exist
that define their own verifiable credential formats; for example, W3C Verifiable
Credential Data Model (VCDM) 2.0
[
W3C.VCDM
]
defines a data model for verifiable credentials encoded as JSON-LD, and
ISO/IEC 18013-5:2021
[
ISO.18013-5
]
defines a representation of verifiable credentials in the mobile document (mdoc)
format encoded as CBOR and secured using COSE.
¶
12.1.
Privacy-Preserving Retrieval of Type Metadata
In
Section 6.3
, various methods for distributing and retrieving
...
Work in Progress
,
Internet-Draft, draft-ietf-oauth-status-list-13
,
20 October 2025
,
<
...
A.2.1.
application/dc+sd-jwt
The Internet media type for an SD-JWT VC is
application/dc+sd-jwt
.
¶
Type name:
...
¶
Person & email address to contact for further information: Oliver Terbu
...
in the "PID Rulebook" in
[
EUDIW.ARF
]
, but does not intend to fully comply with
that specification.
¶
...
¶
eyJhbGciOiAiRVMyNTYiLCAidHlwIjogImRjK3NkLWp3dCJ9.eyJfc2QiOiBbIjJyMDA
...
jQ0U2dDRqVDlGMkhaUSJ9fX0.E8eiMhI9SoKldGeOVL9ix5wHO7Z-P0KgZYfnhRSgk-M
edDZHf0u8CmcD1Wcm65u8ePmOAvRdeT7hWSmNcGIU5w~WyIyR0xDNDJzS1F2ZUNmR2Zy
...
pbERsczd2Q2VHZW1jIiwgInkiOiAiWnhqaVdXYlpNUUdIVldLVlE0aGJTSWlyc1ZmdWV
jQ0U2dDRqVDlGMkhaUSJ9fX0.E8eiMhI9SoKldGeOVL9ix5wHO7Z-P0KgZYfnhRSgk-M
edDZHf0u8CmcD1Wcm65u8ePmOAvRdeT7hWSmNcGIU5w~WyJlSzVvNXBIZmd1cFBwbHRq
...
3ZlcmlmaWVyIiwgImlhdCI6IDE3NjI0NDczOTQsICJzZF9oYXNoIjogIkNkc0c2NXpqZ
GkwYVo5WGxIZlljYXlqS1pJZDZpNjQzekpZaG5fX0R1OGcifQ.xuGUnqsCP_T-GEhWzB
GH8SoIDniwETaFo7p0Vtp3AONWN3cHZ3iMPv9_B3cAen-Fsx4LbUdQ27oeda7T2AQ0Ww
¶
...
¶
{
  ...
  "iat": 1762447394,
  "sd_hash": "CdsG65zjdi0aZ9XlHfYcayjKZId6i643zJYhn__Du8g"
}
¶
After validation, the Verifier will have the following processed SD-JWT payload available for further handling:
¶
...
¶
Note that in this example, there are four definitions affecting the
degrees
claim:
¶
The
degrees
array itself is marked as
sd: never
, meaning the element
degrees
cannot be selectively disclosed and will always exist in the disclosed SD-JWT. Changing this to
sd: always
would mean that the
degrees
...
array (denoted by
null
in the path) is marked as
sd: always
, meaning each degree object will be selectively disclosed as a whole.
¶
The
field_of_study
property of each degree object is marked as
sd: never
, meaning that if the respective degree object is disclosed, the
field_of_study
...
or
sd=never
to avoid ambiguity and introduce rules for
sd
and
mandatory
when extending types
¶
Provide some guidance on versioning via the
vct
value
¶
