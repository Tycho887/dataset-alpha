---
{
  ...
  "date": "",
  "length": 86390,
  "crawled_at": "2026-04-23T20:53:04"
}
---
...
RFC 7591
OAuth 2.0 Dynamic Registration            July 2015
Copyright Notice
...
33
A.1
.  Open versus Protected Dynamic Client Registration . . . .
34
A.1.1
...
34
A.2
.  Registration without or with Software Statements  . . . .
34
A.2.1
.  Registration without a Software Statement . . . . . .
34
A.2.2
.  Registration with a Software Statement  . . . . . . .
34
A.3
.  Registration by the Client or Developer . . . . . . . . .
34
A.3.1
.  Registration by the Client  . . . . . . . . . . . . .
35
A.3.2
.  Registration by the Developer . . . . . . . . . . . .
35
A.4
.  Client ID per Client Instance or per Client Software  . .
35
A.4.1
...
35
A.5
.  Stateful or Stateless Registration  . . . . . . . . . . .
35
A.5.1
.  Stateful Client Registration  . . . . . . . . . . . .
36
A.5.2
.  Stateless Client Registration . . . . . . . . . . . .
36
Acknowledgments . . . . . . . . . . . . . . . . . . . . . . . . .
36
Authors' Addresses  . . . . . . . . . . . . . . . . . . . . . . .
...
1
.  Introduction
In order for an OAuth 2.0 [
RFC6749
] client to utilize an OAuth 2.0
   ...
   Registration 1.0" [
OpenID.Registration
] and used by "User Managed
   Access (UMA) Profile of OAuth 2.0" [
UMA-Core
] in a way that is
...
"refresh token", "resource owner", "resource server", "response
   type", and "token endpoint" defined by OAuth 2.0 [
RFC6749
] and uses
   the term "Claim" defined by JSON Web Token (JWT) [
RFC7519
].
...
.  Absolute URIs can also be used as values for this
      ...
      authentication scheme as specified in
Section 2.3.1
of OAuth 2.0.
...
RFC7522
      ...
      2.0,
Section 4.5
.  If omitted, the default behavior is that the
...
If the authorization endpoint is used by the grant type, the value
      ...
      process is described in OAuth 2.0,
Section 4.5
.  If omitted, the
...
RFC6749
   ...
   contacts
      ...
      support requests for the client.  See
Section 6
for information on
      Privacy Considerations.
...
.
   ...
   jwks_uri
      ...
      [
RFC7517
] document, which contains the client's public keys.  The
...
of this document.  The
   ...
   suitable defaults as described in
Section 3.2.1
or by returning an
...
.
   ...
   body of a registration request, as described in
Section 3.1
, or
   included as claims in a software statement, as described in
Section 2.3
; a mixture of both is also possible.  If the same client
...
RFC5646
   ...
   character.  Since JSON [
RFC7159
] member names are case sensitive, it
...
IANA.Language
   ...
   spelled with mixed-case characters.  However, since
BCP 47
language
   ...
   recommendations in
BCP 47
, language tag values used in metadata
...
.
   ...
   The client registration endpoint MAY be an OAuth 2.0 [
RFC6749
]
...
This operation registers a client with the authorization server.  The
   ...
   HTTP Entity Payload is a JSON [
RFC7159
] document consisting of a JSON
...
The following is a non-normative example request not using an initial
   ...
   this example as an OAuth 2.0 Bearer Token [
RFC6750
], but any OAuth
...
In addition to JSON elements, client metadata values MAY also be
   provided in a software statement, as described in
Section 2.3
.  The
...
In the following example, some registration parameters are conveyed
   as claims in a software statement from the example in
Section 2.3
,
...
.
   ...
   mechanism such as that defined in [
RFC7592
...
The successful registration response uses an HTTP 201 Created status
   ...
   JSON object [
RFC7159
] with all parameters as top-level members of the
   object.
...
This specification establishes the "OAuth Dynamic Client Registration
   ...
   registered with a Specification Required ([
RFC5226
]) after a two-week
   ...
   specification will be published, per [
RFC7120
].
...
o  Client Metadata Name: "jwks"
   o  Client Metadata Description: Client's JSON Web Key Set [
RFC7517
]
      document representing the client's public keys
   o  Change Controller: IESG
...
Since requests to the client registration endpoint result in the
   ...
   registration endpoint.  The server MUST support TLS 1.2 [
RFC5246
] and
...
In some cases, authorization servers will maintain state about
   ...
   operations upon stateful registrations is described in [
RFC7592
].
A.5.2
.  Stateless Client Registration
