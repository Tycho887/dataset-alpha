---
{
  ...
  "url": "https://datatracker.ietf.org/doc/html/draft-ietf-oauth-security-topics-29",
...
, OAuth 2.0 (referred to as simply "OAuth" in the following) has gained massive traction in the market
...
, Section 4.5.4). With the increasing
...
¶
OAuth 2.1, under developement as
[
I-D.ietf-oauth-v2-1
]
, will incorporate
...
Section 4.4
) is REQUIRED. To this end, clients
SHOULD
¶
use the
iss
...
responses), processing it as described in
[
RFC9207
]
.
...
preventing the use of constant PKCE challenge or OpenID Connect
nonce
values.
¶
...
access token replay as described in
Section 4.1
,
Section 4.2
,
...
¶
Clients SHOULD instead use the response type
code
(i.e., authorization
...
and that clients make use of this Authorization Server Metadata to configure themselves
when available.
¶
...
¶
It is RECOMMENDED to use end-to-end TLS according to
[
BCP195
]
...
connections. To this end, authorization servers MUST NOT allow redirect URIs that use the
http
...
, a threat model is laid out that describes the threats against
which OAuth deployments must be protected. While doing so,
[
RFC6819
]
makes
...
relationships involving multiple parties (as described in
Section 1
), to
...
. This is a minimal attacker model.
...
by using authorization server metadata as described in
Section 2.6
. Such an attacker is therefore not listed here.
¶
...
checking of redirect URIs (see
Section 4.1
), problems
existing on mobile operating systems (where different apps can register
themselves on the same URI), mix-up attacks (see
Section 4.4
), where the
...
using an attacker-controlled resource server. Also see
Section 4.9.2
.
¶
...
are not listed here, except where new recommendations are made.
¶
This section further defines additional requirements beyond those defined in
Section 2
for certain cases and protocol options.
¶
4.1.
...
). Insufficient validation of the redirect URI effectively breaks
...
For a client using the grant type
code
, an attack may work as
follows:
¶
Assume the redirect URL pattern
https://*.somesite.example/*
is
registered for the client with the client ID
s6BhdRkqt3
. The
intention is to allow any subdomain of
somesite.example
to be a
...
the registered redirect URL patterns for the client
s6BhdRkqt3
.
...
), the attack can be performed even without user
interaction.
¶
...
registers the redirect URL pattern
https://*.somesite.example/*
and
the authorization server interprets this as "allow redirect URIs
pointing to any host residing in the domain
somesite.example
". If an
attacker manages to establish a host or subdomain in
somesite.example
, the attacker can impersonate the legitimate client. This
could be caused, for example, by a subdomain takeover attack
[
research.udel
]
, where an
outdated CNAME record (say,
external-service.somesite.example
)
...
) can be subject to a further kind of
...
Referer HTTP header (see
[
RFC9110
]
, Section 10.1.3), by leaking either
from the authorization server's or the client's website, respectively. Most
importantly, authorization codes or
state
values can be disclosed in
this way. Although specified otherwise in
[
RFC9110
]
, Section 10.1.3,
the same may happen to access tokens conveyed in URI fragments due to
browser implementation issues, as illustrated by a (now fixed) issue in the Chromium project
[
bug.chromium
]
.
...
In a similar way, an attacker can learn
state
...
to the document (either as
part of the "referrer" meta attribute or by setting a
Referrer-Policy header). For example, the header
Referrer-Policy:
no-referrer
...
, Section 4.1.2, authorization codes
...
further recommends that, when an
...
state
leaks from the
authorization server's website, since then the
state
...
client.example/redirection_endpoint?code=abcd
as a result of a
...
the way described in Section 2.3 of
[
RFC6750
]
. The authorization
...
¶
the implicit or authorization code grant is used with multiple authorization servers
...
¶
The user selects to start the grant using A-AS (e.g., by clicking on a button on the
client's website).
¶
The client stores in the user's session that the user selected
...
.
¶
When the user's browser navigates to the attacker's authorization endpoint,
...
redirected by the client to that authorization server), as in Attacker A2 (see
Section 3
). This capability
...
mechanism or replays access tokens or ID
...
example, if neither OAuth Authorization Server Metadata
[
RFC8414
]
nor OpenID Connect Discovery
[
OpenID.Discovery
]
is
...
example, via a separate response parameter
iss
, defined in
[
RFC9207
...
authorization response (see Attacker A3 in
Section 3
) can try to redeem the
...
¶
The attacker obtains an authorization code (see attacker A3 in
Section 3
). For the rest
of the attack, only the capabilities of a web attacker (A1) are required.
¶
From the attacker's device, the attacker starts a regular OAuth authorization
process with the legitimate client.
¶
In the response of the authorization server to the legitimate client, the
...
¶
All checks succeed and the authorization server issues access and
...
, Section 4.1.3, requires the authorization server to "... ensure that the
redirect_uri
parameter is present if the
redirect_uri
parameter
...
¶
Other providers just pattern match the
redirect_uri
parameter
...
deployments.
¶
It is also assumed that the requirements defined in
[
RFC6749
]
,
...
4.5.3.1.
PKCE
The PKCE mechanism specified in
[
RFC7636
]
...
the authorization server without knowledge of the
code_verifier
.
¶
4.5.3.2.
Nonce
OpenID Connect's existing
nonce
parameter can protect against authorization
code injection attacks. The
nonce
...
with the initial request to the OpenID Provider (OP). The OP puts the received
nonce
...
itself against code injection using the
nonce
parameter
¶
MUST validate the
...
¶
MUST ensure that, unless and until that check succeeds, all tokens (ID
...
inject the stolen code in Step 3 even if PKCE or
nonce
are used.
¶
...
the leaked access token. Since the response includes the
state
value
...
additionally contains an ID Token containing the
at_hash
claim. The attacker
...
the stolen access token, as the
nonce
...
, Section 5.3.5. The
same protection is provided by PKCE or the OpenID Connect
nonce
value.
¶
When using PKCE instead of
state
or
nonce
...
state values. One example of this is discussed in the now-expired draft
[
I-D.bradley-oauth-jwt-encoded-state
]
.
¶
The authorization server therefore MUST provide a way to detect their support for PKCE. Using Authorization Server Metadata according to
[
RFC8414
]
is RECOMMENDED, but authorization servers MAY instead provide a
...
can read the authorization response (see Attacker A3 in
Section 3
). When
state
is used or an ID Token is returned in the authorization response (e.g.,
response_type=code+id_token
), the attacker either learns the
state
value and
can replay it into the forged authorization response, or can extract the
nonce
from the ID Token and use it in a new request to the authorization server to
mint an ID Token with the same
nonce
...
particular flow. The presence or absence of the
code_challenge
parameter lends
...
¶
The second prerequisite for this attack is that the client is not using
state
...
achieves the same goal as in the attack described in
Section 4.7
: The attacker injects an
...
4.8.1.
Attack Description
The user has started an OAuth session using some client at an authorization server. In the
authorization request, the client has set the parameter
code_challenge=hash(abc)
as the PKCE code challenge (with the hash function and parameter encoding as defined in
[
RFC7636
]
). The client is now
...
¶
To conduct the attack, the attacker uses their own device to start an
authorization flow with the targeted client. The client now uses another
PKCE code challenge, say
...
¶
If the authorization server allows for flows without PKCE, it will create a
code that is not bound to any PKCE code challenge.
¶
The attacker now redirects the user's browser to an authorization response
URL that contains the code for the attacker's session with the authorization server.
¶
The user's browser sends the authorization code to the client, which will
...
that many OAuth clients do not use or check
state
properly.
¶
...
¶
This fact can be used to mitigate this attack.
[
RFC7636
]
already mandates that
¶
...
the token endpoint containing a
code_verifier
is rejected.
¶
...
(see Attackers A1 and A5 in
Section 3
). If the client sends a valid access token to
...
¶
Sender-constrained access tokens, as described in
Section 4.10.1
,
...
¶
Audience restriction as described in
Section 4.10.2
SHOULD be
...
where access tokens leak (see Attacker A5 in
Section 3
).
¶
4.10.
...
and
Section 4.9
. Some of these attacks can be mitigated by
...
¶
The authorization server associates data with the access token
...
client.
¶
This key material must be distributed somehow. Either the key
...
connection.
¶
The resource server must implement the actual proof of possession check. This
...
ensure that a replay of the proof of possession is not possible.
¶
...
resource server. The mechanism in
[
RFC8707
]
can be used for this or the
information can be encoded in the scope value (Section 3.3 of
[
RFC6749
]
).
...
had the same property since different
token-binding IDs must be associated with the access token. Using
[
RFC8705
]
, on the other hand, allows a client to use the
...
a non-standard Authorization Server Metadata parameter
resource_servers
:
¶
HTTP/1.1 200 OK
...
Section 4.1.2
. Another abuse case is to produce URLs that
...
¶
Intentionally send an erroneous authorization request, e.g., by using an
...
and
redirect_uri
controlled by the attacker. After the user authenticates, the
...
and
redirect_uri
controlled by the attacker. In this
...
trustworthiness of content behind the URI, and the source of the redirect URI and
other client data.
¶
...
, the HTTP status code 302 is used for this purpose, but
...
, Section 15.4.8. This status code does not require the user
...
practice of reverse proxies to accept
X-Forwarded-For
headers and just
...
already provides robust baseline protection by requiring
¶
...
also lays the foundation for further
...
the client credentials grant, the value of
sub
...
namespace for client IDs and user identifiers exists, such as in the
sub
claim
...
4.16.
Clickjacking
As described in Section 4.4.1.9 of
[
RFC6819
]
...
instead of HTTP redirects, messages may
...
communication are described in
[
research.rub
]
:
...
is a variant of the CSRF attacks described in
Section 4.7
. The
countermeasures described in
Section 4.7
apply to this attack as well.
¶
In the case of a maliciously injected token response, sender-constrained
access tokens as described in
Section 4.10.1
may prevent the attack under
...
Section 4.1.3
. Authorization servers MUST send postMessages to
...
the authorization response listed in
Section 2.1
...
Work in Progress
,
Internet-Draft, draft-ietf-oauth-token-binding-08
,
19 October 2018
,
<
...
,
<
https://www.nds.rub.de/media/ei/arbeiten/2021/05/03/masterthesis.pdf
>
.
[research.ubc]
Sun, S.-T.
...
Reworded
Section 4.15.1
to be more in line with OAuth 2.1
¶
Editorial changes
¶
