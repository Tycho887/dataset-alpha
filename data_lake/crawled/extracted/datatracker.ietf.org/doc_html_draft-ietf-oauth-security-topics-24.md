---
{
  ...
  "url": "https://datatracker.ietf.org/doc/html/draft-ietf-oauth-security-topics-24",
  ...
  "date": "",
  "length": 116886,
  "crawled_at": "2026-04-23T20:50:00"
}
---
...
OAuth implementations are being attacked through known implementation
  ...
  in the OAuth 2.0 Threat Model and Security Considerations
[
RFC6819
]
,
...
ensured that the authorization server supports PKCE
[
RFC7636
]
MAY
...
and that clients make use of this metadata to configure themselves
when available.
¶
...
connections. To this end, AS MUST NOT allow redirect URIs that use the
http
...
, an attacker model is laid out that describes the
...
(as described in
Section 1
), to include new types of attackers and to define
...
using a attacker-controlled RS. See also
Section 4.9.2
.
¶
...
mitigations already covered in
[
RFC6819
]
are not listed here, except
...
somesite.example
, he can impersonate the legitimate client. This
...
native apps using a
localhost
URI: In this case, the AS MUST allow variable
...
, Section 5.5.2), by leaking either
...
¶
The user selects to start the grant using A-AS (e.g., by clicking on a button at the
client's website).
¶
...
¶
The user authorizes the client to access her resources at H-AS. (Note that a
...
: This variant works only if the attacker can
...
redirected by the client to that AS), as in Attacker A2 (see
Section 3
). This capability
...
¶
Implicit Grant
: In the implicit grant, the attacker receives an access
...
: If clients use different redirect URIs for
...
¶
OpenID Connect
: There are variants that can be used to attack OpenID
...
example, if neither OAuth metadata
[
RFC8414
]
...
¶
From the attacker's own device, the attacker starts a regular OAuth authorization
...
, Section 4.1.3, requires the AS to "... ensure that the
redirect_uri
...
associated with a
code_challenge
that does not match this verifier. PKCE is a
...
nonce
value is one-time use and created by the
...
properly checks the
nonce
parameter in the ID Token and does not use any
...
today (originally intended for OAuth native apps) whereas
nonce
is
...
is RECOMMENDED, but AS MAY instead provide a
...
4.8.1.
Attack Description
The user has started an OAuth session using some client at an AS. In the
authorization request, the client has set the parameter
code_challenge=sha256(abc)
as the PKCE code challenge. The client is now
waiting to receive the authorization response from the user's browse.
¶
To conduct the attack, the attacker uses his own device to start an
...
connection.
¶
The RS must implement the actual proof of possession check. This
...
): The approach as specified in this
...
contributed the TLS-related proposal to
[
RFC8705
]
,
...
has the same property since different
...
a non-standard metadata parameter
resource_servers
:
¶
HTTP/1.1 200 OK
...
the user.
¶
Intentionally send a valid silent authentication request (prompt=none)
with
client_id
and
redirect_uri
...
already provides a robust baseline protection by requiring
¶
...
also lays the foundation for further (implementation
...
client_id
or any claim that could cause confusion with a genuine resource
...
Work in Progress
,
Internet-Draft, draft-sakimura-oauth-jpop-05
,
22 July 2019
,
<
https://datatracker.ietf.org/doc/html/draft-sakimura-oauth-jpop-05
>
.
[OAuth.Post]
Jones, M.
...
,
<
https://bugs.chromium.org/p/chromium/issues/detail?id=168213/
>
.
[owasp.redir]
