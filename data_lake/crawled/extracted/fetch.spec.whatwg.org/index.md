...
environment
’s
WebDriver BiDi network is offline
.
To
...
.
HTTP whitespace
is only useful for specific constructs that are reused outside
the context of HTTP headers (e.g.,
MIME types
). For HTTP header values, using
HTTP tab or space
...
it.
Normalization
is done for backwards compatibility and
consistency across APIs as
methods
...
CHICKEN
` is perfectly
acceptable (and not a misspelling of `
CHECKIN
...
uses the more colloquial term "header".
[HTTP]
A
header list
is a
...
A
header list
is essentially a specialized multimap: an ordered list of
...
header list
list
, run these steps. They return null
or a
header value
.
If
...
The definition of
header value
is not defined in terms of the
field-value
token production as it is
not compatible with deployed content
.
To
normalize
a
...
have this value start with `
Mozilla/5.0 (
` and be generally modeled after other web
browsers.
To get the
environment settings object
environment
...
.
processBodyError
must be an algorithm optionally accepting an
exception
.
If
taskDestination
...
’s
stream
. If that threw an exception, then run
errorSteps
...
’s
URL list
. It is provided as a distinct field solely for
...
navigation requests
and worker requests, but not service
worker requests. It references an
environment
for a
navigation request
and an
...
. This provides a convenient way for standards to not have to
  explicitly set a request’s
traversable for user prompts
.
a
traversable navigable
The UI shown will be associated with the browser interface elements that are displaying that
traversable navigable
.
When displaying a user interface associated with a request in that request’s
traversable for user prompts
, the user agent should update the address bar to
display something derived from the request’s
current URL
(and not, e.g., leave
...
traversable for user prompts
, especially in the case of cross-origin requests.
...
device to assist defining CSP and Mixed Content. It is not exposed to JavaScript.
[CSP]
[MIX]
A
destination type
...
(null or an
implementation-defined
object). Unless otherwise stated it is null.
A
request
has an associated
origin
...
during
fetching
. It provides a convenient way for standards to not have to set
request
’s
origin
.
...
", makes
  the request a
CORS request
— in which case, fetch will return a
network error
if the
  requested resource does not understand the
CORS protocol
, or if the requested resource is
...
. Upon success, fetch will return an
...
webtransport
"
This is a special mode used only by
...
The
use-CORS-preflight flag
being set is one of several conditions that results
in a
CORS-preflight request
. The
use-CORS-preflight flag
is set if either one or more
event listeners are registered on an
...
omit
"
Excludes credentials from this request, and causes any credentials sent back in the response
  to be ignored.
"
same-origin
"
Include credentials with requests made to same-origin URLs, and use any credentials sent back
  in responses from same-origin URLs.
"
include
"
Always includes credentials with this request, and always use any credentials sent back in the
  response.
Request
’s
credentials mode
...
" and
fetch
does not currently account for other
 values. If
HTML
changes here, this standard will need corresponding changes.
A
request
has an associated
...
URL
’s
credentials are preferred over that of the
authentication entry
. Modern specifications avoid
setting this flag, since putting credentials in
URLs
is discouraged, but some older
...
"
Fetch
will inspect the HTTP cache on the way to the network. If the HTTP cache
  contains a matching
fresh response
it will be returned. If the HTTP cache contains a
  matching
stale-while-revalidate response
it will be returned, and a conditional network
  fetch will be made to update the entry in the HTTP cache. If the HTTP cache contains a matching
stale response
, a conditional network fetch will be returned to update the entry in
...
reload
"
Fetch behaves as if there is no HTTP cache on the way to the network. Ergo, it creates a
  normal request and updates the HTTP cache with the response.
"
no-cache
"
Fetch creates a conditional request if there is a response in the HTTP cache and a normal
...
Return a
network error
when a request is met with a redirect.
"
manual
"
Retrieves an
...
and
parser metadata
are generally populated from attributes and flags on the HTML
element responsible for creating a
request
. They are used by various algorithms in
Content Security Policy
to determine whether requests or responses are to be blocked in
a given context.
[CSP]
A
request
...
This list is for exclusive use by
WebTransport(url, options)
when
options
contains
...
more
URLs
). Unless stated otherwise, it is a list containing a copy of
request
’s
URL
.
...
is 0 and
last
is 500, is a range of 501 bytes.
...
’s
URL list
is not directly exposed to script as that would violate
atomic HTTP redirect handling
.
A
response
...
to determine which headers to expose.
A
response
has an associated
...
this is also tracked internally using the request’s
timing allow failed flag
.
A
response
...
(a
response body info
). Unless stated otherwise, it is a new
response body info
.
A
response
...
is a
response
that offers a limited view on an associated
response
. This associated
response
...
filtered response
’s associated concepts (such as its
body
) refer to the associated concepts of its
internal response
. (The exceptions to this are listed below as part
of defining the concrete types of
filtered responses
.)
The
fetch
...
processResponse
and
 equivalent parameters exposes
filtered responses
to callers to ensure they do not
 accidentally leak information. If the information needs to be revealed for legacy reasons, e.g., to
 feed image data to a decoder, the associated
internal response
can
 be used by specification algorithms.
New specifications ought not to build further on
opaque filtered responses
or
opaque-redirect filtered responses
. Those are legacy constructs and cannot always be
...
and an
opaque-redirect filtered response
are nearly indistinguishable from a
network error
.
 When introducing new APIs, do not use the
internal response
for
...
of a
response
is exposed to script through the
type
getter:
console
.
...
fetch
(
"https://crossorigin.example/image"
,
{
mode
:
...
ASCII string
requestFragment
, is the value returned by the following steps. They return null, failure,
or a
URL
.
If
...
location
will
  only parse successfully if it is an
...
The
location URL
algorithm is exclusively used for redirect
handling in this standard and in
HTML
’s navigate algorithm which handles redirects
manually.
[HTML]
2.2.7.
Miscellaneous
...
A
fetch group
holds information about fetches.
A
fetch group
has associated:
fetch records
...
or null.
A
deferred fetch record
is a
struct
used to maintain state needed to
...
It is also
implementation-defined
whether other operations might be performed to get
  connection information beyond just
IP addresses
. For example, if
origin
’s
...
is an
HTTP(S) scheme
, the implementation might perform a DNS query
  for HTTPS RRs.
[SVCB]
...
The results of
resolve an origin
may be cached. If they are cached,
key
should
...
key
being taken into account. Depending on the implementation it might also not be
 possible to take
key
...
If there is a
connection
capable of supporting unreliable transport in
connections
, e.g., HTTP/3, then return that
connection
.
Let
proxies
be the result of finding proxies for
url
in an
implementation-defined
manner. If there are no proxies, let
proxies
be
  « "
DIRECT
...
Let
connection
be the result of running this step: run
create a connection
given
key
,
...
number of times,
in parallel
from each other, and wait for
    at least 1 to return a value. In an
implementation-defined
manner, select a value to
...
keyed on
credentials
. The latter clarifies that, e.g., TLS session identifiers are not
reused across
connections
whose
credentials
...
and use
connection
to establish an HTTP connection to
host
, taking
proxy
and
...
If
requireUnreliable
is true, then establish a connection capable of unreliable
   transport, e.g., an HTTP/3 connection.
[HTTP3]
...
If
credentials
is false, then do not send a TLS client certificate.
If
webTransportHashes
is not empty
...
to
connection
’s ALPN Protocol ID, with the following caveats:
[RFC7301]
...
’s
ALPN negotiated protocol
is intended to identify the network
...
If the user agent used TLS False Start for this connection, this interval must not include
   the time needed to receive the server’s Finished message.
[RFC7918]
...
Suppose the user agent establishes an HTTP/2
  connection over TLS 1.3 to send a
GET
request and a
POST
request. It
  sends the ClientHello at time
t1
and then sends the
GET
request with early
...
request is not safe (
[HTTP]
, section 9.2.1), so the user
  agent waits to complete the handshake at time
t2
before sending it. Although both
  requests used the same connection, the
GET
...
t2
.
If a secure transport is used,
timingInfo
’s
secure connection start time
should be the result of calling
unsafe shared current time
immediately before starting the handshake process to
 secure
connection
.
[TLS]
...
key
is null, then return null.
Return the unique HTTP cache associated with
key
.
[HTTP-CACHING]
2.9.
...
is a
bad port
if it is listed in the first column of the following table.
Port
Typical service
0
—​
...
to
request
be blocked due to its MIME type?
Run these steps:
Let
mimeType
be the result of
...
request
:
If the user agent is configured to disable cookies for
request
, then it should
 return.
Let
sameSite
be the result of
determining the same-site mode
for
request
.
Let
...
be true if the result of
determine the same-site mode
for
request
is "
strict-or-less
...
Its possible
values
are all the return values of
byte-serializing a request origin
, given a
request
.
This supplants the definition in
The Web Origin Concept
.
[ORIGIN]
To
...
all fetches where the fetcher did not explicitly opt into sharing their
origin
with the
...
firewall (intranets). Additionally, for
requests
including
credentials
it needs to be opt-in to prevent leaking potentially-sensitive data.
This section explains the
CORS protocol
as it pertains to server developers.
Requirements for user agents are part of the
fetch
algorithm,
except for the
new HTTP header syntax
.
3.3.1.
General
The
CORS protocol
consists of a set of headers that indicates whether a response can
be shared cross-origin.
For
requests
that are more involved than what is possible with HTML’s
form
element, a
CORS-preflight request
...
A
CORS request
is an HTTP request that includes an
`
Origin
...
a future
CORS request
to the same resource might use.
A
CORS-preflight request
...
same-origin
", i.e., it excludes
  credentials, but for any subsequent
CORS requests
it might not be. Support therefore needs
  to be indicated as part of the HTTP response to the
CORS-preflight request
as well.
An HTTP response to a
CORS-preflight request
...
header
is
  not relevant for the purposes of the
CORS protocol
.
`
...
Indicates which
headers
can be exposed as part
 of the response by listing their
names
.
...
stated above with
values
matching up with the request.
A successful HTTP response to a
CORS-preflight request
is similar, except it is restricted
to an
ok status
, e.g., 200 or 204.
...
include
" it
has an impact on the functioning of the
CORS protocol
...
exist for a
request
can it be observed by virtue of the
credentials
being included. Note that even so, a
CORS-preflight request
never includes
credentials
.
...
credentials
can be shared. And also needs to decide if
requests
necessitating a
CORS-preflight request
can
include
credentials
. Generally speaking, both sharing responses and allowing requests
with
credentials
is rather unsafe, and extreme care has to be taken to avoid the
...
. (Neither
credentials
nor response header access is
 important.)
var
url
=
"https://bar.invalid/api?key=730d67a37d7f3d802e96396d00280768773813fbe726d116944d814422fc1a45&data=about:unicorn"
;
fetch
(
url
...
This will use the
CORS protocol
, though this is entirely transparent to the
 developer from
foo.invalid
. As part of the
CORS protocol
...
` response header. If the request
 would have included
credentials
, the response header names would have to be listed
 explicitly and `
*
` could not be used.
The developer of
...
CORS protocol
is no longer transparent to the developer as
credentials
...
credentials
in the request.
 It will also put stricter requirements on the response. Not only will
bar.invalid
need
 to list `
https://foo.invalid
...
failure
callback will be invoked. However, any `
Set-Cookie
...
non-preflighted requests with the following non-safelisted `
Content-Type
` header
values:
`
...
defined here as the model defined in HTTP is not compatible with web content.
[HTTP]
To
extract a length
from a
...
header list
headers
, run these steps. They return failure or a
MIME type
.
Let
charset
...
When
extract a MIME type
returns failure or a
MIME type
whose
essence
is incorrect for a given format, treat this as a fatal error.
...
’s
parameters
can typically be safely ignored.
This is how
extract a MIME type
...
mimeType
to be failure so it can be more easily combined with
extract a MIME type
.
...
response
header
can be used to require checking of a
response
’s
`
Content-Type
...
pertain to them. Also, considering "
image
...
response
header
can be used to require checking a
request
’s
current URL
’s
...
forNavigation
set to
true, and it’s always for nested navigations. Otherwise,
response
is either the
internal response
of an
...
`
  ends up as
allowed
below as it will never match anything, as long as
embedderPolicyValue
is "
unsafe-none
".
...
serializedURL
be the result of
serializing a response URL for reporting
with
response
.
Let
...
Let
body
be a new object containing the following properties:
key
value
"
type
...
fetching
. In broad strokes, it takes
a
request
and one or more algorithms to run at various points during the operation. A
response
is passed to the last two algorithms listed below. The first two algorithms
...
useParallelQueue
(default false), run
the steps below. If given,
processRequestBodyChunkLength
must be an algorithm accepting
an integer representing the number of bytes transmitted. If given,
processRequestEndOfBody
must be an algorithm accepting no arguments. If given,
processEarlyHintsResponse
must be an algorithm accepting a
response
. If
...
.
Run the
WebDriver BiDi clone network request body
steps with
request
.
...
be an algorithm that runs the following
   step given a
response
response
: set
...
` header value
.
Otherwise, the user agent should set
value
to the first matching statement, if
    any, switching on
request
’s
destination
...
The
implementation-defined
object could encompass stream weight and
...
for transports where it applies (including HTTP/3), and equivalent information used to prioritize
  dispatch and processing of HTTP/1 fetches.
[RFC9218]
If
request
is a
...
Upgrade
request
to a potentially trustworthy URL, if appropriate
.
Upgrade a mixed content
request
to a potentially trustworthy URL, if appropriate
.
If
...
should
request
be blocked by Integrity Policy Policy
returns
blocked
, then set
response
...
As stated in
Referrer Policy
, user agents can provide the end user with
  options to override
request
’s
referrer
...
includeSubDomains
directive
   or a congruent match (with or without an asserted
includeSubDomains
directive)
[HSTS]
; or
...
implementation-defined
. As DNS
  operations are not traditionally performed until attempting to
obtain a connection
, user
...
If
recursive
is false, then run the remaining steps
in parallel
.
If
response
is null, then set
response
to the result of running the steps
...
Otherwise, if
headerNames
is non-null or failure, then set
response
’s
CORS-exposed header-name list
to
...
is not a
network error
and any of the following returns
blocked
should
internalResponse
to
...
content type
to the result of
minimizing a supported MIME type
given
mimeType
.
If
...
This
TransformStream
is needed for the purpose of receiving a notification when
  the stream reaches its end, and is otherwise an
...
potentially override response for a request
on
request
.
If
...
response
or null. Its behavior is
implementation-defined
, allowing user agents to intervene on the
request
by
...
body
be [
insert a byte sequence representing the shimmed
      content here
].
Return a new
response
...
slice blob
algorithm, we have to increment
rangeEnd
.
Let
slicedBodyWithType
...
in
transformStream
. The user agent may split the chunk into
implementation-defined
practical sizes and
enqueue
each of
       them. The user agent also may concatenate the chunks into an
implementation-defined
practical size and
enqueue
it.
...
If
makeCORSPreflight
is true and one of these conditions is true:
There is no
...
and if there is no suitable entry
    it performs a
CORS-preflight fetch
which, if successful, populates the cache. The purpose
    of the
CORS-preflight fetch
is to ensure the
fetched
resource is
    familiar with the
CORS protocol
. The cache is there to minimize the number of
...
As the
CORS check
is not to be applied to
responses
whose
status
is 304 or 407, or
responses
from a service worker for
...
The
cross-origin resource policy check
runs for responses coming from the
...
is non-null, and the
connection
uses HTTP/2, then user agents
    may, and are even encouraged to, transmit an
RST_STREAM
frame.
...
’s
stream
is still being enqueued to after
 returning.
4.5.
HTTP-redirect fetch
To
...
’s
source
is null as only a single body is needed in
      that case. E.g., when
request
’s
body
...
inflightKeepaliveBytes
is greater
     than 64 kibibytes, then return a
network error
.
The above limit ensures that requests that are allowed to outlive the
environment settings object
and contain a body, have a bounded size and are not allowed
...
httpRequest
.
Append the Fetch metadata headers for
httpRequest
.
[FETCH-METADATA]
If
...
Set
storedResponse
to the result of selecting a response from the
httpCache
...
’s
status
is in the range 200 to 399, inclusive,
...
and
password
be the result of prompting the end user
     for a username and password, respectively, in
request
’s
traversable for user prompts
.
...
fetchParams
.
Prompt the end user as appropriate in
request
’s
traversable for user prompts
...
connection
be the result of
obtaining a WebTransport connection
,
   given
networkPartitionKey
and
...
Set
response
to the result of making an HTTP request over
connection
using
request
...
’s
source
is null, then the user agent may have a
      buffer of up to 64 kibibytes and store a part of
request
’s
body
in that buffer. If the user agent reads from
request
’s
body
beyond that buffer’s size and the user agent needs to resend
request
, then instead
      return a
network error
...
object, which means
body
cannot be recreated and that is why the buffer is needed.
While true:
Set
timingInfo
’s
...
Let
status
be the HTTP response’s status code.
If
status
is in the range 100 to 199, inclusive:
If
timingInfo
’s
first interim network-response start time
...
is a
traversable navigable
, then make the dialog available in
request
’s
traversable for user prompts
.
...
If the size of
buffer
is smaller than a lower limit chosen by the user agent and the
     ongoing fetch is
suspended
,
resume
...
.
Run the
WebDriver BiDi clone network response body
steps with
request
and
response
...
If
includeCredentials
is true, then the user agent should
parse and store response `
Set-Cookie
` headers
given
...
:
While true:
If one or more bytes have been transmitted from
response
’s message body, then:
Let
bytes
be the transmitted bytes.
Let
codings
be the result of
extracting header list values
...
Otherwise, if
codings
[0] is the empty string, or it is supported by the
         user agent, and is a
byte-case-insensitive
match for an entry listed in the
...
If the size of
buffer
is larger than an upper limit chosen by the user agent, ask
         the user agent to
suspend
the ongoing fetch.
Otherwise, if the bytes transmission for
response
’s message body is done
       normally and
stream
is
readable
...
RST_STREAM
frame.
Otherwise, the user agent should close
connection
unless it would be bad for
...
These are run
in parallel
as at this point it is unclear whether
response
’s
body
is relevant (
...
If
max-age
is failure or null, then set
max-age
to 5.
If
max-age
is greater than an imposed limit on
max-age
, then set
max-age
to the imposed limit.
If the user agent does not provide for a
cache
, then
   return
response
...
max-age
field have passed since storing the entry.
Cache entries
may
...
’s
origin
would be the container document’s
origin
and the
TAO check
would return failure. Since navigation timing
  never validates the results of the
TAO check
, the nested document would still have access
...
timeout.
The
deferred fetch task source
is a
task source
used to update the result of a
deferred fetch. User agents must prioritize tasks in this
task source
before other task
sources, specifically task sources that can result in running scripts such as the
DOM manipulation task source
, to reflect the most recent state of a
fetchLater()
call before running any scripts that might depend on it.
To
queue a deferred fetch
given a
request
request
, a null or
...
If
activateAfter
is non-null, then run the following steps
in parallel
:
...
The deferred-fetch quota is allocated to a
...
The top-level
document
, and subsequently its nested documents, can control how much
...
document
, only 64 kibibytes can be used
concurrently for the same reporting origin (the
request
’s
URL
’s
origin
). This prevents a situation where particular third-party libraries would reserve
...
would throw due to
 ...
 size of the request includes the
URL
itself, the
body
, the
...
In the following sequence, the first two requests would succeed, but the third one would throw.
 ...
 3rd request exceeds the reporting-origin quota for
https://a.example.com
, and would
 throw.
fetchLater
...
640 kibibytes are initially granted to the
top-level traversable
.
...
minimal quota
, which is 8 kibibytes, and
normal quota
, which is 0 or 64 kibibytes. Unless
...
document
’s
deferred-fetch control document
.
Let
navigable
be
...
Let
quota
be the result of the first matching statement:
isTopLevel
is true and
deferredFetchAllowed
is false
...
If
quota
is equal or less than 0, then return 0.
If
quota
is less than
quotaForRequestOrigin
...
navigable
’s parent document. It potentially reserves either 64kb or 8kb of quota for
...
the document ends up being
same origin
with its parent, the quota would be
freed
.
Set
container
...
document
is created. It ensures that same-origin
...
fetch
a resource and extract its contents as a
Blob
:
fetch
(
...
This
can be a pointer to the
header list
of something else, e.g.,
of a
request
as demonstrated by
Request
...
object.
init
can be used to fill its internal header list,
...
Returns whether there is a header whose name is
name
.
headers
.
...
with
value
and removes any remaining headers whose name is
name
.
for(const [
name
...
headerName
of
privileged no-CORS request-header names
:
Delete
headerName
from
...
Passing a dummy
header value
ought not to have any negative repercussions.
If
this
’s
guard
...
are the return value of running
sort and combine
with
this
’s
...
’s
type
attribute is not the empty byte sequence, set
type
to its value.
byte sequence
Set
...
If
action
is non-null, then run these steps
in parallel
:
Run
action
.
Whenever one or more bytes are available and
stream
is not
errored
,
...
Body
interface mixin have an associated
body
(null or a
body
).
...
The
consume body
algorithm, given an object that includes
Body
object
and an algorithm that takes a
byte sequence
and returns a JavaScript value or throws an exception
...
with
this
and the following step given a
byte sequence
bytes
: return the result of
creating
...
with
this
and the following steps given a
byte sequence
bytes
:
Let
...
If
mimeType
is non-null, then switch on
mimeType
’s
essence
and run the corresponding steps:
"
multipart/form-data
"
Parse
...
` header contains a
      `
filename
` parameter must be parsed into an
entry
whose
      value is a
File
object whose contents are the contents of the part. The
name
attribute of the
File
object must have the value of the `
filename
` parameter
      of the part. The
type
...
If that fails for some reason, then
throw
a
TypeError
.
...
The
init
argument is an object whose properties can be set as follows:
method
A string to set
request
’s
...
A
Headers
object, an object literal, or an array of two-item arrays to set
request
’s
headers
.
...
request
follows redirects, results in an error upon
...
Returns a
Headers
object consisting of the headers associated with
request
.
...
.
destination
Returns the kind of resource requested by
request
, e.g., "
document
" or
...
.
referrer
Returns the referrer of
request
. Its value can be a same-origin URL if
 explicitly set in
init
, the empty string to indicate no referrer, and
 "
about:client
" when defaulting to the global’s default. This is used during
 fetching to determine the value of the `
Referer
` header of the request being made.
request
.
referrerPolicy
Returns the referrer policy associated with
request
. This is used during
 fetching to compute the value of the
request
’s referrer.
request
.
...
Returns
request
’s subresource integrity metadata, which is a cryptographic hash of
 the resource being fetched. Its value consists of multiple hashes separated by whitespace.
[SRI]
request
.
keepalive
...
request
can outlive the global in which
 it was created.
request
.
isReloadNavigation
...
.
signal
Returns the signal associated with
request
, which is an
AbortSignal
object indicating whether or not
request
has been aborted, and its abort
 event handler.
request
.
duplex
...
origin
is only significant for navigation requests being handled by a service
...
signal
to the result of
creating a dependent abort signal
from
signals
, using
AbortSignal
...
Creates a
Response
whose body is the JSON-encoded
data
, and status, status
 message, and headers are provided by
init
.
response
.
...
Returns
response
’s URL, if it has one; otherwise the empty string.
response
.
redirected
Returns whether
response
was obtained through a redirect.
response
.
status
Returns
...
["
status
"] is not in the range 200 to 599, inclusive,
 then
throw
a
RangeError
...
["
statusText
"] is not the empty string and does not match
 the
reason-phrase
...
redirect, do this directly through the API, e.g.,
...
Let
requestObject
be the result of invoking the initial value of
Request
as
 constructor with
input
and
init
as arguments. If this throws an exception,
reject
p
with it and return
p
...
The user agent may
terminate
an ongoing fetch if that termination
...
data:
URLs
For an informative description of
data:
URLs, see RFC 2397. This section replaces
that RFC’s normative processing requirements to be compatible with deployed content.
[RFC2397]
A
data:
URL struct
...
If
mimeType
ends with U+003B (;), followed by zero or more U+0020 SPACE, followed by
  an
ASCII case-insensitive
match for "
base64
...
worker layer (typically through a
Request
object). Developers have almost no control over
forbidden request-headers
, but can control `
Accept
...
(unfortunately relatively common still), using the
CORS protocol
is
unsafe
...
authentication, using the above
header
will not reveal
...
the aforementioned
header
is not to be included. If it can be accessed
however, it is perfectly fine to do so.
CORS protocol and HTTP caches
If
CORS protocol
requirements are more complicated than setting
`
Access-Control-Allow-Origin
` to
*
...
` is
not
used and a server is
configured to send `
Access-Control-Allow-Origin
` for a certain
resource only in response to a
CORS request
. When a user agent receives a response to a
non-
CORS request
for that resource (for example, as the result of a
navigation
request
), the response will lack `
Access-Control-Allow-Origin
`
and the user agent will cache that response. Then, if the user agent subsequently encounters a
CORS request
for the resource, it will use that cached response from the previous
non-
CORS request
, without `
Access-Control-Allow-Origin
...
or a static
origin
for a particular resource, then configure the server
to always send `
Access-Control-Allow-Origin
...
WebSocket
object initiates a special kind of
fetch
(using a
request
whose
...
. Ultimately this results in fetch calling into
WebSockets
to obtain a dedicated connection.
[WEBSOCKETS]
[HSTS]
...
for a
response
. In reality it is rather complex mechanism for standards to adopt and use
...
’s
destination
using the guidance in the
destination table
.
Destinations
affect
Content Security Policy
and have other implications such as the `
Sec-Fetch-Dest
`
...
to the
environment settings object
you’re operating in. Web-exposed APIs are generally defined with
Web IDL, for which every object that implements an
interface
has a
relevant settings object
...
’s
relevant settings object
. All features that are directly web-exposed
by JavaScript, HTML, CSS, or other
Document
subresources should have a
client
.
If your
fetching
is not directly web-exposed, e.g., it is sent in the background
without relying on a current
Window
or
Worker
, leave
...
, and
referrer
to appropriate values instead, e.g., by copying them from the
environment settings object
ahead of time. In these more advanced cases, make sure the
details of how your fetch handles
Content Security Policy
and
referrer policy
...
the requests, in which case set your
request
’s
credentials mode
to
...
include
".
Figure out if your fetch needs to be reported to
Resource Timing
, and with which
initiator type
. By passing an
...
, reporting to
Resource Timing
will be done automatically once the
fetch is done and the
response
is fully downloaded.
[RESOURCE-TIMING]
...
a
header list
that contains those headers, e.g., « (`
My-Header-Name
`,
`
My-Header-Value
...
the
fetch
operation takes several optional
...
Once the
request
is set up, to determine which algorithms to pass to
fetch
...
’s
body
is read in its entirety into a
byte sequence
, and then processed by the caller.
To process a
response
upon completion, pass an algorithm as the
processResponseConsumeBody
argument of
fetch
. The given
  algorithm is passed a
response
and an argument representing the fully read
body
(of the
response
’s
internal response
). The second argument’s values have the following
  meaning:
null
The
response
’s
body
is null, due to the response being a
network error
or having a
null body status
.
...
’s
body
failed, e.g., due to an I/O error.
a
byte sequence
Fully reading
the contents of the
...
origin
. For example, the caller may use contents of a "
no-cors
"
response
to display image contents directly to the user, but those image contents
...
If
contents
is null or failure, then present an error to the user.
Otherwise, parse
contents
considering the metadata from
response
,
      and perform your own operations on it.
Headers first, then chunk-by-chunk
In some cases, for example when playing video or progressively loading images, callers might
  want to stream the response, and process it one chunk at a time. The
response
is
...
To process a
response
chunk-by-chunk, pass an algorithm to the
processResponse
argument of
fetch
. The given
  algorithm is passed a
response
when the response’s headers have been
...
’s
stream
in order to download the rest
  of the response. For convenience, you may also pass an algorithm to the
processResponseEndOfBody
argument, which is called once you have finished
...
The
processResponse
argument is also useful for handling the
response
’s
header list
and
...
without
  handling the
body
at all. This is used, for example, when handling responses
  that do not have an
ok status
.
Let
...
is a
network error
, then present an error to the user.
Otherwise, if
response
’s
status
...
’s
stream
, and process in an appropriate way for the
      MIME type identified by
extracting a MIME type
from
response
’s
...
response
at all, e.g., in the case of
navigator.sendBeacon()
. Processing a response and passing callbacks to
fetch
is optional, so omitting the callback would
fetch
without
  expecting a response. In such cases, the
response
’s
body
’s
stream
will be discarded, and the caller does not have to worry about downloading
...
whose
status
is 103, and is currently handled only by
navigations.
processRequestBodyChunkLength
and
processRequestEndOfBody
notify the caller of request body uploading
progress.
Note that the
fetch
operation starts in the same thread from which it was called,
and then breaks off to run its internal operations
in parallel
. The aforementioned callbacks
are posted to a given
event loop
...
responses
in parallel
and handle interactions with the main thread by yourself,
fetch
with
useParallelQueue
set to true.
...
To manipulate a
fetch
operation that has already started, use the
fetch controller
returned by calling
fetch
. For example, you may
...
the
fetch controller
due the user or page logic, or
terminate
it due to browser-internal circumstances.
In addition to terminating and aborting, callers may
report timing
if this was not done automatically by passing the
initiator type
, or
extract full timing info
and handle it on the caller side (this is
done only by navigations). The
fetch controller
is also used to
process the next manual redirect
for
