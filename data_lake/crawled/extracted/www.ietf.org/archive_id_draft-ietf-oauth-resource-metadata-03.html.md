---
{
  ...
  "url": "https://www.ietf.org/archive/id/draft-ietf-oauth-resource-metadata-03.html",
  "domain": "www.ietf.org",
  "depth": 2,
...
.
 ...
 plays in OAuth Dynamic Client Registration
[
RFC7591
]
.
...
Each protected resource publishing metadata about itself makes its own
 ...
 the protected resource, as described in
Section 7.3
.
¶
...
¶
Section 2
defines metadata values that a protected
       ...
       These values may be used by other specifications, such as the
jwks_uri
used to publish public keys the resource server uses to sign
       resource responses, as described in Section 5.6.2 of
[
FAPI.MessageSigning
]
.
...
https
scheme and has no query or fragment components.
     Using these well-known resources is described in
Section 3
.
¶
...
document.
     ...
     The JWK Set MAY also contain signing key(s) that the resource
     server uses to sign responses.
...
Protected resources supporting metadata
 MUST make a JSON document containing metadata as specified in
Section 2
...
and the well-known URI path suffix is
...
A protected resource MAY use a
WWW-Authenticate
response to return a URL to its protected resource metadata to the client.
 The client can then retrieve protected resource metadata as described in
Section 3
.
...
¶
The resource server responds with a
WWW-Authenticate
...
¶
The protected resource responds with the protected resource metadata according to
Section 3.2
.
¶
The client validates the protected resource metadata.
¶
The client builds the authorization server metadata URL from an issuer identifier in the resource metadata according to
[
RFC8414
]
and makes a request to fetch the authorization server metadata.
¶
The authorization server responds with the authorization server metadata document according to
[
RFC8414
]
.
...
¶
The response below is an example of a
WWW-Authenticate
...
There are some existing methods by which an unrecognized client can make use of an authorization server,
   such as using Dynamic Client Registration
[
RFC7591
]
...
Resource servers MAY return other
WWW-Authenticate
...
An attacker may also attempt to impersonate a protected resource by publishing
   a metadata document that contains a
resource
claim
...
If a client expects to interact with multiple resource servers, the client
       SHOULD request audience-restricted access tokens using
[
RFC8707
]
,
...
While this attack is not explicitly enabled by this specification, and is possible in
       ...
       of audience-restricted access tokens and Resource Indicators
[
RFC8707
]
...
Work in Progress
,
Internet-Draft, draft-ietf-oauth-security-topics-23
,
5 June 2023
,
<
